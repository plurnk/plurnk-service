...
- **Newest-first rollback, staged.** The grinder folds the newest material in the packet, re-measuring between stages, and ONLY the newest: **stage 1** — the immediately-prior turn's emissions (turn N>1, the latest model output that pushed it over); still over → **stage 2** — the current turn's own **pre-model** rows (the grinder runs pre-generate, so every current-turn row is engine-written: turn 1's foists, a wake turn's auto-surfaced stream conclusion — #332's 68KB search payload was exactly this, riding open to a needless death while the grinder folded only the tiny prior turn). The engine may always fold what the engine wrote this turn; it never reaches older history — the model alone curates stale context via FOLD/KILL, and the engine never janitors. Folded, not deleted: rows and bodies persist and are re-OPENable, so log *history* is preserved while the render collapses to coordinates. {§grinder-layer1-rollback}
...
- **Hard stop.** If the packet still overflows after both stages, the loop abandons at **413 Content Too Large** (`engine_loop_set_status`) — the content genuinely won't fit, and the anchor's name is finally its status. Its sibling engine-imposed terminals are HTTP-precise too: `maxTurns` → 429, a strike-out → 500 (508 when cycle-driven) — no longer the old catch-all 499. No further passes. {§grinder-hard-413-abort}
