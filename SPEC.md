...
- **Groundless hibernate** {§send-groundless-hibernate} — the model emitted a retrieval op (`READ`/`FIND`/`OPEN` — a result that folds back next turn) and a `SEND[202]` in the *same* turn, with **no wake edge** — no live thing held (the pre-dispatch snapshot) and nothing wake-capable opened this turn. The retrieval's result folds back on the NEXT turn, but a park with no wake edge never *has* a next turn — the model is sleeping on its own unanswered question, an eternal park only an operator inject could revive (the config-lookup dead-park `PLAN → READ → SEND[202]`; the FIND twin wedged a live run for its full timeout, benchmarks/run15). Refused **409**, record-faithful (the row keeps its `[202]` attempt, the loop stays a continue, the steer strikes). This stays engine-side because the wake-edge test is **runtime** state, not a shape the parser can judge: wake-edge-capable ops ground the park from the emission itself, spawn-then-hibernate style — `EXEC` (stream conclusion / `<T,P>` poll, §exec-poll), `WORK(run://…)` (child-conclusion wake, §run-lifecycle-child-wake), directed `SEND(run://…)` (irc), an `http` `READ` (a web fetch streams). A **bare park holding nothing is legal** — the voice door: a sibling irc or operator inject wakes it (§actor-boundary-passive-wake), and the daemon surfaces the idle park as `loop/quiesced` (§run-lifecycle-quiesced) rather than the engine refusing it. Steer: *"Attempted [202] hibernation with submitted retrieval operation(s) — the results arrive NEXT turn, and nothing here would wake you. SEND[102] to receive them."*
...
- **Per-turn weight.** A markdown table groups render-weight by turn — the `loop/turn` coordinate prefix — listed chronologically (oldest first). On overflow the rail folds the run's whole open log in one move (§grinder); the model sees which turns are fat and can FOLD ahead of the rail. {§tokenomics-turn-totals}
...
**Decision — a pre-LLM grinder, fired only on actual overflow.** In `Engine.runTurn`, after the packet is assembled (`PacketBuilder.buildRequestPacket`) and before `provider.generate`, the assembled render-weight (§tokenomics) is measured against the ceiling. At or under → the packet ships untouched; the grinder never trims speculatively or "helpfully." {§grinder-overflow-only} On overflow it folds the run's open log rows (errors exempt), then hard-stops if that isn't enough:

- **One rule, every turn: fold everything.** On overflow the grinder folds **all** of the run's still-open log rows in one set-op — turn 1 and turn 101 alike, no staging, no newest-first ordering, no turn-number cases. The fold scope is the render scope (the packet renders the run's whole log). This is safe because folding is **reversible and lossless in the listing**: every folded row stays at its coordinate line, and the model re-OPENs precisely the rows it needs — the model knows its own working set; the engine never guesses which subset to spare. The pressure not to lean on the grinder is the strike (§grinder-compaction-strikes), not engine restraint. Folded, not deleted: rows and bodies persist and are re-OPENable, so log *history* is preserved while the render collapses to coordinates. {§grinder-layer1-rollback}
...
**Rationale.** The model owns curation (§tokenomics); the grinder is the exceptional backstop, and its story must stay simple enough to hold in one head: *overflow → fold everything → strike → still over → 413*. It only *folds* — reversibly — nothing is deleted, so the model OPENs back what it needs and log history stays intact. (Rummy's spec described clearing log *bodies*; its code folded instead — body-clearing is destructive. The code was the lesson.) An earlier staged design (fold the prior turn, then the current turn, turn-1 special-cased) died of its own complexity: the stages diverged across the owner's model, the agent's model, the code, and this spec, and the gap shipped a needless 413 death on a wake turn (#332, benchmarks/run14). One rule replaced it.
