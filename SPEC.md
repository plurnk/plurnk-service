...
**Edit landing is a compare-and-swap.** {§membership-edit-write-cas} An EDIT proposal carries the `synced_sig` its baseline snapshot was taken at; at accept, the write-back re-stats disk and lands the proposed content only if that signature still matches. If disk moved out-of-band between propose and accept — a sibling run, the user's editor, a build step — the write is refused with a `write_conflict` and **nothing is written**: the ambient change is never clobbered, and the engine never silently re-diffs against a state the model never saw. The model re-reads (the next reconcile narrates disk truth via §membership-emi-divergence-signal) and re-proposes against the fresh snapshot. This is the write-side twin of the read-side change-gate (§membership-change-gated-sync): one `synced_sig`, gating both the re-read and the write. On a clean landing the signature is restamped to the written file so the reconcile recognizes the model's own write, not an external divergence.
