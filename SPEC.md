...
- **Constraint overlay — `add`.** {§14.3-constraint-add} `add` globs admit members git misses, resolved by a targeted, client-dictated disk scan (`node:fs` glob) and enumerated so the manifest stays exhaustive. git-absent, `add` is the *sole* membership source — the whole mechanism without git.
- **Constraint overlay — `read-only`.** {§14.3-constraint-readonly} `read-only` globs admit a member for reading but refuse `File.edit` on a match — a file visible without write.
