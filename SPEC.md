...
**Timeout and poll — `<T,P>` on the `<L>` slot (grammar 0.74.20).** EXEC repurposes the line-marker slot as `<timeout, poll>` in **seconds** (consistent with the `seconds=` stream-age render). `T` (mark[0]) caps the spawn's lifetime: at T the service aborts it (a bounded reap — polite signal then SIGKILL after `PLURNK_EXEC_KILL_GRACE_MS`) and stamps the stream **504**, distinct from a deliberate kill (499) or a clean exit (200). Absent / ≤0 → unbounded, the prior background-stream behavior. `P` (mark[1]) is the hibernation **poll cadence** (§run-lifecycle): while the loop is *parked* at `SEND[202]`, a polled stream wakes it every P seconds to inspect progress — it does nothing while the loop is **active**, since an active loop already gets the ambient folded stream deltas (§exec-stream). {§exec-timeout}
