...
if (ctor) { out.splice(ctorEndIdx, 0, construction); const lastFieldIdx = out.findIndex((l) => l === L(ctor.n)) - 1; let fieldInsert = lastFieldIdx; while (fieldInsert > 0 && out[fieldInsert].trim() === "") fieldInsert--; out.splice(fieldInsert + 1, 0, `    readonly ${spec.instanceField}: ${spec.newClass};`); }
