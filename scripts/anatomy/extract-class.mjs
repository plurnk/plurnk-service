...
let body = moved.replace(new RegExp(`${spec.origin}\\.(${usedStatics.map((sf) => sf.name).join("|") || "__none__"})(?![a-zA-Z0-9_(])`, "g"), "this.#$1").replace(new RegExp(`this\\.(${calledPublic.join("|") || "__none__"})\\(`, "g"), "this.#$1(").replace(new RegExp(`${spec.origin}\\.#(${calledStatic.join("|") || "__none__"})\\(`, "g"), "this.#$1(");
...
const originRest = src.filter((l, i) => !spec.members.some((m) => i + 1 >= byName.get(m).start && i + 1 <= byName.get(m).end) && !initFields.some((fld) => fld.n === i + 1)).join("\n");
...
const injected = [...injectedStatics, ...sharedInitFields, ...usedFields.map((f) => ({ name: f.slice(1), decl: fields.get(f).decl.replace(/^    (?:readonly )?/, "    readonly "), param: `        ${f.slice(1)}${fields.get(f).optional ? "?" : ""}: ${fields.get(f).type};` })), ...callbacks.map((c) => ({ name: c.name, decl: `    readonly #${c.name}: ${c.type};`, param: `        ${c.name}: ${c.type};` }))];
