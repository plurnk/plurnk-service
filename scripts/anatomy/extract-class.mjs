...
const ctor = byName.get("constructor"); if (!ctor) throw new Error("no constructor");
for (const h of heads) if (h.start < ctor.n && h.end >= ctor.n) throw new Error("span overlaps constructor");
...
for (const f of usedFields) { const outside = src.filter((l, i) => new RegExp(`this\\.${f} = `).test(l) && (i + 1 < ctor.start || i + 1 > ctor.end)); if (outside.length) throw new Error(`${f} is reassigned outside the constructor: ${outside[0].trim()}`); }
// callback types from the private methods' signatures
const signature = (name) => { const h = byName.get(name); let out = "", n = h.n; const headRe = h.priv ? /^    (static )?(async )?#/ : /^    (static )?(async )?/; const balanced = (text) => { let p = 0, b = 0, a = 0; for (const ch of text.replace(/=>/g, "")) { if (ch === "(") p++; if (ch === ")") p--; if (ch === "{") b++; if (ch === "}") b--; if (ch === "<") a++; if (ch === ">") a--; } return p === 0 && b === 0 && a === 0; }; for (;;) { const l = L(n); out += (n === h.n ? l.replace(headRe, "") : "\n" + l); if (/\{\s*$/.test(l) && !/[:<|&,=(]\s*\{\s*$/.test(l) && balanced(out.replace(/\{\s*$/, ""))) break; if (++n > h.end) throw new Error(`signature of ${name} not closed`); } const m = out.match(/^([a-zA-Z_][a-zA-Z0-9_]*)(\([\s\S]*\))(?::\s*([\s\S]+?))?\s*\{\s*$/); if (!m) throw new Error(`cannot parse signature of ${name}: ${out.slice(0, 80)}`); if (!m[3]) throw new Error(`${name} needs an explicit return type to be injected`); return { params: m[2], ret: m[3].trim().replace(/\n\s*/g, " ") }; };
...
const paramType = (params) => { const inner = params.trim().slice(1, -1); const parts = splitTop(inner).map((p, idx) => { const q = p.trim(); if (/^[{[]/.test(q)) { let depth = 0; for (let i = 0; i < q.length; i++) { if ("{[(<".includes(q[i])) depth++; if ("}])>".includes(q[i])) depth--; if (depth === 0) { const rest = q.slice(i + 1).trim(); if (!rest.startsWith(":")) throw new Error(`destructured parameter without a type: ${q.slice(0, 40)}`); return `arg${idx}: ${rest.slice(1).trim().replace(/ = [\s\S]*$/, "")}`; } } throw new Error(`unbalanced parameter ${q.slice(0, 40)}`); } let depth = 0; for (let i = 0; i < p.length; i++) { const ch = p[i]; if ("<([{".includes(ch)) depth++; if (">)]}".includes(ch)) depth--; if (depth === 0 && p.startsWith(" = ", i)) { const head = p.slice(0, i).trim(); const dflt = p.slice(i + 3).trim(); const colon = head.indexOf(":"); if (colon >= 0) return `${head.slice(0, colon)}?${head.slice(colon)}`; const lit = /^"/.test(dflt) ? "string" : /^(true|false)$/.test(dflt) ? "boolean" : /^-?\d/.test(dflt) ? "number" : null; if (!lit) throw new Error(`untyped default parameter ${head} = ${dflt}`); return `${head}?: ${lit}`; } } return p.trim(); }); return `(${parts.join(", ")})`; };
...
// new class text
let body = moved.replace(new RegExp(`this\\.(${calledPublic.join("|") || "__none__"})\\(`, "g"), "this.#$1(").replace(new RegExp(`${spec.origin}\\.#(${calledStatic.join("|") || "__none__"})\\(`, "g"), "this.#$1(");
...
const injected = [...usedFields.map((f) => ({ name: f.slice(1), decl: fields.get(f).decl.replace(/^    (?:readonly )?/, "    readonly "), param: `        ${f.slice(1)}${fields.get(f).optional ? "?" : ""}: ${fields.get(f).type};` })), ...callbacks.map((c) => ({ name: c.name, decl: `    readonly #${c.name}: ${c.type};`, param: `        ${c.name}: ${c.type};` }))];
