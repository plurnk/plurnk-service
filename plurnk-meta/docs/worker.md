...
**Concluding with live workers.** `SEND[200]` is refused (`409`) while you hold a live worker or open stream; the system packet lists them (`## Plurnk Service Child Runs`, `## Plurnk Service Child Streams`). Either `SEND[202]<seconds>` to await them, or `KILL(worker://<name>)` the ones you no longer need, then terminate — a same-turn `KILL` + `SEND[200]` concludes cleanly in one move.
