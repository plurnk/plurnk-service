...
// duplicates packet.user.prompt, so its LOG ROW is folded by default (indexed=0):
...
type LogRow = { op: string; pathname: string; scheme: string; indexed: number; turn_id: number };
...
test("[§prompt-fold] the foisted prompt EDIT log row is folded by default; a normal op in the same turn stays open", async () => {
    await withDaemon(mock(), async (db, _daemon, addr) => {
        ...
        try {
            ...
            assert.equal(prompt!.indexed, 0, "the prompt EDIT log row is FOLDed by default — its body lives in packet.user.prompt");
            const send = rows.find((r) => r.op === "SEND" && r.turn_id === prompt!.turn_id);
            assert.ok(send !== undefined, "the model's own op shares the turn");
            assert.equal(send!.indexed, 1, "a normal op in the same turn stays OPEN — only the prompt foist is folded, not a grinder sweep");
        } finally { ws.close(); }
    });
});
