...
test("Known declares static channels manifest + defaultChannel", () => {
    assert.deepEqual(Known.manifest.channels, { body: "text/markdown", preview: "text/markdown" });
...
test("[§5.5-fragment-selects-named-channel] Known.edit with #preview fragment writes ONLY preview channel", async () => {
    ...
    try {
        const k = new Known();
        await k.edit(editStmt(urlPath("known", "x"), "default body"), makeSchemeCtx({ db, sessionId, runId }));
        const r = await k.edit(editStmt(urlPath("known", "x", "preview"), "custom preview"), makeSchemeCtx({ db, sessionId, runId }));
...
test("[§5.5-fragment-on-nonexistent-404] Known.edit with #preview on nonexistent entry returns 404", async () => {
    const { db, sessionId, runId } = await setup();
    try {
        const r = await new Known().edit(editStmt(urlPath("known", "nope", "preview"), "x"), makeSchemeCtx({ db, sessionId, runId }));
...
test("Known.read with #preview fragment returns preview channel content", async () => {
    ...
    try {
        ...
        await k.edit(editStmt(urlPath("known", "x", "preview"), "preview content"), makeSchemeCtx({ db, sessionId, runId }));

        const r = await k.read(readStmt(urlPath("known", "x", "preview")), makeSchemeCtx({ db, sessionId }));
        assert.equal(r.status, 200);
...
test("[§5.5-fragment-targeted-show-hide] Known.show/hide with #preview flips ONLY that channel", async () => {
    ...
    try {
        ...
        const h = await k.hide(hideStmt(urlPath("known", "x", "preview")), makeSchemeCtx({ db, sessionId, runId }));
        assert.equal(h.status, 200);
