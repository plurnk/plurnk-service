...
// @plurnk/plurnk-mimetypes-embeddings). Isolated in its own file with the opt-in
// flag ON, so the rest of the intg tier runs PLURNK_SEMANTIC_ENABLED off and never
// loads the 16 MB model. One test → no intra-file concurrency on the model →
// deterministic (mimetypes guarantees byte-identical output).
process.env.PLURNK_SEMANTIC_ENABLED = "1";
