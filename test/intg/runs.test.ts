...
test("runs: trunk run insert — null parent_run_id, defaults populate", async () => {
    ...
    try {
        ...
        await (db.test_runs_insert as PrepMethod).run({ session_id: sessionId });
        const row = await (db.test_runs_get_by_session as PrepMethod).get<{
            id: number; version: number; session_id: number; created_at: string;
...
test("runs: fork insert — non-null parent_run_id pointing at trunk run", async () => {
    ...
    try {
        ...
        const trunk = await (db.test_runs_insert_returning as PrepMethod).get<{ id: number }>({ session_id: sessionId });
        const fork = await (db.test_runs_insert_with_parent_returning as PrepMethod).get<{ id: number }>({ session_id: sessionId, parent_run_id: trunk?.id });
...
test("runs: session_id FK — insert against non-existent session rejected", async () => {
    ...
    try {
        await assert.rejects(
            () => (db.test_runs_insert as PrepMethod).run({ session_id: 99999 }),
...
test("runs: parent_run_id FK — insert against non-existent parent rejected", async () => {
    ...
    try {
        ...
        await assert.rejects(
            () => (db.test_runs_insert_with_parent as PrepMethod).run({ session_id: sessionId, parent_run_id: 99999 }),
            /FOREIGN KEY constraint failed/,
        );
    } finally { await db.close(); }
});
...
test("runs: parent_run_id self-reference CHECK — parent != id rejected", async () => {
    ...
    try {
        ...
        const run = await (db.test_runs_insert_returning as PrepMethod).get<{ id: number }>({ session_id: sessionId });
        await assert.rejects(
...
test("runs: ON DELETE CASCADE via session — deleting session removes all its runs", async () => {
    ...
    try {
        ...
        await (db.test_runs_insert as PrepMethod).run({ session_id: sessionId });
        await (db.test_runs_insert as PrepMethod).run({ session_id: sessionId });
        await (db.test_runs_insert as PrepMethod).run({ session_id: otherSessionId });
...
test("runs: ON DELETE CASCADE via parent_run_id — deleting a parent run removes its forks", async () => {
    ...
    try {
        ...
        const forkA = await (db.test_runs_insert_with_parent_returning as PrepMethod).get<{ id: number }>({ session_id: sessionId, parent_run_id: trunkId });
        await (db.test_runs_insert_with_parent as PrepMethod).run({ session_id: sessionId, parent_run_id: trunkId });
        await (db.test_runs_insert_with_parent as PrepMethod).run({ session_id: sessionId, parent_run_id: forkA?.id });
        const before = await (db.test_runs_count as PrepMethod).get<{ n: number }>();
...
test("runs: negative cost_pico rejected by CHECK", async () => {
    ...
    try {
        ...
        await assert.rejects(
            () => (db.test_runs_insert_cost as PrepMethod).run({ session_id: sessionId, cost_pico: -1 }),
            /CHECK constraint failed/,
        );
    } finally { await db.close(); }
});
...
test("runs: negative version rejected by CHECK", async () => {
    ...
    try {
        ...
        await assert.rejects(
            () => (db.test_runs_insert_version as PrepMethod).run({ session_id: sessionId, version: -1 }),
            /CHECK constraint failed/,
        );
    } finally { await db.close(); }
});
...
test("runs: trunk-run lookup uses the (session_id, created_at) index", async () => {
    ...
    try {
        ...
        const trunk = await (db.test_runs_insert_returning as PrepMethod).get<{ id: number }>({ session_id: sessionId });
        await (db.test_runs_insert_with_parent as PrepMethod).run({ session_id: sessionId, parent_run_id: trunk?.id });
        await (db.test_runs_insert_with_parent as PrepMethod).run({ session_id: sessionId, parent_run_id: trunk?.id });
