...
test("[§tools-capability-sheet] # Plurnk System Tools renders above Requirements; omitted when the list is empty", () => {
    const withTools = PacketWire.renderUserContent({
        prompt: "go",
        tools: ["- `<<PLAN:...:PLAN` — plan first."],
        system_requirements: "Conclude with SEND.",
    });
    ...
    const noTools = PacketWire.renderUserContent({
        prompt: "go", tools: [], system_requirements: "Conclude with SEND.",
    });
