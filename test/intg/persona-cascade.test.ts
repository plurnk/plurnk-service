...
test("[§persona-null-falls-through] persona cascade: no overrides → null (caller's default kicks in)", async () => {
...
test("[§persona-cascade-precedence] persona cascade: loops.persona is most specific — wins over everything", async () => {
    const db = await openMigrated();
    try {
...
test("[§persona-empty-suppresses] persona cascade: empty string is treated as a non-null override", async () => {
