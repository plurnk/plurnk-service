...
test("[§prompt-auto-read] a short prompt foists READ(prompt)<1,-1> — whole, the teaching form", async () => {
    await withDaemon(mock(), async (db, _daemon, addr) => {
        ...
        try {
            ...
            const autoRead = rows.find((r) => r.op === "READ" && r.origin === "plurnk" && r.pathname?.startsWith("/prompt/1/") === true); // loop-SEQ coordinates: the run's first loop is prompt/1/… regardless of db id
...
test("[§prompt-auto-read] a 12+-line prompt foists READ(prompt)<1,12> and the section lists the PATH only", async () => {
    await withDaemon(mock(), async (db, _daemon, addr) => {
        ...
        try {
            ...
            assert.match(promptSection!.content, /^\* plurnk:\/\/prompt\/1\/1$/m, "paths-only, loop-SEQ coordinates — the first loop is 1/1, never the db id (owner)");
