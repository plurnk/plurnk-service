...
`jq` runtime executor for [plurnk-service](https://github.com/plurnk/plurnk-service)'s `exec` scheme. Drives `jq` by shelling the system `jq` binary to filter/transform JSON — no third-party JSON-filter library.
...
| --------------------------------------------------------------------- | --------------------------------- | ---------------------------------- |
| `jq`<br>`{"a":1}`                                         | `jq -n '{"a":1}'`                 | Construct or validate inline JSON. |
...
| `jq (sqlite:///a3b7c921#results)`<br>`.[] \| .name`     | Consumer materializes the target. | Filter a prior runtime's output.   |
