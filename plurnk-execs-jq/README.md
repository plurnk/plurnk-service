...
`jq` runtime executor for [plurnk-service](https://github.com/plurnk/plurnk-service)'s `exec` scheme. Drives `### EXEC0 [jq]` by shelling the system `jq` binary to filter/transform JSON — no third-party JSON-filter library.
...
| `### EXEC0 [jq] (sqlite:///1/2/3/EXEC#results)`<br>`.[] \| .name`     | Consumer materializes the target. | Filter a prior runtime's output.   |
