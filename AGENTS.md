...
**Staged plan:** S0 this plan + clarifying answers (DONE) → S1 the log contract: hierarchical prefix coords (Q1) + log-as-first-class-substrate so find/read/matcher/semantic are uniform (Q3, Z4 refactor), matrix test + SPEC anchors, grammar#issue for plurnk.md → S2 the error taxonomy (Q2: op-result attribution + real messages + errors-line-points-at-row; confirm internal faults crash) → S3 recoverable budget (Q4 recovery rail, SPEC anchor + guard) → S4 re-run SPEC-jumbo as the end-to-end trust proof (model curates, searches log, reads SPEC.md, answers).
