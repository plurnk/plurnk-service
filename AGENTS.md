...
- [ ] **#382 ROOT-CAUSED — awaiting owner's addressing call before building** — TWO faults: (1) SERVICE BUG — a WORK-spawned worker's turn-1 prompt foist collides with the parent's on /prompt/1/1 (loopSeq per-run always 1, entries session-shared); worker clobbers parent's task. FIX needs owner's pick: (a) run-qualify /prompt/<runId>/<loop>/<N> vs (b) run-scope prompts under run://. (2) DESIGN (owner) — active task body folds out mid-run, model won't re-read the listed path. Both proven in run43. OLD: — User Prompts section EMPTY every packet incl. 001 (mission points at nothing; task only ever in the foldable turn-1 log row → vanishes when grinder folds). PLUS /prompt/1/1 overwritten mid-run 1409tok→205chars. REMAINING: why the section sourced empty when entry+body existed (prep replays clean — fault is packet-side sourcing/pattern, PacketBuilder ~206-220 + line 314); and whether the prompt entry should be model-writable. Old note: **#382** — per-turn packet-frame re-derivation tax (model treats its session as a pasted transcript, third-persons its own past). Packet-framing design — OWNER territory (packet-refinement doctrine); high leverage for the gemma-floor goal. Discuss before building.
