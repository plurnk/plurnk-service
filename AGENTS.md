...
- **Q2 — a model-op failure is dressed as an internal engine error with an empty message.** The mint (`Engine.ts:1118`) attributes action_failures `source:"engine"`, message from the op's `rx.error/reason` — "" because `Log #resolveIds` returns bare `{status:400}`. **Fix (taxonomy): errors typed by WHO failed, which decides WHERE they go + HOW attributed.** model-op failure (op ≥400) → model-facing item attributed to the MODEL's op (not "engine"), carrying the op's OWN message (ops MUST explain their failures); rail signal (budget/strike/steer) → model-facing, `engine:rail`; engine-internal fault → telemetry ONLY; provider → telemetry. Errors stay information — attributed truthfully.
- **Q3 — log:// is advertised FIND-able but has no find handler (501).** plurnk.md:148 shows `FIND(log:///**/error):#…#`; `Log.ts` registers read/open/fold/kill, no find — a silent exception to the universal FIND/READ/OPEN/FOLD paradigm. **Fix (universality): Log implements `find`** — content-match its rows, return matching coordinates as (file, span) items (#286) so `FIND(log)→READ(coord)` composes like every scheme. Dialect scope — OWNER Q.
- **Q4 — the hard-413 kills without recovery.** `enforceBudget` folds only the newest turn (§grinder-layer1-rollback: model curates history); a hard overflow → immediate 413 death, no strike, no curate turn — yet the overflow lives in foldable HISTORY the model owns. **Fix (recovery rail): a hard-413 is a RECOVERABLE strike** — one turn, minimal packet, "over budget; KILL/FOLD history" steer, + a strike. Curate → recover; repeated → strike out. Death only after the model was told and declined. **The 95% question (owner): a margin MASKS the pointer** (against "errors are information") — recommend NO margin, make the 413 recoverable instead; decode is separately reserved so 100% of promptBudget needs no response headroom. — OWNER CONFIRM.

**Staged plan:** S0 this plan + clarifying answers → S1 the log contract (Q1 hierarchical coords + Q3 Log.find, matrix test + SPEC anchors, grammar#issue) → S2 the error taxonomy (Q2, ops populate messages, internal→telemetry, SPEC anchor) → S3 recoverable budget (Q4 recovery rail, SPEC anchor + guard) → S4 re-run SPEC-jumbo as the end-to-end trust proof (model curates, searches log, reads SPEC.md, answers).
