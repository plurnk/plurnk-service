...
- **Q3 — log:// is advertised FIND-able but has no find handler; the real defect is that log is BESPOKE (owner-reframed 2026-07-08).** plurnk.md:148/159 show `FIND(log:///**):#…#` and `READ(log:///1/2/3):~high-signal`; `Log.ts` is coordinate-only, hand-rolled, doesn't even touch the shared `Matcher`. The bad decision was making log operate OUTSIDE the general paradigm. **Fix (universality, not "add find"): log stops being a special case** — its rows route through the SAME find/read/matcher/semantic substrate as every entry (Z4), so ALL patterns work BY CONSTRUCTION. Log rows get embedded like any entry (the one real implementation cost — the consequence of the bad decision, not a reason to keep it), so `~query` works for free. (log stays READ-oriented — no EDIT/COPY/MOVE on history — a legitimate op subset, not a matcher exception.)
