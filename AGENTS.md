...
A broad survey (open issues + this doc's User Notes + the budget chapter) clustered by shared context. **v0.68.0 is the published stable checkpoint** (bench + endpoint unblocked) — the budget/context/reasoning math is DEFINED AND SHIPPED (window partition {§tokenomics-window-partition} + calibration + maxTokens on every call), delegation inherits authority {§run-delegation-inherits-flags}, thinking posture settled per backend (gemma 4096 box-clamped / fireworks 0 — the measured optimum; parallax documented in SPEC env table + template), budget test pins are floor-relative (self-calibrating). Issues #307/#311/#314 closed at the pub; #308 awaits a bench grok re-run on 0.63.0; #313 awaits providers' generate({strikes}) surface (the endpoint's pro/flash router depends on it); #312 (tokenizer_id keying) is post-freeze, more relevant now that the endpoint interleaves models per turn. **Cluster I is the active agenda** — teaching/steer iteration against the frozen engine. **First digest grind (v0.64.0 sweep, 2026-07-03) — the tripartite findings:** the sweep's reds reduce to TWO mechanism classes, each filed on all three lenses. (1) **Pressure-correlated valid-prefix runaways** (grind/delegate 422s: 12–21k code-point legal prefixes, never terminating, discarded whole): train = emission-brevity-under-pressure teaching (grammar#50), rail = prefix salvage with the strike kept (#317, feasible via providers 0.31 err.attempt), ponder = strict-reject-vs-salvage recorded on #317. (2) **Pressure-strike vs convergence** (jumbo: correct FIND→range-READ scanning, novel fingerprints, struck out at 3 by per-fire budget strikes + 413 double-jeopardy): train = curate-while-scanning habit (grammar#50), rail options + the evidence against §grinder-compaction-strikes' no-exemption ruling = #318, OWNER RULING PENDING — the citing tests move in lockstep with the ruling. paris→tokyo stays pure test calibration (grant turns or admit the bound). Specimens kept: grind db-764d651a, delegate db-a76a7e88, jumbo db-d8e45afa.
