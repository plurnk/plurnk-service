// Build-time drift guard (§61). The framework bakes its own Notice copy
// (Notice.ts) so it carries no RUNTIME grammar dependency — but the
...
import type { Notice as Baked, ContentOffset as BakedCO, LogCoordinate as BakedLC } from "./Notice.ts";
import type { Notice as Contract, ContentOffset as ContractCO, LogCoordinate as ContractLC } from "@plurnk/plurnk-grammar";
