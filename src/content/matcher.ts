...
export default class Matcher {
    ...
    // one match per line — the same N:\t convention READ already emits, so the model
    // reads a query result exactly like a slice. Value is bare for a single-line
    // string, JSON-encoded otherwise (objects, arrays, multi-line strings) so the
    // one-match-per-line invariant holds and `<L>` can still page the result set.
