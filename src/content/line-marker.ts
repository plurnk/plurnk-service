...
export default class LineMarkerOps {
    ...
    static sliceLines(content: string, marker: LineMarker): SliceResult {
        const { lines } = LineMarkerOps.#splitLines(content);
        const norm = LineMarkerOps.#normalize(marker, lines.length);
    ...
    static sliceJsonItems(content: string, marker: LineMarker): JsonSliceResult {
        ...
        const items = LineMarkerOps.#jsonValueToItems(parsed);
    ...
    static applyJsonItemEdit(content: string, marker: LineMarker, body: string): EditResult {
        ...
        const bodyResult = LineMarkerOps.#itemsFromBody(body);
        ...
        if (Array.isArray(parsed)) return LineMarkerOps.#applyJsonArrayEdit(parsed, marker, items);
        if (parsed !== null && typeof parsed === "object") return LineMarkerOps.#applyJsonObjectEdit(parsed as Record<string, unknown>, marker, items);
        return LineMarkerOps.#applyJsonScalarEdit(parsed, marker, items);
    }
    ...
    static applyLineMarkerEdit(content: string, marker: LineMarker, body: string): EditResult {
        const { lines, trailingNewline } = LineMarkerOps.#splitLines(content);
        const norm = LineMarkerOps.#normalize(marker, lines.length);
        if ("error" in norm) return { status: 416, error: norm.error };

        const bodyLines = LineMarkerOps.#splitLines(body).lines;
        let newLines: string[];
