...
// — the fork-bomb / destabilization brake. `PLURNK_SERVICE_SESSION_RUNS_MAX_ACTIVE` is the
...
export default class RunCap {
    static async deny(db: Db, sessionId: number): Promise<{ status: number; error: string } | null> {
        const raw = process.env.PLURNK_SERVICE_SESSION_RUNS_MAX_ACTIVE;
        ...
        if ((row?.n ?? 0) >= cap) return { status: 508, error: `session active-run ceiling reached (PLURNK_SERVICE_SESSION_RUNS_MAX_ACTIVE=${cap})` };
        return null;
    }
}
