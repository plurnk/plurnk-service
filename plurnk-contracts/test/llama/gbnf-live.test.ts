 ...
 *   boundary, then `sep`, mandatory PLAN fence, operation fences, and disposition SEND.
...
const grammar = serializeGbnf(buildModel(), `root-${railProfile}`);
...
test(`llama.cpp accepts the generated plurnk.${railProfile}.gbnf (size/format check)`, async () => {
...
test("llama projection separates reasoning and the model completes a PLAN turn", async () => {
    ...
    // Feed the projected content directly; parsing begins at the PLAN fence.
