...
test("ex 4 — bare READ, empty body", () => {
    assert.equal(statementsOf("<|READ(https://www.britannica.com/biography/Donald-Rumsfeld)|>").length, 1);
});
...
test("unescaped unmatched target parentheses require canonical spelling", () => {
    ...
    for (const encoded of ["a%28b", "a%29b"]) {
        const result = PlurnkParser.parseStatements(`<|READ(https://example.test/${encoded})|>`);
        assert.equal(result.items.filter((item) => item.kind === "error").length, 0);
...
test("target escapes preserve literal and percent-encoded URI spelling", () => {
    const source = String.raw`<|READ(https://example.test/x?literal=\)&encoded=%29#preview\()|>`;
    const result = PlurnkParser.parseStatements(source);
...
test("COPY/MOVE reject close-tag residue after a destination scope", () => {
    assert.throws(
...
test("ex 12 — EDIT with empty body (clear)", () => {
    assert.equal(statementsOf("<|EDIT(known://countries/france/capital)|>").length, 1);
});
...
test("ex 21 — FIND with range pagination, empty body", () => {
    assert.equal(statementsOf("<|FIND(known://**)<1-20>|>").length, 1);
});
...
test("WORK and FORK preserve an absent prompt for their runtime owner to reject", () => {
    for (const op of ["WORK", "FORK"]) {
        const empty = statementsOf(`<|${op}(worker://child)|>`);
        assert.equal(empty.length, 1);
        if (empty[0]?.kind === "statement") assert.equal(empty[0].statement.body, "");
...
test("malformed path (unclosed paren) produces a tail boundary, not silent swallowing", () => {
    ...
    assert.deepEqual(result.unparsedTail?.from, { line: 1, column: input.indexOf("<|EDIT(broken") });
...
test("value-add: expected-token list is deduped (no `<|OPsuffix` repeated 10x)", () => {
    const msg = errMsgs("<|PLAN>t<PLAN|> <|READ(x)|>")[0]!.message;
...
test("value-add: a turn that derails mid-op does NOT get a misleading SEND imperative", () => {
    ...
    const result = PlurnkParser.parse("<|PLAN>t<PLAN|> <|READ(<|SEND[200]>d<SEND|>");
...
test("value-add: near-miss op swallowed as prose surfaces a warning, not an error", () => {
    const emission = "<|PLAN>t<PLAN|> <|CLOSE(log://x)>x<CLOSE|> <|SEND[200]>d<SEND|>";
...
test("parser points use LF and CRLF line boundaries while lone CR remains a code point", () => {
    ...
    const cr = PlurnkParser.parseStatements("<|EDIT(a)>x<EDIT|>\r<|READ(a)|>");
    ...
    assert.deepEqual(positions(cr), [{ line: 1, column: 0 }, { line: 1, column: 19 }]);
});
...
test("value-add: a bare `<|DELETE` mention without a close is NOT flagged (op-shape gate)", () => {
    const r = PlurnkParser.parse("<|PLAN>weighing <|DELETE vs KILL<PLAN|> <|KILL(x)|> <|SEND[200]>d<SEND|>");
    assert.equal(r.items.filter((i) => i.kind === "error").length, 0);
});
...
test("each malformed statement surfaces only its first hard diagnostic", () => {
    ...
    assert.match(matcher[0]!.message, /matcher belongs after `>` and before the matching close tag/);
...
test("valid path (https URL) accepted", () => {
    const result = PlurnkParser.parseStatements("<|READ(https://example.com/page)|>");
    assert.equal(result.items.filter((i) => i.kind === "statement").length, 1);
    assert.equal(result.items.filter((i) => i.kind === "error").length, 0);
});

test("valid path (custom scheme) accepted", () => {
    const result = PlurnkParser.parseStatements("<|READ(known://entries/foo)|>");
    assert.equal(result.items.filter((i) => i.kind === "statement").length, 1);
});

test("bare relative target remains local", () => {
    const result = PlurnkParser.parseStatements("<|READ(./README.md)|>");
    assert.equal(result.items.filter((i) => i.kind === "statement").length, 1);
});

test("valid path (glob pattern under file://) accepted", () => {
    const result = PlurnkParser.parseStatements("<|FIND(config/**/*.xml)|>");
    assert.equal(result.items.filter((i) => i.kind === "statement").length, 1);
});
...
test("invented closer: Unicode offset conversion cannot scan a pre-op tag as body text", () => {
    const cut = `<|PLAN|> ${"😀".repeat(10)}<BEFORE|><|EDIT(x)>body <INSIDE|>`;
    const result = PlurnkParser.parse(cut);
    assert.ok(result.unparsedTail);
...
test("plan-body advisory: op-shaped text in a parsed PLAN body warns", () => {
    // `<PLAN|>` is omitted at the intended boundary, so the body reaches a later PLAN closer.
...
test("misplaced-target advisory: path in [signal] with null target redirects to (…)", () => {
    for (const [op, suggested] of [
        ["<|EDIT[evaluator/functions.go]<38,61>>package x<EDIT|>", "<|EDIT(evaluator/functions.go)>…<EDIT|>"],
        ["<|COPY[src/a.go]>x<COPY|>", "<|COPY(src/a.go)>…<COPY|>"],
        ["<|MOVE[old/notes.md]>x<MOVE|>", "<|MOVE(old/notes.md)>…<MOVE|>"],
        ["<|EDIT[functions.go]>new body<EDIT|>", "<|EDIT(functions.go)>…<EDIT|>"],
    ] as const) {
        ...
        assert.ok(w.error.message.includes(`Try \`${suggested}\``), `should suggest ${suggested}, got: ${w.error.message}`);
    }
});
...
test("ParsedPath: https URL decomposed fully", () => {
    const result = PlurnkParser.parseStatements("<|READ(https://user:pass@sub.example.com:8080/foo/bar?q=1&q=2#frag)|>");
    const item = result.items[0];
    if (item.kind !== "statement") return;
    const p = item.statement.target;
    assert.ok(p);
...
test("ParsedPath: worker:/// — empty authority, leading-slash pathname", () => {
    const result = PlurnkParser.parseStatements("<|READ(worker:///entries/foo/bar)|>");
    const item = result.items[0];
    if (item.kind !== "statement") return;
    const p = item.statement.target;
    assert.ok(p);
...
test("ParsedPath: log:/// — empty authority, full address in pathname", () => {
    const result = PlurnkParser.parseStatements("<|READ(log:///1/turn/2/action/3/get)|>");
    const item = result.items[0];
    if (item.kind !== "statement") return;
    const p = item.statement.target;
    assert.ok(p);
...
test("ParsedPath: file:///… scheme parses, empty hostname", () => {
    const result = PlurnkParser.parseStatements("<|READ(file:///tmp/foo.txt)|>");
    const item = result.items[0];
    if (item.kind !== "statement") return;
    const p = item.statement.target;
    assert.ok(p);
...
test("ParsedPath: absent query and fragment are null", () => {
    const result = PlurnkParser.parseStatements("<|READ(https://example.com/foo)|>");
    const item = result.items[0];
    if (item.kind !== "statement") return;
    const p = item.statement.target;
    assert.ok(p);
...
test("ParsedPath cleavage: HTTPS retains authority decomposition", () => {
    const result = PlurnkParser.parseStatements("<|READ(https://user:pw@example.com:8080/api/data?q=1#frag)|>");
    const item = result.items[0];
...
test("ParsedPath: sh:/// — empty authority, leading-slash pathname", () => {
    const result = PlurnkParser.parseStatements("<|READ(sh:///run-tests)|>");
    const item = result.items[0];
    if (item.kind !== "statement") { assert.fail("expected statement"); return; }
    const p = item.statement.target;
    if (p?.kind !== "url") { assert.fail("expected url"); return; }
...
test("ParsedPath: wiki:/// — single-segment pathname under empty authority", () => {
    const result = PlurnkParser.parseStatements("<|READ(wiki:///Paris)|>");
    const item = result.items[0];
    if (item.kind !== "statement") { assert.fail("expected statement"); return; }
    const p = item.statement.target;
    if (p?.kind !== "url") { assert.fail("expected url"); return; }
...
test("ParsedPath: empty-authority scheme preserves query and fragment", () => {
    const result = PlurnkParser.parseStatements("<|READ(wiki:///Paris?lang=fr#History)|>");
    const item = result.items[0];
    if (item.kind !== "statement") { assert.fail("expected statement"); return; }
    const p = item.statement.target;
    if (p?.kind !== "url") { assert.fail("expected url"); return; }
...
test("ParsedPath: worker:/// nested path stays whole", () => {
    const result = PlurnkParser.parseStatements("<|READ(worker:///philosophy/existentialism/meaning)|>");
    const item = result.items[0];
    if (item.kind !== "statement") { assert.fail("expected statement"); return; }
    const p = item.statement.target;
    if (p?.kind !== "url") { assert.fail("expected url"); return; }
...
test("target: a regex-shaped hash spelling remains a local path", () => {
    const result = PlurnkParser.parseStatements("<|FIND(#draft.*#i)|>");
    const item = result.items[0];
    if (item.kind !== "statement" || item.statement.op !== "FIND") return;
...
test("OPEN and FOLD reject positional scope", () => {
    for (const op of ["OPEN", "FOLD"] as const) {
        const result = PlurnkParser.parseStatements(`<|${op}[memory](log:///**)<1,2>|>`);
        assert.equal(
            result.items.some((item) => item.kind === "statement"),
            false,
...
test("error message: <| inside target says 'in target'", () => {
    ...
    assert.match(e.message, /unrecognized character '<\|' in target slot/);
});
...
test("error message: stray char in slot region names what's allowed", () => {
    ...
    assert.match(e.message, /\[signal\].*\(target\).*<scope>.*>` for a body.*\|>` to self-close/);
});
...
test("error message: the redirect is EXEC-scoped — SEND/KILL (SIGNAL_INT) do not get it", () => {
    ...
    for (const src of ["<|SEND[1,300]>x<SEND|>", "<|KILL[1,300](p)|>"]) {
        const e = firstError(src);
...
test("error message: matcher-shaped char in the statement header redirects to the body", () => {
    for (const src of [
        "<|FIND(data.json)$.role|>",
        "<|FIND(notes.md)~budget|>",
        "<|FIND(main.ts)@<handler<FIND|>",
    ]) {
        const e = firstError(src);
        assert.match(e.message, /a matcher belongs after `>` and before the matching close tag/, src);
    }
});

test("error message: a non-matcher header failure keeps the generic slot list", () => {
    // `/` (xpath) is excluded from the redirect - it collides with a forgotten `(path)` wrap - so
    // it, and any other stray char, falls through to the generic header-slot list.
    for (const src of ["<|FIND(a.go)//sel|>", "<|FIND(a.go)Xbody|>"]) {
        const e = firstError(src);
        assert.match(e.message, /\[signal\].*\(target\).*<scope>.*>` for a body.*\|>` to self-close/, src);
        assert.doesNotMatch(e.message, /matcher belongs after/, src);
    }
});
...
test("AST: discriminated union narrows correctly per op", () => {
    const result = PlurnkParser.parseStatements("<|EDIT(p)>body<EDIT|><|READ(q)|>");
    const statements = result.items.filter((i) => i.kind === "statement");
...
test("error message: slot region errors enumerate the four slots", () => {
    const e = firstError("<|EDIT(p)X>body<EDIT|>");
    assert.match(e.message, /\[signal\].*\(target\).*<scope>.*body.*self-close/);
});
...
test("parseLog: a multi-turn log of <|TURN>-wrapped turns parses clean and flattens in order", () => {
    const log =
        turn("<|PLAN>find it<PLAN|> <|READ(worker:///x)|> <|SEND[102]>reading<SEND|>") + "\n" +
...
test("parseLog: TEXT inside a wrapped turn is preserved without language semantics", () => {
    const result = PlurnkParser.parseLog(
        turn("thinking <|PLAN>p<PLAN|> consider <|READ(worker:///x)|> <|SEND[200]>done<SEND|>"),
    );
    assert.equal(result.items.filter((item) => item.kind === "error").length, 0);
    assert.deepEqual(
...
test("parseLog: a wrapped turn missing its inner PLAN is invalid (sandwich is law)", () => {
    assert.ok(logInvalid(turn("<|READ(worker:///x)|> <|SEND[200]>done<SEND|>")), "no PLAN inside the TURN");
});

test("parseLog: a wrapped turn missing its terminal SEND is invalid", () => {
    assert.ok(logInvalid(turn("<|PLAN>p<PLAN|> <|READ(worker:///x)|>")), "no terminal SEND inside the TURN");
});
...
test("parseLog: TURN suffix discipline — only the matching <TURNsuffix|> closes", () => {
...
// -------------------------------------------------------------------------
// {§canonical-statement} Self-closing and bodyful statements have explicit boundaries.
// -------------------------------------------------------------------------
...
test("self-closing bodyless operations compose across newlines", () => {
    const r = PlurnkParser.parseStatements("<|READ(known://x/a)|>\n<|READ(known://x/b)|>");
    assert.equal(r.items.filter((i) => i.kind === "statement").length, 2);
    assert.equal(r.items.filter((i) => i.kind === "error").length, 0);
});

test("self-closing operations end at EOF and before the next operation", () => {
    assert.ok(ssClean("<|READ(t)|>"));                     // EOF boundary
    assert.equal(ssCount("<|READ(t)|>"), 1);
    assert.equal(ssCount("<|READ(a)|><|FIND(b)|>"), 2); // glued, explicit next opener
