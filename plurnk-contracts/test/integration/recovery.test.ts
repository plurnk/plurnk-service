...
const turn = (...blocks: string[]) => [frame("PLAN", "[]"), ...blocks, frame("NEXT", "next")].join("\n");
...
test("a scope inside a target is applied with one factual warning per selection", () => {
    ...
    const errs = errors(r);
    assert.deepEqual(errs.map((e) => [e.line, e.severity]), [[4, "warning"], [4, "warning"], [5, "warning"]]);
    ...
    assert.deepEqual(ops.map(({ op }) => op), ["PLAN", "COPY", "READ", "NEXT"]);
    const copy = ops[1];
    ...
    assert.equal(r.items.findIndex((i) => i.kind === "error"), 2);
...
test("conflicting scopes on one resource selection are rejected without affecting the next block", () => {
    for (const header of ["READ (a.ts<1,2>) <3,4>", "COPY (a.ts<1>) <2> (b.ts)", "sh (script.js<5>) <10>"]) {
        ...
        assert.deepEqual(statements(r).map(({ op }) => op), ["PLAN", "NEXT"], header);
    }
});

test("a malformed block never downgrades a conclusion", () => {
    const r = PlurnkParser.parse([frame("PLAN", "[]"), frame("READ (b.ts) <1,-1>", null), frame("READ [+diff] (a.ts) <1,-1>", null), frame("DONE", "done")].join("\n"));
    assert.equal(errors(r).length, 1);
    assert.deepEqual(statements(r).map(({ op }) => op), ["PLAN", "READ", "DONE"]);
    const send = statements(r).find((s) => s.op === "DONE");
    assert.equal(send?.op, "DONE");
    assert.equal(send?.position.line, 6);
});
...
test("bracket modifiers have one bounded diagnostic without guessing intent", () => {
    for (const header of ["READ [+diff] (a.ts) <1,-1>", "SEND [102]", "KILL [memory] (log://**)", "EXEC [sh]"]) {
        ...
        assert.equal(errors(r)[0].line, 4, header);
    ...
    assert.deepEqual(statements(r).map(({ op }) => op), ["PLAN", "EXEC", "NEXT"]);
});
...
test("a second path on a one-path operation names the slot contract", () => {
    ...
    assert.deepEqual(statements(r).map(({ op }) => op), ["PLAN", "NEXT"]);
});
...
test("duplicate dispositions are structural failures, never a false unclosed tail", () => {
    for (const labels of [["DONE", "NEXT"], ["NEXT", "DONE"]]) {
        const r = PlurnkParser.parse([frame("PLAN", "[]"), ...labels.map((label) => frame(label, "done"))].join("\n"));
        assert.equal(r.unparsedTail, undefined);
        assert.equal(errors(r).length, 1);
        ...
        assert.equal(errors(r)[0].line, 7);
        assert.deepEqual(statements(r).map(({ op }) => op), ["PLAN", labels[0]]);
    }
});
