...
test("GBNF: PLAN-anchored root — PLAN mandatory & first, SEND-closed", () => {
    ...
    assert.equal(derivesTurn("<|READ(worker:///x)|>\n<|SEND[102]>done<SEND|>"), false);
...
test("PlurnkParser.parse requires PLAN and terminal SEND while preserving admitted TEXT", () => {
    ...
    assert.equal(invalid("<|READ(worker:///x)|>"), true);                  // op, no PLAN, no SEND
    // Operations plus SEND without a PLAN anchor remain invalid.
    assert.equal(invalid("<|READ(worker:///x)|>\n<|SEND[200]>done<SEND|>"), true);
    // TEXT is admitted before PLAN, between operations, and after the terminal SEND.
    assert.equal(valid("thinking out loud <|PLAN>intent<PLAN|> now I read <|READ(worker:///x)|> <|SEND[200]>done<SEND|> and done"), true);
...
test("PlurnkParser.parse: a mid-turn termination is ILLEGAL — a disposition-coded SEND is the terminal", () => {
    ...
    assert.equal(invalid("<|PLAN>p<PLAN|>\n<|SEND[200]>done<SEND|>\n<|KILL(proc://x)|>"), true);    // op after the terminal
...
test("GBNF: a header-bearing http target derives so constrained models can emit auth", () => {
    ...
    assert.equal(derivesTurn("<|PLAN>p<PLAN|>\n<|READ(https://api.dev/me{Authorization: Bearer x})|>\n<|SEND[102]>fetching<SEND|>"), true);
...
test("GBNF: 14 mid-steps derive, 15 do not, and exhaustion requires a terminal", () => {
    ...
    const read = "<|READ(worker:///x)|>";        // retrieval step
    ...
    const flail = [read, read, "<|FIND(src/**)|>", midSend, read, read, "<|FIND(src/**)|>", midSend, ...Array(10).fill(midSend)];
...
test("GBNF: empty matcher spellings are equivalent while close-like prefixes remain body text", () => {
    for (const op of ["FIND", "OPEN", "FOLD"]) {
        ...
        assert.equal(derivesTurn(turn(`<|${op}(a)>/^:needle/<${op}|>`)), true, `${op} can match a literal leading colon through regex`);
    }
    assert.equal(
        derivesTurn("<|PLAN>p<PLAN|>\n<|READ(https://example.com)<17,50>>:<READ|>\n<|SEND[102]>c<SEND|>"),
        true,
        "a colon is ordinary matcher content in the new enclosure",
    );
...
test("GBNF: COPY/MOVE destination scopes meet the close tag without residue", () => {
    for (const op of ["COPY", "MOVE"]) {
        ...
        assert.equal(
            derives("statement", `<|${op}(worker:///src.md)>worker:///slice.md<0>:<${op}|>`),
            false,
            `${op} excludes scope-following close-tag residue`,
        );
        assert.equal(
...
test("GBNF: dash-separated <scope> is parse-side tolerance only — not derivable", () => {
    assert.equal(derives("statement", "<|READ(a.md)<1-5>|>"), false);
});
...
test("GBNF: targets use the canonical percent-encoded spelling for parentheses", () => {
    ...
    assert.equal(derives("statement", "<|FIND(worker:///**)|>"), true);
    assert.equal(derives("statement", "<|READ(https://en.wikipedia.org/wiki/Igor_Smirnov_(politician))|>"), false);
    assert.equal(derives("statement", "<|READ(https://en.wikipedia.org/wiki/Igor_Smirnov_%28politician%29)|>"), true);
    // Unescaped delimiters remain outside the canonical generation subset.
    assert.equal(derives("statement", "<|FIND(a)b)|>"), false);
    assert.equal(derives("statement", "<|FIND(a(b)|>"), false);
...
test("GBNF: target delimiters and backslashes use the canonical lexical escape", () => {
    assert.equal(
        derives("statement", String.raw`<|READ(https://example.test/x?q=\)&encoded=%29#preview\()|>`),
        true,
    );
    assert.equal(derives("statement", String.raw`<|FIND(docs/\\*.md)|>`), true);
    assert.equal(derives("statement", String.raw`<|FIND(docs/\*.md)|>`), false);
    assert.equal(derives("statement", String.raw`<|FIND(docs/trailing\)|>`), false);
});
...
test("GBNF: permissive decimal marker shapes derive for runtime interpretation", () => {
    ...
    assert.equal(derives("statement", "<|READ(a.md)<2.>|>"), false); // bare trailing dot is not a decimal
});
...
test("GBNF: inter-op separator is WS{0,7} — none, mixed, up to 7; 8+ rejected", () => {
    ...
    assert.equal(derivesTurn(lead + "<|READ(worker:///x)|><|SEND[102]>done<SEND|>"), true);
    ...
    assert.equal(derivesTurn(lead + "<|READ(worker:///x)|>" + " ".repeat(7) + "<|SEND[102]>done<SEND|>"), true);
    // 8 whitespace chars — over the cap, rejected (no unbounded stall)
    assert.equal(derivesTurn(lead + "<|READ(worker:///x)|>" + " ".repeat(8) + "<|SEND[102]>done<SEND|>"), false);
    ...
    assert.equal(derivesTurn(lead + "<|READ(worker:///x)|> prose <|SEND[102]>done<SEND|>"), false);
});

test("GBNF: glued output round-trips through the parser", () => {
    const turn = "<|READ(worker:///x)|><|EDIT(worker:///y)>z<EDIT|><|SEND[200]>done<SEND|>";
