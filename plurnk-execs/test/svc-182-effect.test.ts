...
test("§2.3 C1: default effect is the conservative `host`, for any target", () => {
    const ex = new Bare({ runtime: "x", glyph: "•" });
    assert.equal(ex.effect(null), "host");
    assert.equal(ex.effect("/anything"), "host");
});

test("§2.3 C2: effect() is synchronous (not a Promise) — it runs on the propose hot path", () => {
    const out: Effect = new Bare({ runtime: "x", glyph: "•" }).effect(null);
    assert.equal(typeof out, "string");
    assert.equal(out, "host");
});

test("§2.3 C3: subprocess runtimes are always host (regardless of target)", () => {
