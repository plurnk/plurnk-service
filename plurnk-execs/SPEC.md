...
**The search prefetch seam resolves here (service#596).** The consumer implements `entry()` as auto-prefetch: acquire, MIME-project, and materialize the `https://` entry. A guarded browser render is a fallback only when the HTTP response has no useful model-facing projection. Resolve/reject becomes the candidate's `materialized: true|false` enrichment while the SearXNG-ranked discovery row remains intact. There is no executor pre-validation (that would fetch — forbidden), consumer post-filter, or double fetch. A URL prefetch carries no page body for the executor to supply, so `content` is **consumer-sourced** — the executor contributes the candidate `url` as `path` and learns only the materialization verdict, never the body.

A successful candidate also carries `readTarget`, its canonical HTTPS `url` with the `#body` channel selected. This is an explicit next-operation affordance, not a second address or retrieval dialect: the model must `READ` that target when it needs authoritative rendered-page evidence. Selecting `#body` avoids an undirected channel fan-out into transport headers and archival HTML. Rejected candidates omit `readTarget`; their title, URL, and snippet remain useful discovery metadata but the system does not claim that a page body is available.
