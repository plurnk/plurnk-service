...
**Live-dispatch note (#448):** the target-as-program shape above is this **legacy `resolveRuntime` helper**, not the shipped subprocess dispatch. The current exec scheme routes a *subprocess* op's `(target)` slot to the **working directory** — `EXEC[sh](./dir):cmd` runs `cmd` in `./dir` (`docs/sh.md`), passing the executor `target=null` (subprocess ignores the data-source slot). So a script file is run via the **body** (`EXEC[sh]:bash greet.sh:EXEC`), never `(greet.sh)` — which parses as "empty command, cwd=greet.sh" → 400.
