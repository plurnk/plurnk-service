...
### §2.6 The executor-is-a-scheme READ face

An executor's output is addressed at `<tag>://<coord>` and READ back: each discovered tag is both an `EXEC[tag]` run target and a `tag://` scheme. `BaseExecutor implements @plurnk/plurnk-schemes' SchemeHandler` (schemes#20 / service#240). Its **manifest is derived** from the runtime declaration — no separate scheme authoring — via `OutputScheme.manifestFromRuntime({ name, glyph, channels, defaultChannel })`; the channel set and `defaultChannel` (a bare `READ <tag>://<coord>` resolves to it — first declared channel; subprocess runtimes override to `stdout`) are the executor's only inputs. **`discover()` does not change** — it stays static install/config truth (§3); the scheme face is read off the same manifest, and activation (§3.2) is the consumer's runtime overlay.

**The default READ is the consumer's, not the executor's.** `read`/`find` are *optional* on `SchemeHandler`; `BaseExecutor` implements neither. The generic slice/match over a `<tag>://` entry is the consumer's `DefaultRead` resolver — it needs a `Mimetypes` instance and the output store, neither of which the executor is handed (the executor holds sinks, never the substrate — §5). A consumer routing `READ <tag>://coord` resolves the scheme by name, parses the coord into a schemes `ReadStatement` at the scheme layer, and applies `DefaultRead` over the stored channel output.

**Rich executors override `read`/`find` for custom semantics over their own output** (the MCP read-face, plurnk-execs#13; a future sqlite query face). The signature is **owned by schemes, not execs**: `read(statement: ReadStatement, ctx: SchemeCtx): Promise<SchemeResult>` (likewise `find`). The override receives schemes' already-parsed `ReadStatement` plus the `SchemeCtx` (the consumer's entry/output access) — there is **no execs-specific coord grammar** layered on `<tag>://<coord>`; the consumer parses the coord exactly as for any scheme, the executor's override gets the structured statement.

**The executor mints no orientation receipt.** Resolving service#240's open question — *where is the OrientIndex receipt built, the executor's wrapper or its return?* — the answer is **neither**. No execs wrapper and no executor return produces an engine-pushed shape digest. Orientation is **model-pulled**: the model probes an output's shape by querying the entry (FIND / READ with jsonpath / glob / line-range) against the neutral extent metadata every entry already carries (`mimetype` / `tokens` / `lines`), pulling only the slice it wants. An engine-pushed digest *is* the catalog-as-index the paradigm forbids ("the instant it did, it would be an index again"). schemes' `Summarize` / `OrientIndex` may exist as a model-pulled READ view the consumer's resolver offers; it is never a receipt the executor produces on the dispatch path. (Output never floods context regardless — a `<tag>://` entry is folded by default; the storage-cap risk of a chatty runtime filling sqlite is the daemon's, not the executor's.)
