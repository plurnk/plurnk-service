...
**READ over any executor's output is uniform and model-pulled — MCP included, MCP not special.** `read` / `find` are *optional* hooks on `SchemeHandler`; the consumer's `DefaultRead` already serves the slice/match for every tag, and the paradigm asks no executor to override them — an MCP stream is a `<tag>://` entry read exactly like an `sh` stream. The hooks exist and their signature is schemes', not execs' (`read(statement: ReadStatement, ctx: SchemeCtx)` — the parsed statement plus `SchemeCtx`, no execs-specific coord grammar on `<tag>://<coord>`), but a custom read is a **need a future executor would have to raise**, never a carve-out the contract grants any runtime up front.

**The executor mints no orientation receipt** — resolving service#240's open question (*where is the OrientIndex receipt built, the wrapper or the return?*): **neither, because none is built.** Orientation is model-pulled, uniformly and with MCP no exception: the model probes an output's shape by querying the entry (FIND / READ with jsonpath / glob / line-range) against the neutral extent metadata every entry already carries (`mimetype` / `tokens` / `lines`), pulling only the slice it wants. An engine-pushed digest *is* the catalog-as-index the paradigm forbids ("the instant it did, it would be an index again"). A `<tag>://` entry is folded by default, so output never floods context regardless; the one real risk — a chatty runtime filling sqlite — is a daemon storage-cap concern, not an executor one.
