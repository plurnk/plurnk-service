...
A successful candidate also carries `readTarget`, equal to its canonical HTTPS `url`. This is an explicit next-operation affordance, not a second address or retrieval dialect: the model must `READ` that target when it needs page evidence. Rejected candidates omit `readTarget`; their title, URL, and snippet remain useful discovery metadata but the system does not claim that a page body is available.
