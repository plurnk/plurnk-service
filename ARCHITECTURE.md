...
- **Known deviation to decide deliberately:** `READ(run://running-child)` returns a non-blocking **425** ("still running") — a *try-join* that invites the spin-wait busy-loop the cycle detector then strikes. Textbook fork-join makes the join **blocking**: the READ parks until the child terminates, so a spin-wait is structurally impossible. This is a primitive-design choice, not a per-case patch — change it (if at all) against the theory, with the liveness guarantee (bounded children + join-on-any-terminal + owed-wake) intact.
