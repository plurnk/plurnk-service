...
9. **plurnk-docs dissolved — and the corpus landed in the meta layer** (owner rulings mid-GO, two passes): first into `plurnk-core/docs/`, then corrected — the metaproject layer OWNS family prose, so the corpus (personality, requirements, scheme docs) lives in `@plurnk/plurnk-meta` as exported subpaths and the daemon pulls it at runtime (`Paths.#DOCS_ROOT`), same shape as grammar's sysprompt. The old npm package is deprecated with the -alls.
10. **@plurnk/plurnk-meta extracted** (owner ruling mid-GO; first shipped as `plurnk-plugins`, renamed pre-publish): the published metaproject layer — THE trust rule (knob moved here, reader-declares; `PLURNK_PLUGINS_` prefix kept as an owner-approved subject-named bend), scope-agnostic symlink-aware `packageDirs`, `nearestNodeModules`, plus the teaching corpus and the future home of family tooling (scaffolder). ONE implementation consumed by core (PluginLoader, env-defaults floor, service, test floor) and all four family heads; four duplicated trust predicates and five walks deleted. It is the MOM's package lane — grandma:grammar, mom:meta, big sister:core. ("membership" as a name was rejected — it already means the session-constraint overlay.)
...
- **B. Deprecations:** `npm deprecate @plurnk/<name>@"*" "<msg>"` for the four `-all`s ("retired — the
  ...
  @plurnk/plurnk-meta — the corpus ships in the metaproject layer").
