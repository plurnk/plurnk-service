...
9. **plurnk-docs dissolved into the daemon** (owner ruling mid-GO): the teaching corpus is core-native at `plurnk-core/docs/`, shipped in the tarball (`files`), `Paths.#DOCS_ROOT` package-relative. First-party content that always ships with the daemon needs no pull seam. The npm package is deprecated with the -alls.
