...
test("{§operation-fences}: a three-backtick operation runs and is told the taught width; ordinary code stays prose", () => {
    ...
    assert.deepEqual(warnings("```READ (notes.md)\n```"), ["`READ` ran with three backticks; the taught fence is four."]);
    assert.deepEqual(warnings("```sh\nnpm test\n```"), ["`sh` ran with three backticks; the taught fence is four."]);
    assert.deepEqual(warnings("````READ (notes.md)\n````"), [], "the taught width draws nothing");
    assert.deepEqual(warnings("The config:\n\n```ts\nexport default {};\n```"), [], "an ordinary code block draws nothing");
    // {§quotation} — the fenced example is the untaught form a weak model reaches for; it is told once.
    assert.deepEqual(warnings("Example:\n\n```\n````READ (notes.md)\n````\n```"), ["`READ` inside a code block was shown, not run."]);
    assert.deepEqual(warnings("    ````READ (notes.md)\n    ````"), [], "the taught offset draws nothing");
    assert.deepEqual(warnings("```\n```READ (notes.md)\n```"), ["`READ` inside a code block was shown, not run."], "a wrapped three-backtick operation is told the same");
