...
for (const [tier, parse] of [
    ...
    for (const width of [3, 4, 6]) {
        test(`#810: ${tier} accepts ${width}-backtick native and registered executor operations without warnings`, () => {
            const source = [
    ...
    test(`#810: ${tier} keeps indented and enclosed three-backtick operations inert`, () => {
        ...
        const result = parse([...examples, block(3, "NOTE", "Actual note.")].join("\n\n"));
        clean(result);
...
test("#810: complete nested three-backtick operations stay literal bodies", () => {
...
test("#810: shorter known headings cannot escape an unfinished outer body", () => {
    for (const [outer, inner] of [[4, 3], [5, 3], [5, 4]]) {
...
test("#810: equal-width missing-closer recovery and numeric delimiters work at three backticks", () => {
...
test("#810: compact and inline-chained operations accept either fence width", () => {
...
test("#810: reasoning accepts three-backtick NOTE only, preserving enclosing quotations", () => {
...
test("#810: canonical output stays four-backtick with shorter body fences intact", () => {
