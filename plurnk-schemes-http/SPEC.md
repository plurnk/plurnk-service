...
The **render path** takes one runtime dependency, `playwright`, **lazy-imported** (`Browser.ts`) so byte fetches do not load it. A normal installation provisions Playwright's compatible Chromium hermetically inside the installed package (`PLAYWRIGHT_BROWSERS_PATH=0`) and boot verifies it before reporting the daemon ready; execution therefore does not depend on the installing user's home-directory cache. An operator-set `PLAYWRIGHT_BROWSERS_PATH` remains authoritative. `PLURNK_SCHEMES_HTTP_BROWSER` explicitly selects `managed` (default), `remote`, `system`, or `disabled`; modes never fall back to one another. Remote mode requires `PLAYWRIGHT_WS`, system mode requires `EXECUTABLE_PATH`, and disabled mode makes HTML rendering fail clearly rather than treating raw HTML as equivalent. Package builders may set Playwright's standard `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` only when deploying one of those deliberate non-managed modes.
