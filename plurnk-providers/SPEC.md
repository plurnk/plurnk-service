...
**Native reasoning and the grammar are mutually exclusive; in-DSL reasoning is the working pattern.** The GBNF masks every sampled token. With NATIVE thinking live (llama-server's own `--reasoning-budget` default, or `enable_thinking: true`), the server auto-gates the grammar past the think block — reasoning flows free — but content-channel enforcement then **leaks** (unconstrained prose, degenerates, fabrication — reproduced live in #488: grammar-shaped prefix, then the raw thought channel bleeding into content) or content never arrives; explicit `grammar_lazy`/`grammar_triggers` are ignored on the chat-completions path. **The provider therefore enforces the exclusivity itself (#488): on the `template` style, a TRANSPORTED grammar forces `enable_thinking: false` for that request — rails win the channel, deterministically, per request — warning once (`PLURNK_REASONING_SUPPRESSED_UNDER_GRAMMAR`) when it suppresses a non-off intent.** No operator pairing of GBNF with `REASONING=off` is required (that pairing was the pre-#488 contract, and its omission was the #488 pub-blocker: the `adaptive` floor left the channel live under the rails). Grammarless calls map intent normally; GBNF-debug filter mode (grammar withheld) leaves the channel per intent — the run is deliberately unconstrained. With the channel closed, the model reasons **inside the DSL**: the canonical grammar's required `PLAN` statement is a free-text body the model fills with genuine step-by-step reasoning before acting (probed live, b894+gemma: correct chain-of-thought inside `<<PLAN:…:PLAN`, then a clean `SEND`, `finish_reason: stop`). The `PLAN` element belongs to the consumer's grammar contract; the provider's only job is closing the native channel deterministically.
