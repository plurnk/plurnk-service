...
- **`PLURNK_PROVIDERS_DRY_MULTIPLIER` / `_DRY_BASE` / `_DRY_ALLOWED_LENGTH`** -- the DRY loop-breaker (#567), the GENERIC llama.cpp default floored in `.env.defaults` (`0.8`/`1.75`/`2`, the community-standard DRY values, model-agnostic) and applied on the detected `llamacpp` path exactly like `REPEAT_PENALTY`, customer-overridable per-alias. DRY penalizes repeated SEQUENCES with a penalty escalating in run length -- the tool for a plan-restart loop a single-token `repeat_penalty` over a short window can't see. **`PLURNK_PROVIDERS_REPEAT_LAST_N`** (optional) widens the older `repeat_penalty` window past the box's 64. Sent **only on the `llamacpp` path** (DRY is a llama.cpp sampler); a cloud provider parses but never emits them.
