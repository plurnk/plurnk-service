...
**Debug mode — diagnose grammar/model conflicts unconstrained (`PLURNK_GBNF_DEBUG`).** An optional, off-by-default toggle (`0`/empty = off; any other value = on; the standard-provider config field is `gbnfDebug`). When on and a grammar would be transported, `generate` first validates it with `@plurnk/gbnf` **before any wire call** and **throws fail-hard on a malformed grammar**; it then **withholds** the grammar so the model generates **unconstrained** — and runs that free output back through the grammar, **throwing `grammar_unenforced` at the exact code point where the model's natural output diverges from the grammar**. The debug throw carries the **full unconstrained output** the model produced — both the reasoning and content channels, labeled — so you see what the model wanted to say *and* where it collides (the §12 terseness policy is for production transport errors; debug deliberately dumps everything). The divergence code point indexes into the content channel; the reasoning channel is shown for context, not validated. This is the conflict-diagnosis mode: a constrained call masks every token, so when enforcement traps or degrades the model you cannot see *why*; debug mode lets the model speak freely and reports precisely where its inclination and the grammar collide (e.g. a `SEND[200]` the grammar's `no200` rule forbids, or a `</think>` the committed-`PLAN` region rejects). Withholding-without-verifying would be indistinguishable from sending no grammar at all — the verification is the point. Leave it off in production, where a constrained call should actually constrain.
