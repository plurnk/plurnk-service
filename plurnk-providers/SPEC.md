...
The `plurnk` entry alone sets **`firstPartyMetadata: true`** — it forwards the consumer's per-turn `generate()` `attributions` (which installed plugin packages dispatched) and `client` (the originating frontend, e.g. `plurnk.nvim/1.4.0`) as `Plurnk-Attribution` / `Plurnk-Client` headers, and the opaque `workerId` as `Plurnk-Worker-Id` (#26, wire-name completed #511). The gate lives on the provider, not the call site, so these first-party signals are **structurally incapable** of reaching a third-party backend. Empty values emit no header. It also alone sets **`balanceMetaKey: "balance_pico"`** — the top-level response field the plurnk endpoint reports the running account balance (pico-USD) in. `OpenAICompatProvider` surfaces the backend's extra top-level fields as `ProviderResponse.meta` for **every** provider (passed through), and for a provider that set `balanceMetaKey` it additionally normalizes that field into a validated `meta.balancePico`. So `balancePico` appears only from plurnk; the raw pass-through `meta` is general (#23).

A spec may carry **`promptCacheKey: true`** (#518) -- the provider then sends the OpenAI-standard `prompt_cache_key` set to the `workerId` on every request. Fireworks (and OpenAI) prompt-cache automatically, but on serverless the cache is REPLICA-LOCAL; without an affinity key a worker's turns scatter across replicas and the stable prefix never hits (verified live: `cached_tokens` 0 without the key, ~68% of a 17.5k prefix with it). `workerId` -- already the slot-affinity identity -- is exactly the opaque per-conversation key the cache wants. Managed + reserved from caller `sampling`. Set only where the backend accepts the field (fireworks verified; a strict-validating backend 400s on an unknown one, so it is opt-in per spec, not universal).
