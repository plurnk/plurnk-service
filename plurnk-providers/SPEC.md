...
- **`PLURNK_PROVIDERS_DRY_MULTIPLIER` / `_DRY_BASE` / `_DRY_ALLOWED_LENGTH`** -- the DRY loop-breaker (#567), a llama.cpp sampler penalizing repeated SEQUENCES (escalating in run length) -- the tool for a plan-restart loop a single-token `repeat_penalty` can't see. **Off by default**: the community-standard values (`0.8`/`1.75`/`2`) were MEASURED to regress plurnk -- `ALLOWED_LENGTH=2` penalizes the faithful identifier/path/op-syntax repetition plurnk requires, drifting the model off correct tokens (`app.ts`→`app.js`). Plurnk-tuned values are pending a knob-sweep; the knob is live and per-alias-overridable meanwhile. **`PLURNK_PROVIDERS_REPEAT_LAST_N`** (optional) widens the `repeat_penalty` window past the box's 64. Sent **only on the `llamacpp` path**; a cloud provider parses but never emits them.
