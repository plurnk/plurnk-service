...
- **`PLURNK_PROVIDERS_THINKING`** — REQUIRED, one of `off | adaptive | on` (#33). Pure ACTIVATION intent; **`PLURNK_PROVIDERS_THINKING_CAPACITY`** (positive int, REQUIRED iff `on`) carries the magnitude separately, so a number can never silently flip wire flags. The provider maps intent to the active backend's mechanism — llama-server `chat_template_kwargs: { enable_thinking }` (always emitted; the explicit FALSE is the only working off-switch; the NUMERIC clamp is the box's `--reasoning-budget` LAUNCH flag — per-request numerics are ignored, so env capacity and launch flag are the same number, changed together), Ollama `think`, relay `include_reasoning`, cloud `reasoning_effort` (tier from capacity when `on`; `off`/`adaptive` omit), fireworks `reasoning_effort` **explicit** (`effort_explicit`: `off` SENDS `"none"`, `adaptive` SENDS `"adaptive"` — omission is NOT off on reason-by-default models like DeepSeek V4, #30. Intent maps **identically under a transported grammar** (the former #32 clamp-to-`"none"` is lifted — it was measured pre-`max_tokens`-cap and caused plan-less turns, service#331; canary-verified reasoning+rails coexist, TUNING-EPIC F9). But the reasoning **channel** is not reliable under `response_format`: the grammar demotes the CoT into `content` (the `reasoning?`/preplan free zone absorbs it), so `reasoning_content` is often EMPTY — near-certainly at high effort, stochastically at low (providers#41, F9-corr). Conformance is unaffected; a consumer wanting the reasoning must parse the preplan region of `content`, not depend on `assistant.reasoning`. Envelope: high-effort reasoners need `max_tokens` headroom — deepseek-v4-pro at `high` overruns a 4096 cap; `low`/`adaptive` conform within it), Anthropic `thinking: disabled | enabled+budget_tokens(capacity) | omit`. Consumers state intent, never mechanism. (In-DSL `<<PLAN>>` reasoning has **no provider footprint** and no operator knob: `PLAN` is a required element of the canonical plurnk grammar the consumer attaches via `generate`'s `grammar`. The provider transports that grammar verbatim and never forces, prefills, or toggles `PLAN`.)
