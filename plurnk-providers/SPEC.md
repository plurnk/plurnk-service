...
- **`PLURNK_PROVIDERS_DRY_MULTIPLIER` / `_DRY_BASE` / `_DRY_ALLOWED_LENGTH` / `_REPEAT_LAST_N`** -- OPTIONAL llama.cpp loop-breakers (#567; read via `parseOptionalFloat`/`parseOptionalInt`), **off by default** (absent = the backend's own default). DRY penalizes repeated SEQUENCES with a penalty escalating in run length -- the tool for a plan-restart loop a single-token `repeat_penalty` over a short window can't see; `REPEAT_LAST_N` widens that window. Sent **only on the `llamacpp` path** (DRY is a llama.cpp sampler). The VALUES are operator/bench config, deliberately NOT a floored default -- a value here would be a per-model/benchmark guess.
