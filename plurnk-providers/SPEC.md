...
**Grammar and reasoning are mutually exclusive in practice (v0).** The GBNF masks every sampled token, so with reasoning off (`PLURNK_REASON=0`) the constraint applies from token 0 and prose thinking is foreclosed — the working configuration. With reasoning ON, llama-server (b894) auto-gates the grammar past the thinking block — reasoning flows free — but content-channel enforcement then **leaks**: emission resumes as unconstrained prose and degenerates. Explicit `grammar_lazy`/`grammar_triggers` are ignored on the chat-completions path (the chat-format handler owns the gating), so the transport cannot fix this by serialization. Operators: do not combine `PLURNK_REASON>0` with constrained sampling until the server's lazy-grammar gating is fixed upstream.
