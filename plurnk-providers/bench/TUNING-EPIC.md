...
- **The mask covers ONLY the content channel**: deepseek-v4-flash, `reasoning_effort:"low"` + grammar → `reasoning_content` populated with free CoT (966c/859c, two workers) AND content ACCEPT (textbook 4-op turn, terminal SEND). `high` + grammar: ACCEPT twice (reasoning native-channel or in-band). Reasoning and rails, one call.
...
## F9-final — reasoning + hard-masked GBNF rails do NOT reliably coexist on fireworks (2026-07-07; exhaustive, supersedes F9's "coexist" claim)
F9 said "reasoning and rails coexist on fireworks." WRONG — retracted after ~60 live tests (flash+pro, real packet+gbnf, all 5 effort tiers, grammar-structure + prefill variants, streamed/not).
- **Fireworks docs**: `response_format` (Structured Output umbrella — INCLUDES grammar/BNF mode) "disables reasoning output." Measured true: under response_format grammar, reasoning reaches the usable reasoning_content channel only STOCHASTICALLY (~1/8 baseline); otherwise it collapses or leaks into content as unmarked prose.
- **Levers tested, none deterministic**: grammar structure (full / strict-no-preplan 5/8 / require-think collapses to ~100c / zero-free-text 0/8 → reasoning simply doesn't happen); prefill (`<think>` 2/8, `<think>\n` **7/10 best**, `<think>\n\n` 4/10 no-runaways-but-less-reasoning); strict+prefill incompatible 0/10; effort tiers don't move the channel. Best-case 7/10 carries a CATASTROPHIC tail — reasoning floods content to 24k–44k chars, invalid.
- **The #32 clamp LIFT (0.36.0) still stands**: sending reasoning_effort under grammar enables the best-effort content-preplan reasoning that avoids the #331 plan-less regression. What's corrected is the CLAIM of clean coexistence, not the lift.
- **Verdict**: fireworks is a rails-first cloud provider. Rails hard-enforced (~92% + verify); reasoning is best-effort, unsegregated (lands in content, not reasoning_content), NOT tunable under the grammar. Single-call reasoning + hard-masked rails needs a llama.cpp lazy-grammar host, not fireworks.
