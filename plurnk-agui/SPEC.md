...
| `log/entry` READ of `reasoning://<worker>/…` or `ops://<worker>/…` | An ordinary operation receipt, not a reasoning stream. Standard reasoning delivery and replay retain original provider evidence under {§agui-readable-reasoning}. |
...
events, replay and `log.read` preserve literal native bodies. No task inventory
or ACP plan is inferred from prose.
...
Core also derives optional complete `reasoning` on admitted SEND, NOTE and disposition
...
atomically before the turn's first corresponding row under `<OP identity>/reasoning`.
...
accepted `ReasoningMessage` once per turn in
`MESSAGES_SNAPSHOT`. Empty evidence emits nothing, NOTE never substitutes for
reasoning ({§agui-lifecycle-projection}), and a response row still precedes its text
sequence.
...
| `workspace.share`        | Workspace | `folder`                                             | `ApplicationPort.shareWorkspace` shares the bound workspace ({§share}) into an absolute `folder` on the service host; returns `{ folder, zip }`. |
...
The observation resolves `log:///` as the thread's conversation worker; `workerId`
pins another workspace worker. Explicit resource authorities retain their identity
under {§turn-source-resources}. The closed observation segment stays on the connection's own
