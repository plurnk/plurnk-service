...
- §agui-thread-is-run **A plurnk SESSION is the world; an AG-UI THREAD is a conversation over it**
...
- §agui-custom-namespace **The custom namespace** — plurnk-specific metadata rides
  ...
  `plurnk.notice`, `plurnk.stream`, `plurnk.branch_batch`, `plurnk.quiesced`, `plurnk.terminated` — the full loop
...
## §agui-broadcast-fan Event fan and run settlement
...
## §agui-run-endpoint The worker endpoint
