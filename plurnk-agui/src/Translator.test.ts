...
test("[§agui-projection] a model op row is a TOOL_CALL triple with its rx as the RESULT", () => {
    ...
    assert.deepEqual(events.map((e) => e.type), ["TOOL_CALL_START", "TOOL_CALL_ARGS", "TOOL_CALL_END", "TOOL_CALL_RESULT"]);
...
test("[§agui-projection] PLAN is thinking; SEND is assistant speech with the signal on plurnk.send", () => {
    ...
    assert.deepEqual(plan.map((e) => e.type), ["STEP_STARTED", "THINKING_TEXT_MESSAGE_START", "THINKING_TEXT_MESSAGE_CONTENT", "THINKING_TEXT_MESSAGE_END"]);
    const send = tr.logEntry(entry({ op: "SEND", signal: 200, status_rx: 200, tx: JSON.stringify({ body: "done and dusted" }) }));
    assert.deepEqual(send.map((e) => e.type), ["TEXT_MESSAGE_START", "TEXT_MESSAGE_CONTENT", "TEXT_MESSAGE_END", "CUSTOM"]);
    const custom = send[3] as { name: string; value: { signal: unknown } };
...
test("[§agui-custom-namespace] ambient (origin plurnk) rows ride plurnk.ambient; the model mirror row emits nothing", () => {
    ...
    assert.deepEqual(ambient.map((e) => e.type), ["CUSTOM"]);
    assert.equal((ambient[0] as { name: string }).name, "plurnk.ambient");
    const mirror = tr.logEntry(entry({ op: "model", tx: "<<PLAN:x:PLAN" }));
    assert.deepEqual(mirror, [], "the verbatim mirror is forensic, not speech");
});
