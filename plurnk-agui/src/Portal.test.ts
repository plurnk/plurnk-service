...
test("a worker without pending interrupts drives the loop, then live events fan as AG-UI", async () => {
    ...
    seen.length = 0;
    m.fire(3, "log/entry", { entry: { id: 2, worker_id: 10, loop_id: 77, origin: "model", op: "SEND", status_rx: 200, rx: { answers: [] }, coordinate: "1.1.1", tx: { body: "hi" }, turn_id: 1 } });
    ...
    seen.length = 0;
    m.fire(99, "log/entry", { entry: { id: 3, worker_id: 1, loop_id: 1, origin: "model", op: "SEND", status_rx: 200, rx: { answers: [] }, tx: { body: "x" } } });
