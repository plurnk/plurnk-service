...
const mockSeam = () => {
    ...
    const seam: DaemonSeam = {
        ...
        resolveProposal: (logEntryId, resolution) => {
            ...
            setImmediate(() => handlers.forEach((h) => h(3, "loop/terminated", { loopId: 1, result: { status: 200 }, hitMaxTurns: false, turnIds: [1], usage: { promptTokens: 1, completionTokens: 1, costUsd: 0, projectedCostUsd: 0, costs: [], contextTokens: 2, promptBudget: 1000, meta: {} } })));
        },
    ...
    const finish = (workspaceId: number | null) => setImmediate(() => handlers.forEach((h) => h(workspaceId, "loop/terminated", { loopId: 9, result: { status: 200 }, hitMaxTurns: false, turnIds: [1], usage: { promptTokens: 1, completionTokens: 1, costUsd: 0, projectedCostUsd: 0, costs: [], contextTokens: 2, promptBudget: 1000, meta: {} } })));
...
test("a workspace's stream events fan to every open AG-UI Run (never last-binder-wins)", async () => {
    ...
    try {
        ...
        emit(3, "loop/terminated", { loopId: 9, result: { status: 200 }, hitMaxTurns: false, turnIds: [1], usage: { promptTokens: 1, completionTokens: 1, costUsd: 0, projectedCostUsd: 0, costs: [], contextTokens: 2, promptBudget: 1000, meta: {} } });
