...
test("log/entry (model op) → TOOL_CALL; loop/terminated → STATE + RUN_FINISHED", () => {
    ...
    const term = r.route("loop/terminated", { loopId: 1, result: { status: 200 }, hitMaxTurns: false, turnIds: [1], usage: { promptTokens: 5, completionTokens: 6, costUsd: 0, projectedCostUsd: 0, costs: [], contextTokens: 11, promptBudget: 200000, meta: {} } });
...
test("terminated serves the standard RAW channel — the provider's native completion frame", () => {
    ...
    const ev = router().route("loop/terminated", { loopId: 1, result: { status: 200 }, hitMaxTurns: false, turnIds: [1], usage: { promptTokens: 5, completionTokens: 6, costUsd: 0, projectedCostUsd: 0, costs: [], contextTokens: 11, promptBudget: 200000, meta } });
    ...
    const bare = router().route("loop/terminated", { loopId: 1, result: { status: 200 }, hitMaxTurns: false, turnIds: [1], usage: { promptTokens: 5, completionTokens: 6, costUsd: 0, projectedCostUsd: 0, costs: [], contextTokens: 11, promptBudget: 200000, meta: {} } });
