...
- **Client activity is DSL ops in its own worker; the model's worker is its conversation.** The packet renders the *model's* worker, so a client's own worker is **structurally invisible** — invisibility by *worker*, not a render-filter (no origin-tag). Two deliberate cross-actor channels: **`btw`/inject** (voice) and the **entry-change → log waterfall** (environment — a signal, not a turn). Chosen over the #194 origin-filter: one way for a thing to be invisible, not two. {§actor-boundary-isolation}, {§machine-processes-one-filesystem}, §connection-lifecycle.
...
- [x] 811s pipeline stall ROOT-CAUSED, engine exonerated: children parked-before-working → ∅-collapse concluded them with the park text AS deliverable → parent re-spawned ×9. Filed: #379 (deliverable design question, owner's call) + grammar#65 (the (worker://name) placeholder bait — a worker literally named 'name').
...
- [x] **#310** — CLOSED on A/B evidence: flash went from 10-docs-EDITs/empty-patch to 21/21 file-scheme source EDITs (bench run42); gemma edit-stories green ×3 boards. Shipped levers did it (File doc line, EDIT diff render, membership gates); catalog-ordering + zero-write-steer levers stay unbuilt.
- [ ] **#333** — search epic service lanes (dedup/caps, TTL, auto-prefetch, dead-row elimination): THE remaining work epic. Fresh-context start recommended — scope the lanes against execs#18 north star before building.
...
- ff4c766 #366 createConversationWorker (client integrates threadId ↔ workerName)
...
- [x] **#381** — CLOSED no-fault: model redirected output to /tmp files (empty streams, correct) + read them back (404, non-member, correct). No service fault; downstream of #382 frame-loss. Teaching reinforcement flagged to execs-common#3.
- [ ] **#382 ROOT-CAUSED — awaiting owner's addressing call before building** — TWO faults: (1) SERVICE BUG — a WORK-spawned worker's turn-1 prompt foist collides with the parent's on /prompt/1/1 (loopSeq per-worker always 1, entries workspace-shared); worker clobbers parent's task. FIX needs owner's pick: (a) run-qualify /prompt/<workerId>/<loop>/<N> vs (b) worker-scope prompts under worker://. (2) DESIGN (owner) — active task body folds out mid-run, model won't re-read the listed path. Both proven in run43. OLD: — User Prompts section EMPTY every packet incl. 001 (mission points at nothing; task only ever in the foldable turn-1 log row → vanishes when grinder folds). PLUS /prompt/1/1 overwritten mid-run 1409tok→205chars. REMAINING: why the section sourced empty when entry+body existed (prep replays clean — fault is packet-side sourcing/pattern, PacketBuilder ~206-220 + line 314); and whether the prompt entry should be model-writable. Old note: **#382** — per-turn packet-frame re-derivation tax (model treats its workspace as a pasted transcript, third-persons its own past). Packet-framing design — OWNER territory (packet-refinement doctrine); high leverage for the gemma-floor goal. Discuss before building.
...
**Follow-up forensics (next workspace):**
- [ ] **topo two-stage pipeline stall** — 0.86.2 drill chunk-2 red at 811s (a STALL-to-timeout, not the usual ~150s contention fail). Corpse kept in test/intg/.tmp (workspace demo-topo/pipeline-*, ~07:4x window 2026-07-11). Digest before dismissing as the heavy-topo variance class.
...
- [x] **#370** — shipped (1352b52): items 1-4 (worker:// one form, catalog-relative file renders, COPY/MOVE span parity, idle-409 remedy). Item 5 DROPPED — collided with anchored §tokenomics-over-budget-floor; was an uncritical model-complaint adoption.
- [x] **#366** — shipped (ff4c766): createConversationWorker on the seam; three doors complete (ensure/fork/create).
...
**Standing workspace lessons now in memory** (adopt-the-lattice-front, digits-are-free, check-before-asserting-state, check-prior-art-and-ownership-before-building, spec-before-code) — read MEMORY.md, don't re-learn.
...
- **Operator env surface (#549, owner ruling):** `~/.plurnk/.env` is the house surface for projectwide box settings shared across agents (default model SKU etc.) — let it cascade naturally. Agents NEVER create or maintain `.env`/`.env.test` in worktree folders (private shadow config drifts and bites — #532/#549). A one-off knob for a single run is a shell override on the command (`PLURNK_MODEL=x npm run test:demo`), never a file. Live/demo model selection is per-operator and categorically off-repo (nobody shares hardware; no portable form exists). The cascade tooling itself is settled machinery — read it before ever proposing changes to it.
...
Invariants: writableBy gates writes; the capability sheet and docs render only what the workspace can actually use (#328 — never taught-then-refused); the entry() sink materializes+tags+narrates via the plurnk worker's ambient rows (tags ride the SIGNAL slot) {§exec-entry-sink}.
Tests: Exec.effect, execs-workspace-policy, exec-entry-sink, run-scheme. Don'ts: never edit another repo's scheme package; daughters own their how.
...
A broad survey (open issues + this doc's User Notes + the budget chapter) clustered by shared context. **v0.69.0 is the published stable checkpoint** — the budget/context/reasoning math is DEFINED AND SHIPPED (window partition {§tokenomics-window-partition} + calibration + maxTokens on every call), delegation is the dedicated FORK/WORK verbs (grammar 0.74.57; named forks, COPY is entry-copy only — fanout + script-greet green on gemma, the delegation-mangle class closed) inheriting authority {§worker-delegation-inherits-flags}, the op-bound (tail unrolled to depth 14) ended the 20k-token stroke class, plurnk:// is model-read-only, and the engine no longer re-derives grammar rules (submitted-read gate removed — preemie reads are GBNF steering). The TEACHING CORPUS lives in @plurnk/plurnk-docs (scheme docs, personality, requirements — Paths.DOCS_ROOT); grammar keeps plurnk.md (language vs world split). Teaching/steer asks file on plurnk-docs, NOT here. #308 awaits a bench re-run on 0.69.0; #312 (tokenizer_id keying) is post-freeze. **Cluster I is the active agenda** — teaching/steer iteration against the frozen engine, now on the docs daughter's cadence. **First digest grind (v0.64.0 sweep, 2026-07-03) — the tripartite findings:** the sweep's reds reduce to TWO mechanism classes, each filed on all three lenses. (1) **Pressure-correlated valid-prefix runaways** (grind/delegate 422s: 12–21k code-point legal prefixes, never terminating, discarded whole): train = emission-brevity-under-pressure teaching (grammar#50), rail = prefix salvage with the strike kept (#317, feasible via providers 0.31 err.attempt), ponder = strict-reject-vs-salvage recorded on #317. (2) **Pressure-strike vs convergence** (jumbo: correct FIND→range-READ scanning, novel fingerprints, struck out at 3 by per-fire budget strikes + 413 double-jeopardy): train = curate-while-scanning habit (grammar#50), rail options + the evidence against §grinder-compaction-strikes' no-exemption ruling = #318, OWNER RULING PENDING — the citing tests move in lockstep with the ruling. paris→tokyo stays pure test calibration (grant turns or admit the bound). Specimens kept: grind db-764d651a, delegate db-a76a7e88, jumbo db-d8e45afa.
...
- **THEN — Budget chapter (cluster C, after bugs clear):** budget-meta.test.ts runs in the demo tier (floor-relative pins, factor 1.6) · revisit budget formulas pre-launch · the 238s degenerate-flail run. (requirements.md lives in @plurnk/plurnk-docs now — its FOLD/KILL MUST + dedupe-vs-plurnk.md questions are the docs daughter's.)
- **Op Semantics Contract (cluster A)** = the TOP-PRIORITY epic immediately below; gates budget; #278 Stage 2 is the live work.
- **Strategic (cluster F):** packet pluggability / the complete-override audit (the moat) + docs-in-hot-path vs manifest + docs-catalog residue (schemes#25, workspace-scope seeding).
- **Launch readiness (cluster G):** shipped-cascade LOOP run — no tier runs a real loop under the full shipped env (installation e2e boots but doesn't loop; shipped-defaults.test covers the template statically); close with a local model + shipped cascade once practical · naive install e2e (`test:installation` — verified, 10/10, guards no-model + pointer) · `--api-key` flag · SEARXNG default → model.plurnk.ai · i18n · out-of-sqlite-space story · digest golden refresh · CI cadence · more-providers shakedown.
- **Last-mile model/agent forensics (cluster I — owner steer 2026-07-01, first thing post-prepub):** the terminal-contract steers verify claims correctly (§send-premature-terminate, §send-groundless-hibernate; live/demo digests prove every rail firing per SPEC) — the open variable is gemma's ADAPTATION to a steer: the same refusal repeats verbatim to strike-out on some runs (paris→tokyo) and resolves next-turn on others (config-lookup). Work items: probe steer WORDING as a tunable (does prescriptive "SEND[102] to receive it" converge faster than a terse conflict?) — steer prose lives in plurnk-docs/engine messages · paris→tokyo test acceptance (429 at `maxTurns:6` is a legible bound, not a failure — admit it or grant turns) · whether any rail should SEE no-progress-with-variety (fingerprints deliberately read varied matchers as exploration; the loop wall + turn ceiling bound it today) · budget-tier digests: under a pinned ceiling the model emits ZERO FOLD/KILL curation even across three consecutive overflow steers (grinder → 3 strikes → 500) — the log-KILL lever is REACHABLE now (writableBy fixed) but UNUSED; a teaching/steer question, and the floor-vs-ceiling arithmetic (the static packet alone can exceed a demo ceiling → instant hard-413) belongs to the budget-formulas revisit (cluster C) · proposal-REJECT rx carries only {status} (deliberate hygiene, but a model gets zero why — reconsider a terse reason on reject) · live-thing 409s remain engine-owned (CFG can't see runtime children) · jumbo chunk-reading under pressure (the model PLANS the right FIND→range-READ recovery, budget exhausts it — the one standing demo red) · #331 (bench): deepseek-v4-pro concluded with ZERO source edits on 0.68 (edited source on 0.62) — bisect the steering delta on a 0.69 re-run · budget grind/codename stay the cluster-C pinned-ceiling frontier · #310 (bench): weak models substitute scratch/docs edits + shell dumps for repo-source EDITs — hypothesis: the source is the ONLY GATED write (file:// proposes, everything else instant-200s), so the least-resistance path is every path but the task; levers = File documentation line / catalog ordering / zero-member-write steer. A/B with the bench DBs.
...
### ACTIVE EPIC — Packet honesty III: the log contract, the error taxonomy, recoverable budget (owner steer 2026-07-08) — TOP PRIORITY, no deadline

**Why (owner):** SPEC-jumbo's demo failure was NOT a hard task the model lost — it was the model doing REASONABLE things (curate a turn, search its log, investigate a flagged error) and getting punished by four packet/engine defects. "We are not hunting the immediate bugs under our noses. Systems perspective — converge on the CORRECT patterns, not sloppily turn red lights green. Errors are information; turning the pointers off is never the goal." Same spine as the Op Semantics Contract epic: the model must be able to TRUST what the packet tells it. [[audit-tooling-before-blaming-model]] [[packet-hygiene-before-instructing-model]]

**The forensic (run: `benchmarks/demo-fresh-fails/`, digest in `test/digest/`):** tight ceiling 13029. The model NEVER read SPEC.md (the task). T4: it tried to curate — `FOLD(log:///1/2)` — 400'd (no trailing slash). The 400 minted `{source:"engine", message:""}` → the model read an "engine run 400 error" and burned 10 turns chasing a phantom: `FIND(log:///**):~engine` ×6 (501 — log has no find), re-reading plan.md. The rabbit-hole log overflowed → hard-413, immediate death, no recovery. Four defects, one theme (the packet must not lie or misattribute):

- **Q1 — the log fold idiom is undocumented AND the intuitive form 400s.** `parseCoordinate` needs 3 parts; `log:///1/2` → null → not a glob, no slash → 400. Only `log:///1/2/` works; plurnk.md documents NEITHER (shows `log:///1/2/3` + `log:///**`). **Fix (pattern): log coordinates are HIERARCHICAL prefixes** — `log:///1` = loop 1's rows, `log:///1/2` = turn 1/2's, `log:///1/2/3` = the row; the trailing slash becomes an optional alias (uniform with `READ(known:///docs/)`). Document the fold-a-turn idiom in plurnk.md (grammar#issue, mom→grandma).
- **Q2 — a model-op failure is dressed as an internal engine error with an empty message (owner-clarified 2026-07-08).** There is NO "internal error we show the model" category — that was my mislabel. A `FOLD 400` is a MODEL-OP RESULT (the model's own action failed); it must see it, as its op result, truthfully. Internal engine faults CRASH (fail-hard — verified `Engine.ts:894/463`: ProviderError→telemetry+throw, others re-throw), never shown to the model. My `source:"engine"` mint made a model-op result read as an engine fault → the phantom "engine run 400 error." **Fix:** op failures attributed to the model's OWN op (drop "engine"), carrying the op's REAL message (ops MUST explain their failures — `Log #resolveIds` returning bare `{status:400}` is the wart); the errors-line points at the OP ROW (the row IS the result — no separate minted item); rail signals (budget/strike/steer) stay `engine:rail`; genuine internal faults keep crashing. No fictional third category.
- **Q3 — log:// is advertised FIND-able but has no find handler; the real defect is that log is BESPOKE (owner-decided 2026-07-08: UNIFORM QUERY CONTRACT, Option B).** plurnk.md:148/159 show `FIND(log:///**):#…#` and `READ(log:///1/2/3):~high-signal`; `Log.ts` is coordinate-only, hand-rolled, doesn't touch the shared `Matcher`. **Decision — unify the QUERY layer, not the storage.** log_entries stays the honest event stream (immutable, coordinate-ordered, event-shaped, kill-only — a genuinely different KIND than a mutable keyed entry; forcing it into the entries table spawns a read-only exception + migrates the grinder/strike/proposal/digest machinery, worse than the disease). Instead make `_entry-find` / `read-resolve` / `Matcher` / `rankSemantic` SOURCE-AGNOSTIC: they enumerate log rows as candidates `{coordinate, rendered-content}`, apply the SHARED matcher, return coordinates. Then FIND/READ with EVERY dialect (regex/jsonpath/xpath + `~query` once the pump embeds log rows) works on log BY CONSTRUCTION — the full contract. The "one address space" invariant is about entries (mutable content); log is a different kind; B honors both the uniform-query truth and the distinct-storage truth. (The impedance analysis that killed Option A: log rows are events not content; immutable → EDIT must 501 → a special-case *entry*; unbounded volume → the pump embeds every op; the budget/strike/proposal/digest machinery is welded to log_entries.)
- **Q4 — the hard-413 kills without recovery (owner-confirmed: recoverable strike, NO margin).** `enforceBudget` folds only the newest turn (§grinder-layer1-rollback: model curates history); a hard overflow → immediate 413 death, no strike, no curate turn — yet the overflow lives in foldable HISTORY the model owns. **Fix (recovery rail): a hard-413 is a RECOVERABLE strike** — one turn, minimal packet, "over budget; KILL/FOLD history to reclaim room" steer, + a strike. Curate → recover; repeated → strike out. Death only after the model was told and declined. **NO 95% margin** — a margin MASKS the pointer (against "errors are information"); decode is separately reserved so 100% of promptBudget needs no response headroom.

**Embedding selection (owner-resolved 2026-07-08): UNIFORM — no log-specific selection.** My "content-bearing only" proposal was a heuristic Z4 doctrine forbids ("junk-eligibility is the operator's NO_EMBED decision table, never heuristics"). Protocol-ack dust ({"status":201}) self-filters at query time anyway (rankSemantic FTS-narrows FIRST — no query's terms select an ack into candidates; its embedding is never consulted). Log rows embed under the SAME pump rules as entries. Known consequence, not a blocker: a READ result row duplicates its source entry's content — ~query may surface both (informative: the content AND when it was seen); ranking is the semantic layer's job.

**Staged plan (resequenced for context warmth):** S0 plan + answers (DONE) → S1a Q1 hierarchical coords (DONE, {§log-coordinate-hierarchy}, grammar#57) → S1b Q3 source-agnostic primitive (DONE, {§find-source-agnostic}, EntryFind equivalence-proven) → S1c Q3 wiring: Log.find on the primitive + candidate query + fanout READ conventions (NOW) → S2 Q2 error taxonomy (op-result attribution + real messages + errors-line-points-at-row; internal faults keep crashing) → S3 Q4 recoverable 413 (recovery rail, no margin) → S4a jumbo re-run (partial proof: curation + honest errors + recovery) → S5 Q3-semantic: the pump embeds log rows (uniform rules) so ~query works on log → S4b final jumbo (the full trust proof incl. the model's ~engine-style semantic log search).
...
`worker://` (spawn/fork/irc) is built + SPEC-anchored (§worker-scheme), honored end-to-end (`run-scheme.test.ts`): spawn = EDIT (new sister + prompt), fork = COPY (Fork.fork the log + prompt), irc = SEND (deliver via Daemon.inject). One engine seam — `ctx.injectWorker` → `Daemon.inject`. Remaining:
- [ ] Pin the remaining hand-curated grammar examples in `plurnk-md-conformance.test.ts` (matcher-only) — `#channel` READ; the `worker://` family optionally (behavior already covered).
...
- daughter (schemes — raise-the-need): `SchemeManifest.category` is `"data"|"logging"` with no control value; `worker://` is forced to `"data"` (inert — category is read nowhere). schemes#21.
...
### ACTIVE EPIC — Scope model `{workspace, run}`: worker-scope scratch + the docs catalog

> **⚡ IN-FLIGHT (2026-06-22) — live design w/ owner. SUPERSEDES parts of this epic (worker_id → owner-in-pathname; item E → manifest→FIND) and reshapes the Packet epic's catalog. worker-scope + the manifest→FIND merger are CO-DEPENDENT (worker-scope rides the unified FIND). Nothing built yet.**
>
> **worker:// — ONE scheme: control + worker-scoped storage. Authority = the worker, ALWAYS.**
> - `<name>`/`<alias>` are the same thing — the worker's **immutable name IS the domain/authority** (refines the older "workerId as authority"; control scheme already resolves by name). Path = the entry. `worker:///` = empty authority = self.
> - **Storage = ordinary entries, NO worker_id** (SUPERSEDES item C's "worker_id discriminator"). Owner = the pathname's first segment = the folded authority; names immutable → stable; a `worker_id` FK would be a 2nd source of truth. Row `(scope='run', scheme='run', pathname='/<owner>/<path>')`. `EntryOps.#pathnameOf` ALREADY folds authority→path, so `worker://other/x`→`/other/x` is free; only self (`worker:///x`) needs the empty→my-run-name inject, in Run.ts before it delegates.
> - **`scope='run'` is LOAD-BEARING** (not redundant w/ scheme='run'): every entry query filters `WHERE scope='workspace'` (crud / Engine.sql manifest / FIND / ops / channel-write) → scope='run' rows auto-excluded → isolation free; owner opted back in only on read paths that should see own.
> - **Ops** — path-presence discriminates: path present → storage (READ any worker's entry by address = cross-worker READ ALLOWED; `EDIT worker:///x` self only, cross-worker write → 403). Path absent → control: `EDIT worker://<new>` spawns (self→400), `SEND worker://<name>` ircs, `COPY` forks.
> - **Breaking**: today Run.ts reads the worker from the PATH; switch to AUTHORITY (`target.hostname`); spawn/irc re-address `worker:///name`→`worker://name`.
> - **No grammar blocker**: grammar parses `worker://` generically; `scope` is our own `entries.scope` column. Item C's "scope enums gain 'run'" is a phantom — verify, don't re-file.
>
...
>
> **▸ manifest→FIND MERGER COMPLETE (2026-06-22, branch `feat/worker-scope-catalog`, pushed @ `4602fab`).** FIND **is** the catalog; there is no `plurnk:///manifest.json`.
...
> - **RESOLVED (87432fa):** SPEC `§worker-scheme` storage anchors landed (`{§worker-scheme-scratch}`/`-find-perspective}`/`-fork-scratch}`). `FIND(**)` ban = non-issue (parses `kind:local`, a scoped file-scheme glob, not a toxic cross-scheme global; the foist never emits bare `**`). plurnk.md advertising is the grammar's, not ours.
>
> **Build sequence:** **A** enrich FIND→catalog-row (cross-scheme JSON array, jsonpath) so `FIND(**)`≡manifest · **B** foist `FIND(**)` at worker's first turn + decouple pump + delete `plurnk:///manifest.json` (+ self-exclusion + the 2 bespoke queries) · **C** worker-scope on the unified FIND (storage scope-threading + Run.ts authority-addressing + ownership=path-prefix). SPEC `§worker-scheme`. **Inc-1 (additive worker-scope SQL variants `crud_find_worker_entry`/`crud_insert_worker_entry`/`ops_read_channel` run-form + a `manifest.scope==='run'` branch in EntryOps/EntryCrud; workspace path byte-identical → 0 test risk) is invariant — needed regardless.** Storage template = Known.ts → EntryOps/EntryCrud/EntryFind.
>
...
>
> **LOCKED (owner, 2026-06-22):** no worker_id (owner in pathname) · scope='run' (isolation partition) · worker name = authority · cross-worker write DENY (SEND/irc owns inter-run comms; safe-to-loosen) · JSON-array materialization (jsonpath, not tabular) · FIND result = filtered manifest.
>
...
> - **FOLD CORRECTED (I had it wrong):** `OPEN/FOLD` toggle `log_entries.expanded` — the worker curating its own LOG view ("what I am looking at", §open-fold / §machine-processes-worker-is-its-log). NOT entry content-vs-metadata; NOT the catalog.
> - **Scratch is REAL** (owner confirmed: a large *evolving private workspace*, not just notes → the log does NOT subsume it). worker:// storage earns its scope; the "a worker is its log and nothing beside" doctrine is RETIRED. SPEC excision started (§155 done); **§157 (`{§machine-processes-one-filesystem}`) + §161 (`{§machine-processes-worker-is-its-log}`) anchored + §171/§173 remain — excise WITH the worker-scope build, guard + tests verified.**
...
**The frame (settled 2026-06-18; supersedes the retired `worker://`-private Deferred note).** Entries live at one of two scopes — the axis is *which identity discriminator*:
- **workspace** — the shared world ("one filesystem"), `workspace_id`-discriminated: the work + the seeded docs.
- **run** — per-worker scratch, `worker_id`-discriminated. Perspective-private, NOT ACL-private: `FIND(worker://other/**)` reaches it cross-worker, so privacy was never on the table and we don't pretend it is.

`agent` scope is **KILLED** — proven vestigial (no scheme defaults to it, the engine never writes it; the lone `agent` literal was the type union). The workspace is the top durable unit; no above-workspace concept exists or is coming (owner, 2026-06-18).

**`Manifest(run) = workspace-scope ∪ this-worker's-worker-scope`.** The manifest is the worker's *perspective*, not one identical catalog for all workers. worker-scope entries are catalogued only in their owning worker's manifest (full token/line/existence data — what a worker needs to manage its own scratch); other workers FIND them but don't see them catalogued. Invariant restated: *complete for the worker's perspective*, not complete-globally. Excluding a whole scope is **structural**, never the forbidden relevance-curation — "deliver, don't decide" stands. Mind [[the model/db-context lock]]: this is all model-context (the manifest the worker sees), orthogonal to database storage.

**worker:// becomes a real storage scheme** (today a fake scheme that can't hold entries — the inconsistency this fixes). `worker://<workerId>/<path>`, workerId as authority. spawn → fresh scratch; **fork → COPIES the scratch** (run state, rides the copied log, then diverges — `fork = everything-in-common-but-name`).

**Docs catalog — the self-documenting surface.** `plurnk://docs/<tag>.md`, **`docs` as AUTHORITY** (double slash; 96cae8f wrongly shipped `plurnk:///docs/`). **SESSION-scoped** (workspaces register different scheme/exec/plugin sets → the doc set is per-workspace), seeded from that workspace's capability set, always in the manifest, READ on demand. `# Plurnk Service Tools` stays terse (live examples — push); deep docs are pull. Every scheme/exec/plugin (and the engine) registers a doc → the model's compose-competence grows with the catalog at ~zero hot-path cost (generalizes the #note12 doc-materialization).
...
> **RECONCILED 2026-06-25 (firsthand): C SHIPPED, E still OPEN.** **C** landed (NOT a worker_id discriminator — the owner's later LOCKED design won: owner-in-pathname). `entries.scope CHECK IN ('workspace','run')`, `entries_worker_identity` unique index `(workspace_id, scheme, pathname) WHERE scope='run'`, `src/schemes/Run.ts` (authority=worker name; write self-only, cross-worker write 403; cross-worker READ ok). §worker-scheme anchors all green (spawn/fork/irc/collect/cap/terminate). **E** is the genuine remainder: the per-scheme catalog/FIND foist does NOT union the building worker's own `scope='run'` rows (workspace queries filter `scope='workspace'`, auto-excluding them) — so a worker's own scratch isn't catalogued in ITS manifest. Wire the union (owner opts back in on the building worker's own-scope read path). **F** rides E (the per-worker-manifest SPEC bit); the worker:// storage anchors themselves exist.
- [x] **E. Per-run manifest — FIND core DONE (0932c03).** Run-scope FIND + catalog perspective: a worker FINDs its own scratch (`worker:///**`) and a sister's only by name (`worker://name/**`); isolation structural. Additive variants (Inc-1, workspace path byte-identical): `find_worker_entry_candidates`, `engine_list_worker_entries(+tags)`, `engine_worker_scratch_count`; EntryFind branches on `manifest.scope==='run'`; `catalogRowsFor` sources worker-scope by owner prefix; `toPath` renders `worker://owner/path`; `Run.find()` folds the owner. Turn-0 foists `FIND(worker:///**)` when the worker holds scratch. SPEC `{§worker-scheme-find-perspective}`. guard 182/182, intg 875/0.
  - **fork-copies-worker-scope — DONE (4731135).** `Fork.fork` now deep-copies the parent's `scope='run'` entries (new ids) with the owner remapped in the pathname (parent → branch) + channels (deep_hash preserved → pump skips re-derivation) + tags. A fork opens with the parent's scratch under its own name and diverges on its own edits; the parent's copy is untouched. SPEC `{§worker-scheme-fork-scratch}`. This ACTIVATES the turn-0 perspective foist (a fork now holds scratch at its first loop). The shared workspace world is still never copied — only the worker's private workspace.

**▸ RUN-SCOPE SCRATCH: COMPLETE (2026-06-25).** Storage (C) + FIND/perspective (E, 0932c03) + fork-inheritance (4731135) all shipped + tested. A worker writes/reads/FINDs its own scratch (cross-worker read by name), the catalog foists it, and forks inherit it. Remaining worker-scope residue lives in the epic above (F: per-worker-manifest SPEC prose; the `FIND(**)` ban; §worker-scheme storage anchors — all minor/doc).
- [x] **D. Docs catalog — service half landed.** `definition`=plurnk.md, a lean `schemes` directory section below `tools`, scheme docs materialize as pull `plurnk://docs/<scheme>.md` (per loop.run). Open residue: **workspace-scope one-time seeding** (docs re-materialize per loop.run today; seeding at workspace.create shifts initial entry/run state + the no-loop.run op.* tests — revisit with this epic); **schemes#25** (add `documentation` to `SchemeManifest` — the catalogue's `teach` is a duck-typed in-tree paragraph today; service-side NOT blocked: catalogue render → `example` + doc-link, in-tree schemes adopt `example`+`documentation`, verbose `teach` relocates into `documentation`); **requirements.md decision** — its 2 YOU MUSTs are verbatim in plurnk.md `## Imperatives`, decide if it's the operator's custom-rules slot (slimmed of dupes) or dropped.
- [x] **F. SPEC — DONE (87432fa).** `{workspace, run}` (agent-scope retired, glossary + §workspace-identity); worker:// storage anchored `{§worker-scheme-scratch}` (self-write/cross-read/403) + perspective `{§worker-scheme-find-perspective}` + fork-inherit `{§worker-scheme-fork-scratch}`; §161 "a worker is its log" reconciled (the doctrine is *no shadow of the world*, not *no private state* — scratch is owned state). `FIND(**)` ban = NON-ISSUE: it parses `kind:local` (a scoped file-scheme glob), not a toxic cross-scheme global — the per-scheme foist never emits bare `**`; no service guard needed (plurnk.md advertising is the grammar's, and it doesn't).
...
### ACTIVE EPIC — Repatriation (§actor-boundary-self-hosting): runtime work is a plurnk worker, not a privileged engine pathway

**The contract (SPEC §actor-boundary-self-hosting).** Runtime-initiated work (fs reconciliation, git auto-add) is an *ephemeral `plurnk` run firing ordinary ops*, seen by other workers through the environment door — not a privileged engine write. The engine keeps only the irreducible kernel (spawn, dispatch, packet assembly, the budget rails, the fs-watch); everything expressible as ops on workspace entries is a worker doing ops. Dogfooding is the architecture, not a test mode.
...
1. **env-delta materialization (the EMI).** Splits into two halves with DIFFERENT correct homes:
   ...
   - **Divergence *narration*** (`#logFsFictions`, `Engine.ts:1634`) is **already a plurnk-worker op** — it writes `origin=plurnk, source='file'` EDIT rows into the reserved plurnk worker; the model pulls them via `engine_pull_env_deltas` (the environment door). Built + tested. Done.
...
2. **git auto-add.** The ONE genuinely-outstanding *expressible* piece. NOT BUILT — `ls-files --others --exclude-standard` member-on-creation, surfaced as a plurnk-worker op. Gated on Forest part-B (`repo` overlay + verb rename).
...
**VERDICT (the Challenger answer to "is now a great time?"): the epic is ~complete and correctly architected — the spec prose, not the code, was behind.** Doc materialization → keystone (built, Phase 1 test). Divergence narration → plurnk-worker op (built). Disk ingestion → fs-watch kernel by nature (not a violation). Derivation pump → kernel. The only outstanding repatriation is **git auto-add**, and it is Forest-part-B-gated (unbuilt overlay). There is no safe standalone materialization-repatriation to "do now" — routing ingestion through a plurnk op is both semantically backwards (EDIT=egress) AND floods env-delta.

**Phasing.**
- **Phase 1 — seam-proof. DONE (d9a4491).** `[§actor-boundary-self-hosting]` is a real test (was `{todo}`): the materializing EDIT lands in the plurnk worker's log (origin=plurnk), absent from the model's, result reached via the env door. The contract is real + green.
...
- **Overlay verbs** `pick | view | hide | repo` (validated in `workspace_constraints.ts` `EFFECTS`); **`drop` = `workspace.unconstrain`** (deleting the row IS the un-declare — never a 5th effect). `{§membership-overlay-pick/view/hide}`.
- **Two flags** — `PLURNK_GIT_ALLOWED` (hard ceiling, un-re-enableable `=0`) gates `PLURNK_GIT_AUTO` (`=1` auto-declares `repo` at `project_root`). Workspace-level `git:false` (#232) tightens further (env AND workspace). `{§membership-git-flags}`.
...
- **Auto-add NEW files only**, logged (`origin=plurnk`) — never auto-stage edits, never auto-commit; the model/user own git history. Rides a plurnk worker distinct from the model's.
...
- **Config scope axis** — source precedence + ceiling-vs-default are settled (SPEC §operator-config); remaining is the {scope} axis — server-global vs per-workspace ceilings (none exist yet, e.g. one workspace capped at 100 turns, another at 50). The project `.env` is shared (plurnk prefix + project vars).
...
**Sequencing (owner): live/demo fixes are POST-EPIC.** Right before the epics, run ONE triaged live/demo sweep to surface real *architectural* gaps (those inform the epics); skip the fails that belong in a focused, collaborative workspace. Don't chase live/demo reds before that sweep — noise until then.
...
- [ ] **plurnk client owes** (both service-ready, client implements): `--run=<name>` (filed plurnk#6; `workspace.attach({workerName})` wired) + the `noProposals` no-review-channel UX (filed plurnk#24; server auto-reject in `noProposals.ts`).
...
- [~] **Run topology & awakening — substrate stays neutral** (CONSOLIDATED 2026-06; **CORE SHIPPED**). **Built:** EXEC `<T>` timeout (5d184f1) + `<T,P>` poll (20d233e); child-conclusion wakes a parked parent — the join (30bd086, `#onDrainExit`→`#wakeParkedWorker`); premature-terminate counts live child workers (9a6240e); 202 subtree-quiescence → soft `loop/quiesced` (9377b3d, reawakable, not terminal — also resolves the dead-park hang). Verbs resolved (200=done/202=yield, no new codes); children = streams uniformly. SPEC: {§worker-lifecycle-child-wake}, {§worker-lifecycle-quiesced}, §send-premature-terminate, §send-groundless-hibernate (READ+202 orphan-park refused 409; the bare voice-door park stays legal → quiesced), §exec-timeout/poll. **REMAINING (refinements, not blocking):** workspace-level GC sweep of quiesced 202s (precise reachability/cycle detection vs today's per-park conservative signal); parked loops must not count against the active-runs cap (§worker-scheme-cap); coalesce N wakes per park into one resume; the wire collapse-vs-distinct is decided (distinct: loop/quiesced≠loop/terminated). Merges with worker-scope #270 for worker:// storage. Models compose their own topologies (fork-join, supervision, pipeline, mesh) from primitives; the runtime never bakes in "a spawn implies an await." **Grounded in POSIX `wait`/SIGCHLD/`exit` + generators (yield/return) + structured concurrency (a scope completes only when its children do; cancel cascades)** — design the parent/child rules (error + cancel propagation) against the structured-concurrency literature. **Substrate SHIPPED: EXEC `<T>` timeout (504, 5d184f1) + `<T,P>` poll-wake (20d233e) — the *stream-half* of one "watch a uri, wake when it ends" primitive; the topology work is that same primitive over `worker://` children.** Shape:
...
- [ ] **Out-of-sqlite-space story.** Owner shape: a telemetry line firing ONLY above ~80% of the storage ceiling (`PLURNK_SQLITE_MAX_PAGE_COUNT`) — "persistent entry storage 80% full: KILL irrelevant logs/knowns/unknowns to free space" — targeting the model's OWN entries, never `file://` (the user's, git-tracked). No new crash: a write past 100% returns a CLEAN storage-full status (model recovers by KILLing); the manifest-too-large corner is already the budget grinder's 413 (the manifest is the un-foldable lifeline → overflow → clean 413 terminal). So the nudge prevents; 413 (packet) + a clean storage-full (disk) catch the ignored-nudge case. Build: the % threshold read, the telemetry kind, the wording.

---

## User Notes

Inbox triaged 2026-06-16 — pending items moved into the TODO (§ ACTIVE — User-Notes follow-ups), grouped by gate; verified-done notes cleared (disposition reported in-workspace). New raw notes land here; triage + clear when processed.

_(inbox drained 2026-06-18 — done notes cleared: sqlite-perf/size → sqlrite#7 + `PLURNK_SQLITE_*` knobs; doc-links → `plurnk:///docs/<tag>.md`; "Plurnk Service Tools" naming → already shipped. Design/feature notes moved to § Deferred / design: SEND[300], worker:// authority, tools placement, prompt-preview knob, out-of-space story.)_
...
**DB MODEL + backup/migration story (design, deferred — dogfood ships monolithic).** `~/.plurnk/plurnk.db` becomes the high-value, high-lock-in artifact (life's work, potentially huge). "Your state is a sqlite file" is a strong, on-ethos story — value-based lock-in, not captivity. OPEN DECISION, per-workspace files vs monolithic:
- **Native per-workspace** (`~/.plurnk/workspaces/plurnk_<name>.db`, one file/workspace): the file IS the workspace — hand someone `plurnk_rockford.db` and you've handed them the work (lean into file paradigms; the handoff story). COHERENT: a workspace is already "the world" (one filesystem + membership; workers are logs within it), so file boundary = workspace boundary. COST: daemon/engine assume ONE db handle → multi-DB routing (open/cache per workspace), `workspace.list` = dir-scan/registry, a global cost total sums across files, cross-workspace shared resources (`wiki://`) need a separate store.
- **Monolithic + export/import** (`plurnk export/import <workspace> → plurnk_<name>.db`): simple one-DB daemon, portability on demand. Pragmatic fallback.
- Rec: native per-workspace is the likely target (story + coherence); decide DELIBERATELY post-dogfood, NOT at the publish boundary (it touches the DB-access seam the whole engine sits on). Dogfood ships monolithic `~/.plurnk/plurnk.db`; monolith→per-workspace is a clean one-time migration (first exercise of the story below).
- BACKUP/MIGRATION best-practices to define+document: backup = copy the file(s), but WAL-SAFE — never raw-`cp` a live WAL'd sqlite mid-write; use the sqlite backup API / `VACUUM INTO` / checkpoint-then-copy for a consistent snapshot. Schema migrations already run idempotently on open. The monolith→per-workspace split is the reference migration.
...
We don't have it yet, and aren't shipping anything like it in v1. But what's our story for having a "global" resource? For example, plurnk-schemes-wiki that lets all workspaces have read-only wiki:///Argentina without needing to copy that resource to every workspace that exists? What's our story with that?
