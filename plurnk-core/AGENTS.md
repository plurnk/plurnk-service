...
- **Operational hygiene on what the model sees.** The model's view (packet.system/user) is its world — leaking automatic-resolution outcomes, resolution mechanisms, or security metadata into it is a bug. It sees *what happened* (status, log, body), never *how it was administratively resolved*. {§telemetry-no-error-scheme}
