...
export default class LoopDriver {
    ...
    constructor({ loopSignals, db, lifecycle, schemes, notices, strikes, acquireWorkspaceTurn, workspaceTurnStarting, runTurn }: {
        loopSignals: Map<number, AbortSignal>;
        db: Db;
    ...
    async runLoop({
        ...
        const ruleTimeout = (): Promise<LoopResult> => ruleTerminal(
            Results.failure(
                ...
                "loop-timeout",
                504,
                `The loop exhausted its active execution allowance after ${modelTurnCount} model turns.`,
                {},
                {
        ...
        try {
            while (true) {
                ...
                // WAIT may commit 202 immediately before daemon shutdown aborts this drain; reading
            ...
            cleanup("forceful", "loop_execution_failed");
            await this.#lifecycle.endExecution(loopId);
        }
    }
