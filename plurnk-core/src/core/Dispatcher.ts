...
export default class Dispatcher {
    ...
    async writeModelEntry({ verbatim, workerId, loopId, turnId, sequence, folded, origin = "model", reasoningEncrypted }: {
        ...
        // {§sealed-reasoning-carrier} (#482) — a provider's SEALED reasoning blobs ride the mirror
        // row's attrs VERBATIM (never decoded, never rendered into a packet — the packet renderer
        // reads no foreign attrs keys). The log/entry broadcast + readLog carry them to the client
        // seam per turn; agui projects REASONING_ENCRYPTED_VALUE from here.
        reasoningEncrypted?: ReadonlyArray<{ data: string; format: string | null }>;
    }): Promise<number> {
