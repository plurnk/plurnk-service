...
export default class CapsResolve {
    ...
    static async entryId(ctx: PlurnkSchemeContext, scheme: string, pathname: string, ownerId?: number): Promise<number | null> {
        return (await CapsResolve.entry(ctx, scheme, pathname, ownerId))?.entryId ?? null;
    }
}
