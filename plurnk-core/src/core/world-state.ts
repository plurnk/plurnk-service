...
export default class WorldState {
    static async check(db: Db): Promise<WorldStateViolation[]> {
        ...
        const dups = await (db.ws_dup_identities as PrepMethod).all<{ workspace_id: number; owner_id: number; scheme: string; pathname: string; n: number }>({});
        for (const d of dups) {
        ...
        const keys = await (db.ws_file_keys as PrepMethod).all<{ workspace_id: number; pathname: string; project_root: string | null }>({});
        for (const k of keys) {
        ...
        const orphanChannels = await (db.ws_orphan_channels as PrepMethod).get<{ n: number }>({});
        ...
        const orphanTags = await (db.ws_orphan_tags as PrepMethod).get<{ n: number }>({});
        if ((orphanTags?.n ?? 0) > 0) violations.push({ invariant: "orphan-freedom", detail: `${orphanTags?.n} entry_tags rows without a parent entry` });

        const alien = await (db.ws_alien_origin as PrepMethod).all<{ workspace_id: number; pathname: string; membership_origin: string }>({});
        ...
        const sigLeak = await (db.ws_sig_on_nonfile as PrepMethod).get<{ n: number }>({});
    ...
    static async entryCount(db: Db): Promise<number> {
        return (await (db.ws_entry_count as PrepMethod).get<{ n: number }>({}))?.n ?? 0;
    }
}
