...
import MembershipMaterialization from "./MembershipMaterialization.ts";
...
export interface SourceProjectionMetadata {
    mimetype: string;
    identity: string;
...
export default class GitMembership {
    ...
    static async #indexGitMembershipUnlocked(ctx: PlurnkSchemeContext): Promise<FsDivergence[]> {
        ...
        for (const pathname of resolution.members) {
            const divergence = await MembershipMaterialization.materializeMember(pathname, root, ctx, identities);
