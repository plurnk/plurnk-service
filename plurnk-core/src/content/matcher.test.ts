...
test("relation dialects (~full-text / &graph) never reach the content matcher", async () => {
    ...
    await assert.rejects(Matcher.matchAgainstContent({ dialect: "fts", raw: "~query" } as MatcherBody, "x", "text/markdown", stub), /content-only/);
