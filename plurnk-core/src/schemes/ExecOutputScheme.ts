...
const streamAddressSpace = (scheme: string): string =>
    `\`${scheme}:///<loop>/<turn>/<item>/EXEC\` addresses this runtime's result streams — your own without a qualifier, another worker's as \`${scheme}://<worker>/…\`. A tool's own ids are arguments: \`## EXEC0 (<runtime>/<tool>)\` with the id in the body.`;
