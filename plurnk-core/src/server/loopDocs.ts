...
//   1. the project AGENTS.md at worker://~/agents.md — Engine.runTurn foists
//      its READ at turn 0 ({§turn0-agents-stunt});
//   2. the exact current scheme and executable-tool reference set under
//      worker://~/skills/plurnk/ and worker://~/tools/ — discovered by the
...
export default class LoopDocs {
    ...
    static async materialize(engine: Engine, db: Db, workspaceId: number, workerId: number): Promise<void> {
        ...
        // worker-private worker://~/agents.md entry, foisted at turn 0.
        ...
        if (agentsContent !== null) desired.set("/agents.md", agentsContent);
