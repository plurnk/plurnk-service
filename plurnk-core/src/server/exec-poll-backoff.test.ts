// #521 — the exec-poll backoff curve: base·2^min(step, turns-1) seconds, doubling then holding.
import test from "node:test";
import assert from "node:assert/strict";
import { execPollBackoffMs } from "./exec-poll-backoff.ts";

test("[#521] the backoff doubles for `turns` steps then holds at the cap — never reverts to blind", () => {
...
test("[#521] the knobs retune base and ladder length", () => {
