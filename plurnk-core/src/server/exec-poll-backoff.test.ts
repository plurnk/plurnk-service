// {§exec-lifetime} — the daemon's observation doubles for the configured steps, then holds.
import test from "node:test";
import assert from "node:assert/strict";
import { execPollBackoffMs } from "./exec-poll-backoff.ts";

test("{§exec-lifetime}: the default backoff doubles for `turns` steps then holds at the cap", () => {
...
test("{§exec-lifetime}: the knobs retune base and ladder length", () => {
