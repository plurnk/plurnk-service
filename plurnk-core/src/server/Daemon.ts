...
export default class Daemon {
    ...
    constructor({
        ...
        this.#engine = new Engine({
            ...
            // worker:// KILL (terminate) — abort any run's in-flight work by id. One
    ...
    async runLoop(args: { workspaceId: number; workerId: number; prompt: string; maxTurns?: number; flags?: Partial<LoopFlags>; openPaths?: string[]; alias?: string; model?: string }): Promise<{ action: "injected_next_turn" | "enqueued_new_loop"; loopId: number; turnSeq?: number }> {
        ...
        // client op.* rows). The module resolves the model run via ensureModelWorker and passes it (or a
        ...
        if (target.origin === "client") throw new Error(`runLoop: run ${args.workerId} is a client run — loops run in model runs (§machine-processes); resolve one with ensureModelWorker(workspaceId)`);
    ...
    // §machine-processes — the workspace's model run (created on first use), distinct from the client
    ...
    // The log-read hook (#355) — a workspace's journal, the module's primary render input. The run is
    // ownership-verified against the workspace (a workspace reads only its own runs — the model run included,
    ...
    // The fork hook (#355) — branch a run's log into a new run in the same workspace (#228), sharing the
    // workspace's world (entries + overlay), copying nothing of it. The module resolves the default (the
    // workspace's model run) from its own connection state and passes the concrete workerId; the seam owns the
    // #366 — a fresh conversation run: AG-UI threads map to RUNS (§machine-processes — the workspace
    // is the workspace, the run is the conversation). ensureModelWorker is the stable DEFAULT door,
    ...
    async forkWorker(args: { workspaceId: number; workerId: number; name?: string }): Promise<{ workerId: number; workerName: string | null; parentWorkerId: number }> {
        ...
        if (name !== undefined) {
            ...
            const taken = await (this.#db.envelope_get_worker_by_name as PrepMethod).get<{ id: number }>({ workspace_id: workspaceId, name });
            if (taken !== undefined) throw new Error(`forkWorker: a run named "${name}" already exists — run names are immutable, pick another`);
        }
