...
| The **workspace** | The user's home + namespace — the persistent environment workers run WITHIN (project root, membership, entries; weeks of life), never the transient login session the industry word implies | Namespaces; home directories | `workspaces`, `project_root` |
...
| **Wake / cancel / kill / strike** | Signals | Signal delivery; cooperative cancellation | `WakeWorker*`, `CancelWorker` |
