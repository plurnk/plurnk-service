...
Workspace = the shared world (one filesystem + membership overlay). Worker = one actor and its history over that world. Loop = one queued-to-terminal unit of work within a worker; each loop contains turns, and each model turn ends with a TASK inventory that determines its workflow outcome. Workers fork and message each other — many clients, many workers, one workspace.
