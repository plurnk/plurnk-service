...
test("conversation: plurnk explains its own operations, and shows examples without running them", { timeout: 600_000 }, async (t) => {
    ...
    try {
        ...
        assert.deepEqual(result.notices.filter((message) => /advisory|turn_no_operations|nothing ran|must start its line|needs four backticks|shown, not run/i.test(message)), [], "the model's examples draw no parser complaint");
...
test("conversation: asked to show a deletion without doing it, plurnk shows it and deletes nothing", { timeout: 600_000 }, async (t) => {
    ...
    try {
        ...
        assert.deepEqual(result.notices.filter((message) => /advisory|turn_no_operations|nothing ran|must start its line|needs four backticks|shown, not run/i.test(message)), [], "showing the deletion draws no parser complaint");
    } finally { await cleanup(); }
});
