...
test("demo: complete a multi-source briefing under tight context pressure", async (t) => {
    ...
    try {
        const floor = await measureFloor({ signal: t.signal, label: "pressure", projectRoot: fixture.workspace, prompt: userPromptText });
        const gauge = Math.round(floor.weight * GAUGE_FACTOR);
        lifetime.defer(pinAliasInputCapacity({ inputCapacity: gauge, outputBudget: floor.outputBudget }));
        const s = await liveWorkspace({ name: `demo-budget-${crypto.randomUUID()}`, projectRoot: fixture.workspace });
        lifetime.defer(s.cleanup);
        const { finalStatus, turnIds, lastContent } = await liveLoop(s, 2, { prompt: userPromptText }, { signal: t.signal });
