...
test("live: model answers a JSON field question (jsonpath extraction)", async (t) => {
    ...
    try {
        ...
        const { finalStatus, lastContent } = await liveLoop(s, 2, { prompt: "What is the value of the `host` field in worker:///config.json?" }, { signal: t.signal });
        assert.equal(finalStatus, 200);
        assert.match(lastContent, /db\.internal/);
    } finally { await s.cleanup(); }
});

test("live: model lists every value from a JSON array (jsonpath wildcard)", async (t) => {
    ...
    try {
        ...
        const { finalStatus, lastContent } = await liveLoop(s, 2, { prompt: "List the names of every user in worker:///team.json." }, { signal: t.signal });
        assert.equal(finalStatus, 200);
...
test("live: model answers an HTML heading question (xpath extraction)", async (t) => {
    ...
    try {
        ...
        const { finalStatus, lastContent } = await liveLoop(s, 2, { prompt: "What does the h1 heading in worker:///page.html say?" }, { signal: t.signal });
        assert.equal(finalStatus, 200);
...
test("live: model picks the first item out of a JSON array (extract then pick first)", async (t) => {
    ...
    try {
        await seedEntry(s.db, s.workspaceId, { pathname: "/team.json", content: '{"users":[{"name":"Alice"},{"name":"Bob"}]}', mimetype: "application/json" });
        const { finalStatus, lastContent } = await liveLoop(s, 2, { prompt: "Who is the first user listed in worker:///team.json?" }, { signal: t.signal });
        assert.equal(finalStatus, 200);
        assert.match(lastContent, /Alice/);
    } finally { await s.cleanup(); }
});
