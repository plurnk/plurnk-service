// SPEC {§tools} — the registered executor catalogue is explicitly titled so its table defines
...
test("registered executable tools render as a closed, titled contract table; omitted when empty", () => {
    ...
    const withTools = PacketWire.renderSlot(userSections("| `[executor]` | `(target)` | body |\n| --- | --- | --- |\n| `[node]` | script | JavaScript |"), "user");
    assert.match(withTools, /\| `\[node\]` \| script \| JavaScript \|/, "the registered executable contract renders");
    assert.match(withTools, /## Registered Executable Tools/, "the heading defines the table as registered selectors");
