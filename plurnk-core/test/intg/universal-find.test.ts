...
const parseFind = (dsl: string): FindStatement => {
    const item = PlurnkParser.parse(`# PLAN1\n${dsl}`).items.find(
...
test("data schemes inherit standard FIND after their optional preparation hook", async () => {
    ...
    try {
        ...
        const result = await engine.dispatch({
            statement: parseFind("## FIND1 (prepared:///*.md)\n*forty-two*"),
            workspaceId,
            workerId,
            loopId,
            turnId,
...
test("exact matcher FIND invokes preparation, then returns flat match locations", async () => {
    ...
    try {
        ...
        const turnId = await insertTurn(db, loopId, 1, 102);
        const result = await engine.dispatch({
            statement: parseFind("## FIND1 (prepared:///fact.md)\n*forty-two*"),
            workspaceId,
            workerId,
            loopId,
            turnId,
...
test("exact URL FIND acquires live HTTP resources, reuses them, and rejects dead URLs", async () => {
    ...
    try {
        ...
        const surveyed = await engine.dispatch({
            statement: parseFind("## FIND1 (https://93.184.216.34/*)\n/spouse/"),
            workspaceId,
            workerId,
            loopId,
            turnId,
        ...
        const dead = await engine.dispatch({
            statement: parseFind(`## FIND1 (${deadUrl})`),
            workspaceId,
            workerId,
            loopId,
            turnId,
...
test("HTTP mutation responses cannot satisfy later READ or exact FIND acquisition", async () => {
    ...
    try {
        ...
        assert.equal((await dispatch(parseSend(`## SEND1 [200] (${readUrl})\nupdate`), 1)).status, 102);
        assert.equal((await dispatch(parseRead(`## READ1 (${readUrl})`), 2)).status, 200);
        ...
        assert.equal((await dispatch(parseSend(`## SEND1 [200] (${findUrl})\nupdate`), 3)).status, 102);
        const found = await dispatch(parseFind(`## FIND1 (${findUrl})\n/current GET/`), 4);
...
test("non-data schemes without FIND remain honestly unsupported", async () => {
    ...
    try {
        ...
        const turnId = await insertTurn(db, loopId, 1, 102);
        const result = await engine.dispatch({
            statement: parseFind("## FIND1 (control-only:///**)"),
            workspaceId,
            workerId,
            loopId,
            turnId,
