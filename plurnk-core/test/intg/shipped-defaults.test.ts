...
test("the template ships no double policy, no active model, ONLY service-owned knobs", async () => {
    ...
    // PLUGINS trust gate and automatic reasoning view). Sibling knobs (PROVIDERS/EXECS/SCHEMES/
    ...
    assert.equal(env.get("PLURNK_REASONING_VIEW_LINES"), "-1", "automatic reasoning feedback requests the complete body by default");
...
test("under the shipped policy wiring, the shipped policy renders in the packet exactly once", async () => {
    ...
    try {
        ...
        const provider = new Mock({ contextWindow: 100000, responses: [{ assistant: { content: "", reasoning: null, ops: [dispositionStmt("DONE", "done") as PlurnkStatement] } }] });
