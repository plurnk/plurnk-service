...
async function seedPromptWorker(db: Awaited<ReturnType<typeof openMigrated>>) {
    ...
    await engine.runTurn({
        provider: new Mock({ contextWindow: 100000, responses: [{ assistant: { content: "", reasoning: null, ops: [{ op: "NEXT", annotation: null, target: null, metadata: null, lineMarker: null, body: null, position: { line: 1, column: 1 } }] } }] }),
...
test("sister workers' turn-1 prompts are distinct owner-keyed rows at the same coordinate", async () => {
    ...
    try {
        ...
        const engine = new Engine({ db, schemes: new SchemeRegistry(), mimetypes: DEFAULT_MIMETYPES });
        const mkProvider = () => new Mock({ contextWindow: 100000, responses: [{ assistant: { content: "", reasoning: null, ops: [{ op: "NEXT", annotation: null, target: null, metadata: null, lineMarker: null, body: null, position: { line: 1, column: 1 } }] } }] });
