...
test("a child FAILING (499) also wakes the parent — any conclusion is a wake edge", async () => {
    ...
    const mock = new Mock({ contextWindow: 16384, responses: [
        makeMockResponse("### WORK_ (worker://doomed)\ntry the risky thing\n\n### SEND_ (WAIT) <-1>\nwaiting on doomed", 10),
...
test("wake propagates UP a grandchild chain (parent→child→grandchild)", async () => {
    ...
    const mock = new Mock({ contextWindow: 16384, responses: [
        ...
        makeMockResponse("### WORK_ (worker://grandchild)\ndo leaf work\n\n### SEND_ (WAIT) <-1>\nawaiting grandchild", 10), // child t1
...
test("a parent wakes across SEQUENTIAL children (multiple wakes)", async () => {
    ...
    const mock = new Mock({ contextWindow: 16384, responses: [
        makeMockResponse("### WORK_ (worker://w1)\nfirst job\n\n### SEND_ (WAIT) <-1>\nawaiting w1", 10), // parent t1
        makeMockResponse("### SEND_ (TERM)\nw1 done", 10),                                       // w1
        makeMockResponse("### WORK_ (worker://w2)\nsecond job\n\n### SEND_ (WAIT) <-1>\nawaiting w2", 10),// parent t2 (woken by w1)
        makeMockResponse("### SEND_ (TERM)\nw2 done", 10),                                       // w2
...
test("an irc (SEND worker://name) wakes a CONCLUDED sibling on that worker's durable model", async () => {
    ...
    await withDaemon(mock, async (db, daemon, addr) => {
        ...
        try {
            ...
            await rpcCall(ws, 5, "op.send", { recipient: `worker://${actor.name}`, body: "the entry code is 4815" });
...
test("spawn and fork carry the delegating loop's policy — an accepting parent's child EDITs without proposing", async () => {
    ...
    const mock = new Mock({ contextWindow: 16384, responses: [
        ...
        makeMockResponse("### EDIT_ (worker:///from-worker)\npayload\n\n### SEND_ (TERM)\nworker done", 10),
        // Fork turn 1: same shape.
        makeMockResponse("### EDIT_ (worker:///from-fork)\npayload\n\n### SEND_ (TERM)\nfork done", 10),
...
test("log-targeted KILL is recorded in the DB, dissolves after rendering once, and a failed one persists", async () => {
    ...
    const mock = new Mock({ contextWindow: 16384, responses: [
        makeMockResponse("### EDIT_ (worker:///note)\nsome content worth folding\n\n### SEND_ (NEXT)\nwrote", 10),
        ...
        makeMockResponse("### KILL_ (log:///1/2/1) <1,-1>\n\n### KILL_ (log:///9/9/9) <1,-1>\n\n### SEND_ (NEXT)\ncurated", 10),
