...
test("{§rail-truth-engine-verdict} the engine stamps local attachment and its own verdict", async () => {
    ...
    try {
        ...
        const reject = Object.assign(new Mock({ contextWindow: 100000, responses: [{ assistant: { content: "```DONE\ndone\n```", reasoning: null } }] }), { constrainsOutput: true }) as unknown as Provider;
...
test("raw rail positions map only from content, and debug evidence is stamped withheld", async () => {
    ...
    try {
        ...
        assert.deepEqual(firstRail?.position, { type: "content-offset", line: 2, column: 1 });
