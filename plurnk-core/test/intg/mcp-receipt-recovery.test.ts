...
for (const body of [null, '{"query":"fixture"}']) {
    test(`MCP ${body === null ? "bodyless" : "one-line JSON"} invocation and output retain distinct readable extents`, { timeout: 30_000 }, async (t) => {
        ...
        const invocation = observed.find((row) => String(row.path).endsWith("/fixture"));
        assert.ok(invocation);
        assert.equal(invocation.lines, body === null ? undefined : 1);
        const automatic = observed.find((row) => row.source === invocation.path && String(row.path).endsWith("/READ"));
        assert.ok(automatic);
        assert.deepEqual(automatic.range, { unit: "line", total: 40, requested: [1, 16], returned: [1, 16] });
        const failed = (await packet(3)).find((row) => row.target === invocation.path && row.status === 416);
        assert.ok(failed);
        ...
        const explicit = (await packet(4)).find((row) => row.target === invocation.stream && row.origin === undefined);
