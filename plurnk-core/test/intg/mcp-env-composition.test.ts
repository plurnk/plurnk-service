...
test("{§mcp-launch-environment} a real MCP probe and server use workspace env, not worker env, and pick up changes only on restart", { timeout: 30000 }, async () => {
    ...
    const boot = () => {
        ...
        instance.registerModule(McpModule.init({ env: { PLURNK_MCP_CONNECT_TIMEOUT: "10000", PLURNK_MCP_REQUEST_TIMEOUT: "10000", PLURNK_MCP_RETRY_FLOOR_MS: "250", PLURNK_MCP_RETRY_CEILING_MS: "5000", PLURNK_MCP_ENABLED: "[]", REF_VALUE: "operator" } }));
        return instance;
    };
    let daemon = boot();
    await daemon.start();
...
test("{§mcp-launch-environment} HTTP authorization references use workspace defaults while retaining symbolic definitions", async (t) => {
    ...
    daemon.registerModule(McpModule.init({ env: { PLURNK_MCP_CONNECT_TIMEOUT: "10000", PLURNK_MCP_REQUEST_TIMEOUT: "10000", PLURNK_MCP_RETRY_FLOOR_MS: "250", PLURNK_MCP_RETRY_CEILING_MS: "5000", PLURNK_MCP_ENABLED: "[]", ENV_AUTH: "operator-token" } }));
    await daemon.start();
    try {
