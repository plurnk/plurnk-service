...
test("two independent root Workers acquire the same https address as distinct private resources", async () => {
    ...
    try {
        ...
        for (const [index, root] of roots.entries()) {
            const read = await dispatch(root, parseOne("### READ_ (https://example.org/feed)") as ReadStatement, 1);
        ...
        for (const [index, root] of roots.entries()) {
            const found = await dispatch(root, parseOne("### FIND_ (https://example.org/**)") as FindStatement, 2);
            assert.equal(found.status, 200);
            assert.equal(found.matchingPathCount, 1, "a root's FIND sees exactly its own acquisition");
            const reread = await dispatch(root, parseOne("### READ_ (https://example.org/feed)") as ReadStatement, 3);
