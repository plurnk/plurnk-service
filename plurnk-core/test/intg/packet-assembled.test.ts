...
import { copyStmt, editStmt, readStmt, findStmt, regex, dispositionStmt, urlPath, noteStmt } from "./_dsl.ts";
...
test("{§worker-auto-name}: assembled receipts and child inventory identify anonymous WORK and FORK", async () => {
    ...
    try {
        ...
        const provider = new Mock({
            ...
            responses: [
                { assistant: { content: "", reasoning: null, ops: [...branches, noteStmt("Continue.")] } },
                { assistant: { content: "", reasoning: null, ops: [noteStmt("Continue.")] } },
            ],
        });
    ...
    try {
        ...
        const provider = new Mock({
            ...
            responses: [
                ...
                { assistant: { content: "", reasoning: null, ops: [dispositionStmt("DONE")] } },
            ],
        });
...
test("assembled packet: the turn-0 catalog foist renders its entries into the log", async () => {
    ...
    try {
        ...
        assert.deepEqual(
            ...
            ["NOTE", "FIND", "FIND", "FIND", "FIND", "FIND", "FIND", "FIND", "READ", "READ"],
            "turn 0 exposes its reasoning note, executed surveys, and reasoning and program READs",
        );
        assert.deepEqual(
...
test("assembled packet: the bodyless Worker reference catalog succeeds when no references are materialized", async () => {
    ...
    try {
        ...
        const docs = rows.find((row) => row.op === "FIND" && row.scheme === "worker" && row.hostname === null && row.pathname === "/_plurnk/plurnk/*.md");
