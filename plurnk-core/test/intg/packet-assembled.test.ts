    ...
    try {
        ...
        const provider = new Mock({
            ...
            responses: [
                ...
                { assistant: { content: "", reasoning: null, ops: [dispositionStmt("completed")] } },
            ],
        });
...
test("assembled packet: the turn-0 catalog foist renders its entries into the log", async () => {
    ...
    try {
        ...
        assert.deepEqual(
            ...
            ["FIND", "FIND", "FIND", "FIND", "FIND", "FIND", "FIND", "READ", "READ", "READ", "TASK"],
            "turn 0 exposes the executed surveys, the reasoning/program/prompt READs and TASK",
        );
        assert.deepEqual(
...
test("{§retrieval-packet-metadata}: exact matcher FIND shows flat surgical coordinates with one compact extent", async () => {
    ...
    try {
        ...
        const provider = new Mock({
            contextWindow: 100000,
            responses: [
                { assistant: { content: "", reasoning: null, ops: [matchedFind, dispositionStmt("in_progress")] } },
                { assistant: { content: "", reasoning: null, ops: [dispositionStmt("completed")] } },
            ],
        });
        await engine.runTurn({ provider, workspaceId, workerId, loopId, messages: [] });
