...
test("the wire is monotone in volatility — byte-stable system; user: log → status clump → recap", async () => {
    ...
    try {
        ...
        assert.deepEqual(slot("user"), ["log", "child-streams", "child-workers", "errors", "git", "budget", "prompt", "requirements"], "user slot: log → status clump → recap (§prompt-auto-read: the prompts paths-list closes the clump)");
