...
test("#713: notes have durable sources, normal log curation, and do not block completion", async () => {
    ...
    try {
        ...
        const provider = new Mock({ contextWindow: 100_000, responses: [
            ...
            { assistant: { content: frame("DONE", "Finished."), reasoning: frame("NOTE", "Both checks passed.") } },
        ] });
        ...
        assert.deepEqual(notes.map((row) => row.resource), ["note:///1/2/1", "note:///1/2/2"], "each ordinary receipt exposes its recoverable source");
        assert.ok(notes.every((row) => row.origin === undefined || row.origin === "model"));
        const firstSource = await engine.look({ ...context, statement: statement(frame("READ (note:///1/2/1) <1,-1>", null)) });
        assert.equal(firstSource.status, 200);
        assert.equal(firstSource.content, "The network is not the cause.");
        const secondSource = await engine.look({ ...context, statement: statement(frame("READ (note:///1/2/2) <1,-1>", null)) });
        ...
        const sourceAgain = await engine.look({ ...context, statement: statement(frame("READ (note:///1/2/1) <1,-1>", null)) });
        assert.equal(sourceAgain.content, "The network is not the cause.", "curating the log does not delete the source note");
        const rawReasoning = await engine.look({ ...context, statement: statement(frame("READ (reasoning:///1/2) <1,-1>", null)) });
        ...
        const found = await engine.dispatch({ ...context, turnId: final.turnId, sequence: 40, origin: "model",
            statement: statement(frame("FIND (note:///1/2/*) /parser/", null)),
        });
        assert.equal(found.status, 200);
        assert.deepEqual(resourcePaths(found as FindResult), ["note:///1/2/2"], "source search still works after log curation");
        const fork = await Fork.fork(db, workerId, "branch");
        const inherited = await engine.look({ ...context, workerId: fork, statement: statement(frame("READ (note:///1/2/1) <1,-1>", null)) });
        ...
        for (const op of ["EDIT", "KILL"]) {
            const refused = await engine.dispatch({ ...context, turnId: final.turnId, sequence: op === "EDIT" ? 50 : 51, origin: "model",
                statement: statement(frame(`${op} (note:///1/2/1)`, op === "EDIT" ? "Changed" : null)),
            });
            assert.equal(refused.status, 403, `${op} cannot change immutable note evidence`);
        }
        const missing = await engine.look({ ...context, statement: statement(frame("READ (note:///1/2/99)", null)) });
        assert.equal(missing.status, 404, "an absent note is not an invented empty source");
    } finally { await db.close(); }
});

test("#713: empty WAIT falls through and DONE plus a retrieval observes before concluding", async () => {
    const db = await openMigrated();
    try {
        ...
        const provider = new Mock({ contextWindow: 100_000, responses: [
            ...
            { assistant: { content: [frame("DONE", "The answer."), frame("READ (ops:///1/1) <1,-1>", null)].join("\n\n"), reasoning: null } },
            { assistant: { content: frame("DONE", null), reasoning: null } },
        ] });
...
test("{§reasoning-notes}: a rejected emission cannot commit its reasoning NOTE", async () => {
    ...
    try {
        ...
        const provider = new Mock({ contextWindow: 100_000, responses: [
            { assistant: { content: [frame("DONE", "Yes."), frame("FAIL", "No.")].join("\n\n"), reasoning: rejectedReasoning } },
            { assistant: { content: frame("DONE", "Finished."), reasoning: acceptedReasoning } },
        ] });
        ...
        const reasoning = await engine.look({ ...context, statement: statement(frame("READ (reasoning:///1/2) <1,-1>", null)) });
