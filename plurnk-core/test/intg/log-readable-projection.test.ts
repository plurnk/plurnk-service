...
test("{§log-readable-projection}: READ and COPY omit deliberate trims in original coordinates", async (t) => {
    ...
    assert.equal((await dispatch("### EDIT0 (worker:///source.txt)\none\ntwo\nsecret\nfour\nfive")).status, 201);
    assert.equal((await dispatch("### READ0 (worker:///source.txt) <1,-1>")).status, 200);
    const target = "log:///1/1/2/READ";
    assert.equal((await dispatch(`### KILL0 (${target}) <3>`)).status, 200);
    const read = await dispatch(`### READ0 (${target}) <2,4>`);
    assert.equal(read.status, 200);
    assert.equal(read.content, "two\nfour");
    assert.deepEqual(read.lineOrdinals, [2, 4]);
    assert.equal((await dispatch(`### COPY0 (log:///1/1/2) <${read.lineAnchors[1]}> (worker:///anchored.txt)`)).status, 201);
    assert.equal((await dispatch("### READ0 (worker:///anchored.txt) <1,-1>")).content, "four", "canonical leaf aliases share anchor identity");
    assert.equal((await dispatch(`### COPY0 (${target}) <2,4> (worker:///copy.txt)`)).status, 201);
    assert.equal((await dispatch("### READ0 (worker:///copy.txt) <1,-1>")).content, "two\nfour");
    ...
    assert.equal((await dispatch(`### READ0 (${target}) <3>`)).status, 204);
    assert.equal((await dispatch(`### COPY0 (${target}) (log:///1/1/2/READ)`)).status, 400);
    assert.equal((await dispatch(`### MOVE0 (${target}) (worker:///moved.txt)`)).status, 400);
...
test("{§log-readable-projection}: FIND prices retained bodies consistently in rows and folders", async (t) => {
    const { db, ids, dispatch } = await runtime(t);
    const content = "apple\nsecret\npear";
    await dispatch(`### EDIT0 (worker:///source.txt)\n${content}`);
    await dispatch("### READ0 (worker:///source.txt) <1,-1>");
    const log = new Log();
    ...
    assert.equal((await dispatch("### KILL0 (log:///1/1/2/READ) <2>")).status, 200);
...
test("{§log-readable-projection}: a byte-view receipt is readable text, not a second binary resource", async (t) => {
    ...
    const sourceRead = await dispatch("### READ0 (worker:///data.bin) <1,-1>");
    ...
    assert.equal((await dispatch(`### COPY0 (${target}) (worker:///hex.txt)`)).status, 201);
    const copied = await dispatch("### READ0 (worker:///hex.txt) <1,-1>");
    assert.equal(copied.content, "00\n02\n03");
    const repeatedSource = await dispatch("### READ0 (worker:///data.bin) <1,-1>");
...
test("{§log-readable-projection}: initially suppressed programs remain readable until explicitly trimmed, and forks preserve both facts", async (t) => {
    ...
    assert.equal((await dispatch(`### READ0 (${target}) <1,-1>`)).content, content);
    await t.test("COPY of a textual program into plain text remains verbatim", async () => {
        const copied = await dispatch(`### COPY0 (${target}) (worker:///program.txt)`);
    ...
    assert.equal(forkRead.content, "## PLAN0\n### SEND0 (NEXT)\ncontinue");
    ...
    assert.equal((await dispatch(`### KILL0 (${target}) <1,-1>`)).status, 200);
    assert.equal((await dispatch(`### READ0 (${target}) <1,-1>`)).status, 204);
    ...
    assert.equal((await dispatch(`### READ0 (${target})`)).status, 404);
    assert.equal((await dispatch(`### COPY0 (${target}) (worker:///retired.txt)`)).status, 404);
});
...
test("{§log-readable-projection}: trimming invalidates search and a racing derivation cannot reattach old content", { timeout: 10_000 }, async (t) => {
    ...
    await dispatch("### EDIT0 (worker:///source.txt)\napple\nsecret\npear");
    await dispatch("### READ0 (worker:///source.txt) <1,-1>");
    await dispatch("### KILL0 (worker:///source.txt)");
    const ctx = makeSchemeCtx({ db, ...ids, mimetypes: DEFAULT_MIMETYPES });
    ...
    assert.equal((await dispatch("### FIND0 (log:///1/1/2/READ)\n~secret")).status, 204);
    const retained = await dispatch("### FIND0 (log:///1/1/2/READ)\n~pear");
    assert.equal(retained.status, 200);
    assert.equal(JSON.parse(retained.content)[0].region.startLine, 3);
    assert.equal((await dispatch("### KILL0 (log:///1/1/2/READ) <3>")).status, 200);
    await SearchIndex.maintain(ctx);
    assert.equal((await dispatch("### FIND0 (log:///1/1/2/READ)\n~pear")).status, 204, "later curation also invalidates an already attached artifact");
    const read = await dispatch("### READ0 (log:///1/1/2/READ) <1,-1>");
