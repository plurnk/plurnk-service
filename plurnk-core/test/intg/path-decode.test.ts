...
const editStmt = (target: UrlPath, body: string): ResolvedEditStatement => ({ metadata: null, op: "EDIT", annotation: null, delimiter: "", target, lineMarker: null, body, position: { line: 1, column: 1 } });
const readStmt = (target: UrlPath): ReadStatement => ({ metadata: null, op: "READ", annotation: null, delimiter: "", target, lineMarker: null, body: null, position: { line: 1, column: 1 } });
...
test("{§path-parentheses} a percent-encoded pathname alias resolves to literal parentheses", async () => {
    ...
    try {
        ...
        const r = await lookThroughScheme("worker", null, readStmt(enc("doc%28v1%29.md")), ctx);
        ...
        const literal = await lookThroughScheme("worker", null, readStmt(enc("doc(v1).md")), ctx);
