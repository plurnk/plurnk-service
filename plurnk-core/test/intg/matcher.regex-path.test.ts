...
test("regex target selects entries whose pathname matches the regex", async () => {
    ...
    try {
        ...
        const r = await new Worker().find(findStmt(regexPath("\\.test\\.ts$")), makeSchemeCtx({ db, workspaceId }));
        assert.equal(r.status, 200);
        assert.deepEqual([...new Set(r.results.map((f) => f.path))], ["worker:///src/auth.test.ts"]);
    } finally { db.close(); }
});
...
test("regex target honors flags — case-insensitive pathname match", async () => {
    ...
    try {
        ...
        const r = await new Worker().find(findStmt(regexPath("readme", "i")), makeSchemeCtx({ db, workspaceId }));
        assert.equal(r.status, 200);
        assert.deepEqual([...new Set(r.results.map((f) => f.path))].sort(), ["worker:///README.md", "worker:///src/readme-helper.ts"]);
    } finally { db.close(); }
});
...
test("malformed regex target is a 400, not a crash", async () => {
    ...
    try {
        ...
        const r = await new Worker().find(findStmt(regexPath("(unclosed")), makeSchemeCtx({ db, workspaceId }));
