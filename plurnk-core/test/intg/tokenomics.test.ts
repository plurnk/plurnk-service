...
test("{§output-allowance-notice} tokensResponseMax is the output floor less the reasoning subset", async () => {
    ...
    try {
        ...
        const loopId = await insertLoop(db, workerId, 1, "p");
        const engine = new Engine({ db, schemes: new SchemeRegistry(), mimetypes: DEFAULT_MIMETYPES });
        const provider = new Mock({ contextWindow: 130816, responses: [{ assistant: { content: "", reasoning: null, ops: [sendStmt(null)] } }] });
...
test("an unrecoverable curation overflow preserves exact pressure evidence in its terminal Problem", async () => {
    ...
    try {
        ...
        const provider = new Mock({ contextWindow: 11, responses: [{ assistant: { content: "", reasoning: null, ops: [sendStmt(null)] } }] });
        const result = await engine.runTurn({ provider, workspaceId, workerId, loopId, messages: [{ role: "system", content: "SD" }, { role: "user", content: "go" }] });
...
test("{§tokenomics-calibrated-readout} three reported prompt counts convert the ceiling without changing cost units", async () => {
    ...
    try {
        ...
        const provider = new Mock({ contextWindow: 100000, responses: [
            ...
            { assistant: { content: "", reasoning: null, ops: [noteStmt("Continue.")] }, usage: charged },
            { assistant: { content: "", reasoning: null, ops: [noteStmt("Continue.")] }, usage: charged },
            { assistant: { content: "", reasoning: null, ops: [sendStmt(null)] }, usage: charged },
        ] });
