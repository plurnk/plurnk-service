...
test("schemes: table is STRICT and WITHOUT ROWID", async () => {
    ...
    try {
        const row = await db.test_sp_table_sql.get<{ sql: string }>({ name: "schemes" });
...
test("schemes: model_visible + volatile CHECK 0/1", async () => {
    ...
    try {
        await assert.rejects(
            () => db.test_schemes_insert_full.run({
...
test("schemes: default_scope enum", async () => {
    const db = await openMigrated();
    try {
        await db.test_schemes_insert_full.run({ name: "b", model_visible: 1, category: "c", default_scope: "workspace", default_channel: "body", writable_by: "[]", volatile: 0 });
        await assert.rejects(
            () => db.test_schemes_insert_full.run({ name: "c", model_visible: 1, category: "c", default_scope: "worker", default_channel: "body", writable_by: "[]", volatile: 0 }),
            /CHECK constraint failed/,
        );
    } finally { await db.close(); }
});
...
test("schemes: writable_by JSON validity", async () => {
    const db = await openMigrated();
    try {
        await db.test_schemes_insert_full.run({ name: "s1", model_visible: 1, category: "c", default_scope: "workspace", default_channel: "body", writable_by: JSON.stringify(["model", "client", "plugin"]), volatile: 0 });
        await db.test_schemes_insert_full.run({ name: "s2", model_visible: 1, category: "c", default_scope: "workspace", default_channel: "body", writable_by: "[]", volatile: 0 });
        await assert.rejects(
            () => db.test_schemes_insert_full.run({ name: "s3", model_visible: 1, category: "c", default_scope: "workspace", default_channel: "body", writable_by: "{broken", volatile: 0 }),
            /CHECK constraint failed/,
        );
    } finally { await db.close(); }
});
...
test("schemes: channel_orientations nullable", async () => {
    const db = await openMigrated();
    try {
        await db.test_schemes_insert_with_orient.run({ name: "s_null", model_visible: 1, category: "c", default_scope: "workspace", default_channel: "body", writable_by: "[]", volatile: 0, orient: null });
        await db.test_schemes_insert_with_orient.run({ name: "sse", model_visible: 1, category: "streaming", default_scope: "workspace", default_channel: "event", writable_by: "[]", volatile: 1, orient: JSON.stringify({ event: "tail", data: "tail" }) });
        await assert.rejects(
            () => db.test_schemes_insert_with_orient.run({ name: "s_bad", model_visible: 1, category: "c", default_scope: "workspace", default_channel: "body", writable_by: "[]", volatile: 0, orient: "{broken" }),
            /CHECK constraint failed/,
        );
    } finally { await db.close(); }
});
...
test("providers: table is STRICT", async () => {
    const db = await openMigrated();
    try {
        const row = await db.test_sp_table_sql.get<{ sql: string }>({ name: "providers" });
        assert.match(row?.sql ?? "", /STRICT/);
    } finally { await db.close(); }
});
...
test("providers: insert minimal — defaults populate", async () => {
    const db = await openMigrated();
    try {
        await db.test_providers_insert.run({ provider: "anthropic", family: "claude", model: "claude-opus-4-7", contextSize: 200000, currency: "USD" });
        const row = await db.test_providers_get.get<{ version: number; provider: string; family: string; model: string; contextSize: number; currency: string; created_at: string }>({ model: "claude-opus-4-7" });
...
test("providers: length CHECK on provider, family, model", async () => {
    ...
    try {
        for (const empty of ["provider", "family", "model"] as const) {
            ...
            cols[empty] = "";
            await assert.rejects(
                () => db.test_providers_insert.run({ ...cols, contextSize: 200000, currency: "USD" }),
                /CHECK constraint failed/,
            );
        }
    } finally { await db.close(); }
...
test("providers: contextSize CHECK >= 1", async () => {
    ...
    try {
        for (const bad of [0, -1, -100]) {
            await assert.rejects(
                () => db.test_providers_insert.run({ provider: "anthropic", family: "claude", model: "claude-opus-4-7", contextSize: bad, currency: "USD" }),
                /CHECK constraint failed/,
            );
        }
    } finally { await db.close(); }
...
test("providers: currency CHECK length = 3", async () => {
    ...
    try {
        for (const bad of ["US", "USDX", "", "AB", "ABCD"]) {
            await assert.rejects(
                () => db.test_providers_insert.run({ provider: "anthropic", family: "claude", model: "claude-opus-4-7", contextSize: 200000, currency: bad }),
                /CHECK constraint failed/,
            );
        }
    } finally { await db.close(); }
...
test("providers: multiple rows for the same (provider, family, model) allowed", async () => {
    ...
    try {
        await db.test_providers_insert.run({ provider: "anthropic", family: "claude", model: "claude-opus-4-7", contextSize: 200000, currency: "USD" });
        await db.test_providers_insert.run({ provider: "anthropic", family: "claude", model: "claude-opus-4-7", contextSize: 200000, currency: "USD" });
        const count = (await db.test_providers_count.get<{ n: number }>({ model: "claude-opus-4-7" }))?.n;
...
test("providers: id auto-assigns", async () => {
    const db = await openMigrated();
    try {
        await db.test_providers_insert.run({ provider: "a", family: "b", model: "c", contextSize: 100, currency: "USD" });
        await db.test_providers_insert.run({ provider: "a", family: "b", model: "d", contextSize: 100, currency: "USD" });
        const rows = await db.test_providers_list_ids.all<{ id: number }>();
...
test("providers: index providers_created_at exists", async () => {
    const db = await openMigrated();
    try {
        const row = await db.test_providers_index.get<{ name: string }>();
