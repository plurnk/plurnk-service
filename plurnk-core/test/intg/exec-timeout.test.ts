...
test("{§exec-lifetime} a bounded lifetime kills the spawn at its deadline and closes the stream 504", { timeout: 150_000 }, async () => {
    ...
    await withDaemon(mock, async (_db, _daemon, addr) => {
        ...
        try {
            ...
            assert.ok(elapsed >= 55_000 && elapsed < 100_000, `the spawn was reaped near 60s, not run to 120s; elapsed=${elapsed}ms`);
            await flush();
            ...
            assert.ok(
                ...
                `the exec stream closed with the exact lifetime Problem; got ${JSON.stringify(closes.map((c) => c.result))}`,
            );
        } finally { ws.close(); }
    });
});
