...
test("{§op-look}: seam look resolves a READ in one closed, rowless observation segment", async () => {
    await withDaemon(null, async (db, daemon, addr) => {
        ...
        try {
            ...
            const edit = parseDsl("## PLAN_\n\n### EDIT_ (worker:///notes/target.md)\nthe looked-up body")[1];
            await daemon.dispatchAsClient({ workspaceId, workerId, functionalityWorkerId: workerId, statement: edit });
            ...
            const read = parseDsl("## PLAN_\n\n### READ_ (worker:///notes/target.md)")[1];
            const result = await daemon.look({ workspaceId, workerId, functionalityWorkerId: workerId, statement: read });
