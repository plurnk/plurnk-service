...
test("PLURNK_SERVICE_PACKET_INJECT: operator file rides as a system section after policy", async () => {
    ...
    try {
        ...
        assert.equal(sysOrder.indexOf("inject"), sysOrder.indexOf("system-policy") + 1, "inject follows privileged policy");
    } finally {
