...
test("{§http-json-presentation}: READ, FIND, COPY and previews share formatted JSON coordinates", async (t) => {
    ...
    try {
        const preview = await dispatch(`### READ0 (${target})`);
        assert.equal(preview.status, 200);
        assert.equal(preview.content, formatted.split("\n").slice(0, 16).join("\n"));
        const scoped = await dispatch(`### READ0 (${target}) <20,22>`);
        ...
        assert.equal((await dispatch(`### COPY0 (${target}) <20,22> (worker:///selection.json)`)).status, 201);
        const copied = await dispatch("### READ0 (worker:///selection.json) <1,-1>");
        assert.equal(copied.content, scoped.content);
        assert.equal((await dispatch(`### EDIT0 (worker:///literal.json)\n${source}`)).status, 201);
        assert.equal((await dispatch("### READ0 (worker:///literal.json) <1,-1>")).content, source, "literal JSON resource coordinates and bytes are untouched");
    } finally {
