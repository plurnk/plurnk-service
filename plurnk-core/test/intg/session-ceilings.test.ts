...
test("[§operator-config-workspace-git] workspace settings.git:false denies git membership for the workspace (env AND workspace)", async () => {
    ...
    try {
        ...
        const deniedMember = await (db.crud_find_workspace_entry as PrepMethod).get<{ id: number }>({ workspace_id: denied.workspaceId, owner_id: await Owner.commonsId(db, denied.workspaceId), scheme: null, pathname: "/tracked.md" });
        ...
        const allowedMember = await (db.crud_find_workspace_entry as PrepMethod).get<{ id: number }>({ workspace_id: allowed.workspaceId, owner_id: await Owner.commonsId(db, allowed.workspaceId), scheme: null, pathname: "/tracked.md" });
