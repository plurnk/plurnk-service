...
const execStmt = (runtime: string): ExecStatement => ({ metadata: null, op: "EXEC", annotation: null, delimiter: "", signal: runtime, target: null, lineMarker: null, body: "echo hi", position: { line: 1, column: 1 } });
...
test("{§operator-config-workspace-execs} one effective set filters executable-tool resources", async () => {
    ...
    try {
        ...
        assert.ok(before.some((d) => d.pathname === "/_plurnk/plurnk/node.md"), "baseline: node's tool resource renders");
        ...
        assert.ok(!after.some((d) => d.pathname === "/_plurnk/plurnk/node.md"), "node disabled by workspace policy → no resource materialized");
        assert.ok(after.some((d) => d.pathname === "/_plurnk/plurnk/sh.md"), "other tool resources survive the filter");
