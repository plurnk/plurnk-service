...
test("{§operator-config-workspace-execs} one effective set filters executable-tool resources", async () => {
    ...
    try {
        ...
        assert.ok(before.some((d) => d.pathname === "/_plurnk/skills/plurnk/node.md"), "baseline: node's tool resource renders");
        ...
        assert.ok(!after.some((d) => d.pathname === "/_plurnk/skills/plurnk/node.md"), "node disabled by workspace policy → no resource materialized");
        assert.ok(after.some((d) => d.pathname === "/_plurnk/skills/plurnk/sh.md"), "other tool resources survive the filter");
