...
test("ordinary operation evidence redacts credential slots once before every durable projection", async () => {
    ...
    try {
        ...
        const read = parseClientStatement(
            "<<READ(credential-probe://primary-user:primary-password@example.test/value?ticket=query-visible#body{Authorization: Bearer primary-header-secret}{X-Api-Key: secondary-header-secret})::READ",
            "READ",
        );
