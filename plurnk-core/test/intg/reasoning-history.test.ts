...
import { AiSdkProvider, Mock, type MockResponse } from "@plurnk/plurnk-providers";
...
test("{§notifications-reasoning-event}: provider SSE reaches standard AG-UI before the reasoning copy exists", async () => {
    ...
    try {
        ...
        try {
            await ready.promise;
            assert.deepEqual(await db.test_reasoning_history.all({ worker_id: workerId }), [], "the live signal cannot originate from a not-yet-created copy");
        ...
        const rows = await db.test_reasoning_history.all<History>({ worker_id: workerId });
        assert.equal(content(rows[0]!.original), "Visible now.");
    } finally {
        await db.close();
    }
});
