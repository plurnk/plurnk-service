...
test("generate carries the workspace/loop/turn coordinate, using loop sequence rather than database id", async () => {
    ...
    try {
        ...
        const mock = new CoordMock({ contextWindow: viableWindow(), responses: [makeMockResponse("### SEND_ (TERM)\ndone", 50)] });
...
test("generate carries primaryWorkerId — a spawned child's differs from its Worker-Id; the root's equals it", async () => {
    ...
    try {
        ...
        const m1 = new CoordMock({ contextWindow: viableWindow(), responses: [makeMockResponse("### SEND_ (TERM)\ndone", 50)] });
        ...
        const m2 = new CoordMock({ contextWindow: viableWindow(), responses: [makeMockResponse("### SEND_ (TERM)\ndone", 50)] });
