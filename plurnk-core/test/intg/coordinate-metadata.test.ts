...
test("generate carries primaryWorkerId — a spawned child's differs from its Worker-Id; the root's equals it", async () => {
    ...
    try {
        ...
        const m1 = new CoordMock({ contextWindow: viableWindow(), responses: [makeMockResponse("```SEND\ndone\n```\n```TASK\n[{\"content\":\"Task completed.\",\"status\":\"completed\"}]\n```", 50)] });
        ...
        const m2 = new CoordMock({ contextWindow: viableWindow(), responses: [makeMockResponse("```SEND\ndone\n```\n```TASK\n[{\"content\":\"Task completed.\",\"status\":\"completed\"}]\n```", 50)] });
