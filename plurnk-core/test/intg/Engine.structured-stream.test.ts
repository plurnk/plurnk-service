...
const response = (op: DispositionStatement["op"]) => ({
    assistant: {
        content: "",
        reasoning: null,
        ops: [dispositionStmt(op, op === "DONE" ? "done" : "continue")],
    },
});
...
const setup = async (
    mimetype: string,
    content: string,
    responses: ConstructorParameters<typeof Mock>[0]["responses"] = [response("NEXT"), response("DONE")],
) => {
    ...
    const fixture = await setup("application/json", "", [
        response("NEXT"),
        {
            assistant: {
                content: "",
                reasoning: null,
                ops: [kill, dispositionStmt("NEXT", "failure observed")],
            },
        },
        response("DONE"),
    ]);
