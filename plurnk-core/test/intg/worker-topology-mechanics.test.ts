...
test("N self-forks of one parent get UNIQUE, individually-addressable names", async () => {
    ...
    try {
        ...
        const f1 = await Fork.fork(db, parent, undefined);
        const f2 = await Fork.fork(db, parent, undefined);
        const f3 = await Fork.fork(db, parent, undefined);
        const nameOf = async (id: number): Promise<string | undefined> => (await db.worker_get.get<{ name: string }>({ id }))?.name;
...
test("{§worker-auto-name}: concurrent unnamed forks atomically claim distinct short names", async () => {
    ...
    try {
        ...
        const parent = await insertWorker(db, workspaceId, null, "worker");
        const forks = await Promise.all(Array.from({ length: 8 }, () => Fork.fork(db, parent, undefined)));
        const names = await Promise.all(forks.map(async (id) =>
            (await db.worker_get.get<{ name: string }>({ id }))?.name ?? ""));
...
test("an automatic fork of a maximum-length parent remains a mintable worker name", async () => {
    ...
    try {
        ...
        const fork = await Fork.fork(db, parent, undefined);
        const name = (await db.worker_get.get<{ name: string }>({ id: fork }))?.name ?? "";
...
test("a fork inherits the parent's loops as HISTORY (clamped terminal), never frozen-live", async () => {
    ...
    try {
        ...
        const fork = await Fork.fork(db, parent, undefined);
        const loops = await db.test_fork_loops.all<{ status: number }>({ worker_id: fork });
