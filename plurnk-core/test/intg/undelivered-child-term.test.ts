...
async function raceScenario(db: Awaited<ReturnType<typeof openMigrated>>) {
    const sessionId = await insertSession(db, `race-${crypto.randomUUID()}`);
    ...
    const child = await insertRun(db, sessionId, parent, "worker-x");
    ...
    return { sessionId, parent, parentLoop, parentTurn, engine };
}

test("[§send-undelivered-child-term] a bare SEND[202] over a just-concluded child CONTINUES — never the ∅-collapse", async () => {
    const db = await openMigrated();
    try {
        const { sessionId, parent, parentLoop, parentTurn, engine } = await raceScenario(db);
        const r = await engine.dispatch({ statement: sendStmt(202, null, "Waiting for worker-x."), sessionId, runId: parent, loopId: parentLoop, turnId: parentTurn, sequence: 1, origin: "model" });
...
test("[§send-undelivered-child-term] a SEND[200] over a just-concluded child is refused with the steer", async () => {
    ...
    try {
        const { sessionId, parent, parentLoop, parentTurn, engine } = await raceScenario(db);
        const r = await engine.dispatch({ statement: sendStmt(200, null, "done"), sessionId, runId: parent, loopId: parentLoop, turnId: parentTurn, sequence: 1, origin: "model" });
