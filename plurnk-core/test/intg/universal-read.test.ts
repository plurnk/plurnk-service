...
test("inherited READ uses the scheme's canonical pathname and owner", async () => {
    ...
    try {
        ...
        const result = await engine.dispatch({
            statement: parseRead("<<READ(resolved-entry:///alias_%28v1%29.txt#body)<1,-1>::READ"),
            workspaceId,
            workerId,
            loopId,
            turnId,
...
test("preparation capabilities bind to the already-resolved canonical owner", async () => {
    ...
    try {
        ...
        const turnId = await insertTurn(db, loopId, 1, 102);
        const result = await engine.dispatch({
            statement: parseRead("<<READ(owner-bound-preparation:///alias.txt)<1,-1>::READ"),
            workspaceId,
            workerId,
            loopId,
            turnId,
...
test("scope-blind preparation durably composes producer status with cold and warm core projection", async () => {
    ...
    try {
        ...
        for (const sequence of [1, 2]) {
            const result = await engine.dispatch({
                statement: parseRead("<<READ(archive:///aliases/latest)<17,18>::READ"),
                workspaceId,
                workerId,
                loopId,
                turnId,
...
test("core selects one channel before applying its independent durable producer result", async () => {
    ...
    try {
        ...
        const body = await dispatch(
            parseRead("<<READ(independent-channel:///item)<1,-1>::READ"),
            1,
        );
        ...
        for (const sequence of [2, 3]) {
            const html = await dispatch(
                parseRead("<<READ(independent-channel:///item#html)<1,-1>::READ"),
                sequence,
            );
...
test("exact FIND shares representation preparation but retains universal query semantics", async () => {
    ...
    try {
        ...
        const turnId = await insertTurn(db, loopId, 1, 102);
        const result = await engine.dispatch({
            statement: parseFind("<<FIND(archive:///aliases/latest)::FIND"),
            workspaceId,
            workerId,
            loopId,
            turnId,
...
test("cold finite HTTP READ acquires before applying the exact text scope", async () => {
    ...
    try {
        ...
        const result = await engine.dispatch({
            statement: parseRead("<<READ(https://93.184.216.34/document.txt)<17,18>::READ"),
            workspaceId,
            workerId,
            loopId,
            turnId,
