...
test("engine.inject: writes prompt entry at plurnk://prompt/<run>/<loop>/<next-turn>", async () => {
    ...
    try {
        ...
        const sessionId = await insertSession(db, "engine-inject");
        const runId = await insertRun(db, sessionId);
        const loopId = await insertLoop(db, runId, 1, "initial prompt");
        ...
        const result = await engine.inject(runId, "follow-up");
        ...
        const entry = await (db.test_get_entry_by_path as PrepMethod).get<{ id: number }>({
            session_id: sessionId, scheme: "plurnk", pathname: `/prompt/${runId}/1/2`,
        });
...
test("engine.inject: last-wins — two injects targeting the same slot collapse", async () => {
    ...
    try {
        const engine = new Engine({ db, schemes: new SchemeRegistry() });
        const sessionId = await insertSession(db, "engine-inject-lastwins");
        const runId = await insertRun(db, sessionId);
        const loopId = await insertLoop(db, runId, 1, "initial");
        await insertTurn(db, loopId, 1, 102);

        const r1 = await engine.inject(runId, "first follow-up");
        const r2 = await engine.inject(runId, "second follow-up");
...
test("engine.inject: returns null when no loop is currently active (status=102)", async () => {
    ...
    try {
        const engine = new Engine({ db, schemes: new SchemeRegistry() });
        const sessionId = await insertSession(db, "engine-inject-no-active");
        const runId = await insertRun(db, sessionId);
        // No loops at all in this run.
        const result = await engine.inject(runId, "orphan prompt");
        ...
        const closedLoop = await insertLoop(db, runId, 1, "done");
        await (db.test_set_loop_status as PrepMethod).run({ id: closedLoop, status: 200 });
        const result2 = await engine.inject(runId, "still orphan");
...
test("engine.inject: per-turn foist reads latest prompt body into packet.user.prompt", async () => {
    ...
    try {
        const engine = new Engine({ db, schemes: new SchemeRegistry() });
        const sessionId = await insertSession(db, "engine-inject-foist");
        const runId = await insertRun(db, sessionId);
        const loopId = await insertLoop(db, runId, 1, "initial");
        await insertTurn(db, loopId, 1, 102);

        await engine.inject(runId, "the new prompt the model should see");
