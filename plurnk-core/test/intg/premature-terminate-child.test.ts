...
test("[§send-premature-terminate] a retrievals-ONLY refusal states the continuation, not a remedy menu (owner wording)", async () => {
    ...
    try {
        ...
        assert.match(refused!.rx, /Termination attempted despite this turn's retrieval operations\. Their results are now in the log for review and reaction\./, "the steer speaks packet-time (#384, owner wording): the results are PRESENT where this is read — never a completion signal ('Results retrieved.' read as done) and never a future tense ('next turn' read as wait-more)");
...
test("[§log-row-self-explains] a FAILED op row carries its failure message on its META LINE — the record states its why, folded or open", async () => {
    ...
    try {
        ...
        assert.match(metaLine!, /"error":"Termination attempted despite this turn's retrieval operations\. Their results are now in the log for review and reaction\."/, "the steer rides the META LINE — visible in every packet, never folded away");
