...
const setup = async () => {
    ...
    engine.setExecutors(new ExecutorRegistry(new Map([
        ["flag-tool", { executor: flagExecutor, namespaceOwner: { kind: "module", name: "flag-tool fixture" }, glyph: "🧪", invocation: { body: { role: "fixture input", required: true }, target: { role: "fixture resource", required: false, kind: "resource" } }, documentation: "", available: true, detail: undefined }],
        ["literal-tool", { executor: flagExecutor, namespaceOwner: { kind: "module", name: "literal-tool fixture" }, glyph: "🧪", invocation: { body: { role: "fixture input", required: false }, target: { role: "tool identifier", required: true, kind: "literal" } }, documentation: "", available: true, detail: undefined }],
    ])));
