...
const findStmt = (target: UrlPath, body: MatcherBody): FindStatement => ({
    op: "FIND", annotation: null, suffix: "", signal: null, target, lineMarker: null, body,
...
const seed = async () => {
    ...
    for (const [pathname, content] of FILES) {
        await EntryCrud.writeEntry(`${pathname}`, { channels: { body: { content, mimetype: "text/typescript" } } }, ctx, "file");
    }
