...
test("a client op.* never enters the model's packet — the client writes to its own run (#194)", async () => {
    ...
    await withDaemon(mock, async (db, _daemon, addr) => {
        ...
        try {
            ...
            assert.ok(modelWorker !== undefined, "the model loop has a run");
...
test("[§machine-processes-model-run-readable] a connection reads the model run by id — loop.run returns modelWorkerId, log.read targets it, ownership-gated", async () => {
    const mock = new Mock({ contextWindow: 8192, responses: [makeMockResponse("<<SEND[200]:done:SEND", 50)] });
    await withDaemon(mock, async (db, _daemon, addr) => {
        const ws = await connect(addr);
        try {
            ...
            assert.equal(typeof modelWorkerId, "number", "loop.run returns the model run's id");
            assert.notEqual(modelWorkerId, clientWorkerId, "the model run is distinct from the connection's client run");
            const byLoop = await (db.test_get_worker_id_by_loop as PrepMethod).get<{ worker_id: number }>({ loop_id: loopId });
            assert.equal(byLoop!.worker_id, modelWorkerId, "the returned modelWorkerId is the run the loop actually ran in");
            ...
            // log.read({ workerId }) → the MODEL run's log: readable by id (unaddressable before #214),
            ...
            // Ownership: a run in a DIFFERENT workspace is refused from this connection.
            ...
            assert.ok(denied.error, "reading a run outside the workspace is refused");
            assert.match(denied.error!.message, /not in this workspace/, "the refusal names the ownership violation");
        } finally { ws.close(); }
    });
});

test("[§machine-processes-run-origin] workspace.workers tags each run with its actor — the model run is found by origin, not name", async () => {
    const mock = new Mock({ contextWindow: 8192, responses: [makeMockResponse("<<SEND[200]:done:SEND", 50)] });
    await withDaemon(mock, async (_db, _daemon, addr) => {
        ...
        try {
            ...
            assert.deepEqual(listed.workers.filter((r) => r.origin === "model").map((r) => r.id), [modelWorkerId], "exactly one model run, found by origin");
        } finally { ws.close(); }
    });
});
