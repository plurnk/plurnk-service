...
class DocStub {
    static manifest = {
        ...
        example: "## READ1 (docstub:///x)", documentation: "# docstub\nFuller reference content.",
    };
}
...
test("{§schemes-directory}: teach() pushes an example while docs() carries the pull reference", async () => {
    ...
    assert.match(teaching, /## READ1 \(docstub:\/\/\/x\)/, "the scheme's canonical example is its bare heading (no bullet, no redundant scheme prefix — the example self-documents)");
