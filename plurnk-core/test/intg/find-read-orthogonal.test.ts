...
const findStmt = (target: ParsedPath | null, matcher: MatcherBody | null = null, lineMarker: FindStatement["lineMarker"] = null): FindStatement => ({
    metadata: null,
    op: "FIND", aside: null, target,
    lineMarker, matcher, body: null,  position: { line: 1, column: 1 },
});
...
const seed = async (db: Db, workspaceId: number, workerId: number, mimetypes: Mimetypes, path: string, content: string): Promise<void> => {
    await new Worker().edit(
        { metadata: null, op: "EDIT", aside: null, target: urlPath("worker", path), lineMarker: null, matcher: null, body: content, position: { line: 1, column: 1 } },
