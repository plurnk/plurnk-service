...
test("#283: a scoped READ of a materialized https entry's html channel returns exactly the window", async () => {
    ...
    try {
        ...
        const acquired = await dispatch(parseRead(`### READ_ (https://${HOST}/scoped)`));
        assert.equal(acquired.status, 200, "materialization read succeeds");

        const scoped = await dispatch(parseRead(`### READ_ (https://${HOST}/scoped#html) <3,16>`));
...
test("#283: a scoped READ of a materialized https entry's body channel returns exactly the window", async () => {
    ...
    try {
        ...
        const acquired = await dispatch(parseRead(`### READ_ (https://${HOST}/scoped-body)`));
        assert.equal(acquired.status, 200, "materialization read succeeds");

        const scoped = await dispatch(parseRead(`### READ_ (https://${HOST}/scoped-body) <3,16>`));
...
test("#283: a scoped READ of a project file still returns exactly the window", async () => {
    ...
    try {
        ...
        const seeded = await engine.dispatch({
            statement: {
                metadata: null,
                op: "EDIT", annotation: null, delimiter: "EDIT",
        ...
        await dispatch(parseRead(`### READ_ (worker:///scope.md) <3,16>`));
        const result = await readContent(db, ids, sequence);
        assert.equal(
            result.content,
...
test("#287: matcher FIND locations name the channel they address", async () => {
    ...
    try {
        ...
        const acquired = await dispatch("### READ_ (https://93.184.216.34/channel-facts)");
        assert.equal(acquired.status, 200, "materialization read succeeds");

        await dispatch("### FIND_ (https://93.184.216.34/channel-facts)\n/v[0-9.]+/i");
        ...
        await dispatch("### FIND_ (https://93.184.216.34/channel-facts#html)\n/v[0-9.]+/i");
