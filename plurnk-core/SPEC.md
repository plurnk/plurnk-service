...
- **The Budget renders as visual mermaid.** {§budget-mermaid} By default (`PLURNK_SERVICE_BUDGET_MERMAID`; set `off` to A/B against the tabular baseline, #440's before/after), the Budget section keeps its ceiling/usage/free headline (weighability) and REPLACES the Turns/Heaviest tables with three budget-scaled mermaid diagrams — turn-composition `treemap-beta` (turn boxes + `system + context` + `free` compose the whole ceiling), heaviest-items `xychart-beta` (ranked bars against the full-ceiling y-axis; the space above is headroom), and a used-vs-free `pie` gauge (which doubles as a training exemplar for the model's own user-facing SENDs). ALL scaled to the CEILING, not the items, so salience tracks pressure: `free` dominates when calm, the turns/bars fill toward urgent as the ceiling nears — so unlike the tables a mermaid budget is NEVER <50%-truncated (the calm low-usage view is the point). `free`/`used`/`system+context` resolve post-assembly like the headline figures. Syntax validated to render on GitHub (plurnk-plurnkdown/demo/budget-mermaid.md). #440
