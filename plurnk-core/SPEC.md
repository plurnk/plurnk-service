...
- **Budget telemetry is state, not instruction.** The same categories of measurement render regardless of occupancy. Plurnk does not hide telemetry below an arbitrary threshold, label rows as FOLD targets, or prescribe which history the model should curate. The model receives accurate state and owns its context decisions. {§tokenomics-neutral-telemetry}
...
- **The hard overflow is a recovery turn first.** {§grinder-hard-413-recovery} When the packet remains over the policy ceiling but is still within the provider's physical window, it is sent once with exact `usage`, `ceiling`, and `deficit` measurements. The recovery turn physically admits only `PLAN`, `FOLD`, `KILL`, and `SEND`; every other authored op resolves 409 without executing. A next fitting turn clears the recovery state, and a later independent overflow can earn a new recovery. A recovery turn that concludes is a legitimate 200. The boundary is exactly 100% of the policy budget; provider decode capacity is reserved separately.
...
**What the model sees.** The overflow is an exact RFC 9457 Problem on an `op='error'` log row. Its stable type/title state the broken contract; `detail` and numeric extensions report Token Usage, Token Ceiling, and the positive deficit. A hard recovery occurrence also carries the enforced `allowedOperations`. It is minted before the rebuild, so its derived `log:///<coord>` pointer surfaces in the errors section (§operation-results) on that turn. The budget remains neutral state. The strike counter stays engine-internal. {§grinder-overflow-error-row}
