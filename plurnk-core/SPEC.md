...
- §provider-surface-calculate-cost `provider.calculateCharge(usage)` (or frozen `calculateCost`) — derives explicitly free, estimated, or unknown evidence only when the response has no provider-settled charge. It never settles an estimate into `costUsd`.
- §provider-surface-reconcile-accounting `provider.reconcileAccounting(scope, signal)` — optional provider-owned settlement for a durable correlated loop scope. Core supplies opaque identity and observations, persists settled or pending truth, and never interprets vendor billing protocols.
...
The first exhaustion in a consecutive sequence closes that unadmitted turn as a continue and opens exactly one ordinary recovery turn. Its packet contains the latest rejected response as the exceptional born-OPEN model item under {§model-entry-log-curation} and one transient `invalid_emission` Notice whose complete message is: `Your previous response contained an unrecoverable syntax error. No operations were performed. Try again.` No parser diagnostic, attempt count, or rail state becomes model-facing. The recovery turn has its own honestly stored packet and its configured private same-packet attempts. Admission clears the recovery state; exhausting the informed turn terminates instead of opening another.
...
- §tokenomics-provider-usage **Provider usage is transport and cost evidence, not curation state.** Every issued provider operation has durable pre-I/O identity and closes as response or error. Response attempts preserve validated `{§provider-cost}` evidence beside provider-reported prompt, completion, reasoning, and cached quantities; response-less errors preserve unknown monetary evidence. Attempt rows own this evidence and the baseline materializes their ordered turn aggregate. `costUsd` is settled money and is `null` if any included cost is estimated or unknown. A provider-settled correlated scope total supersedes—but is never allocated into or added to—its attempt sum for loop, worker, and workspace rollups. `projectedCostUsd` remains the separately named attempt-derived projection and may include estimates. Exact decimal evidence remains authoritative over aggregate floating-point projections. Reasoning and completion are outputs the model cannot FOLD, so they never alter the model-facing Budget ledger.
...
**Schema.** The version-1 baseline stores loop accounting scopes and provider
call attempts beneath turns; their state constraints make open, response,
error, pending-settlement, and settled evidence distinct.
...
| Artifacts | `requiem.md` carries testimony and nullable settled/projected totals. `requiem.json` is durably materialized before each call and preserves scope/call identity, state, messages, responses, usage, and monetary evidence. |
...
When Git is admitted for the workspace, `## Git Status` reports only the current
branch, upstream ahead/behind counts, and staged/unstaged/untracked totals. It
never repeats an unbounded path list on every packet. Per-path state belongs to
