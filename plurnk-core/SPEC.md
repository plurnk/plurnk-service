...
- §packet-token-accounting Every row reports its real weight so the packet self-reconciles against the budget: `tokensMetadata` is the rendered metadata line (always present — no row reads as free), `tokensBody` is the body's weight whenever a body exists (the cost of an OPEN on a folded row, the cost in the packet when open), and `tokensTotal` is the row's weight in the packet right now (metadata plus body only when rendered). Fold frees `tokensBody`; KILL frees `tokensTotal`. A FIND's nonzero `itemsTokenTotal` weighs the complete matched set; a nonzero `returnedItemsTokenTotal` appears only when the returned page has a different weight. These are curation weights, not dollars. The invariants bind regardless of shape ({§packet}): addressability (`path`/`target`/`#channel`/coordinate-prefixed bodies), weighability (per-item `tokens`), honesty (every 4xx/5xx row and the exact body/display state). {§jsonplurnk} {§packet-jsonplurnk-exception}
