...
| §db-space-reclamation The daemon keeps its own file healthy | At start, before any drain, a database not in `auto_vacuum=INCREMENTAL` is converted once (set the mode, one `VACUUM`) and the journal says so with page counts before and after. Every retention pass ends with `PRAGMA incremental_vacuum`, stepped to completion, so freed pages return to the OS and the pass reports `reclaimedPages`. No operator step is involved. The WAL stays bounded by SQLite's automatic checkpoint (#764). |
