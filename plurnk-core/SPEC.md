...
- §overflow-turn-script **Recovery is one ordinary admitted `_plurnk` program.** Its PLAN body is `Automatically FOLD log bodies newly active at token-budget overflow.`, followed by every causal whole-body FOLD and terminal `SEND0 [102]` with body `Next: YOU MUST FOLD or KILL ALL superseded, stale, or irrelevant log items before continuing.` The final sentence requires comprehensive curation before work continues without restricting the successor to a curation-only program. Its exact `turnOps` is born FOLDED; successful FOLD rows follow {§fold-open-meta-operations} and therefore remain durable but packet-suppressed. Every recovery row carries `_plurnk` and `overflow`; no model call, synthetic receipt, or parallel explanation exists.
