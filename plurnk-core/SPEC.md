...
**Embedding has an optional size posture.** {§semantic-max-embed-size} `PLURNK_SERVICE_MAX_EMBED_SIZE` is an operator-set maximum UTF-8 body byte size eligible for vectors; `0` is unlimited and is the shipped default. Exhaustive embedding therefore remains the normal posture. When a nonzero ceiling rejects an oversized entry, it remains a workspace member with unchanged READ/edit behavior, full graph extraction, and full FTS indexing; only its vector projection is rejected. The setting is folded into the deep derivation signature, so changing it honestly re-derives affected entries. Each constrained warm pass emits a warning with the complete rejected-path list and structured `{pathname, actualBytes, maxBytes}` rows—never a silent skip or a misleading 100%.
