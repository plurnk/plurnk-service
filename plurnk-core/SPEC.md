...
- §packet-image-parts A READ of an image member ({§mimetype-image}) carries `image: { mimetype, width, height, bytes }`
  from the member's projection facts ({§mimetype-projection-facts}); its open row weighs the picture at
  `ceil(width × height / 750)` tokens (`tokensImage`, inside `tokensActive`), and the packet lists it as an
  attachment. On a route whose provider declares image input ({§provider-image-input}) the wire form carries
  the user slot as parts, the text then one native image per attachment read from the scheme's bytes at
  request time; every other route receives the text alone. An attachment follows visibility exactly: a
  folded or killed row sends nothing. The weight is an estimate; the provider's reported usage corrects the
  readout as it does for text.
