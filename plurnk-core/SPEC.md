...
| Resource | `reasoning:///<loop>/<turn>/<call>` is a worker-owned, independently editable text/plain copy; the final coordinate is the producing inference call's sequence. Ordinary entry READ, FIND, EDIT, KILL, anchors, search, persistence, and snapshot inheritance apply. |
| Delivery | The next packet observes the latest model turn's reasoning through actual `_plurnk` READ dispatch under {§reasoning-initial-read}. The receipt is an ordinary `log:///.../READ`, with the resource as its target and normal anchors and range metadata. There is no actionless reasoning log event or editable-log exception. |
| Curation | EDIT changes the source, not earlier READ receipts. A subsequent READ observes current text. Scoped log KILL suppresses receipt lines; whole log KILL retires the receipt. Neither affects the source. Resource KILL has ordinary deletion semantics. |
