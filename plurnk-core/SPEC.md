...
- §packet-attachment-parts A successful READ of an attachable resource carries projection facts with its
  ...
  ({§provider-input-modalities}), the READ contributes one native file part read from the scheme's bytes or
  its ordinary stored binary channel ({§binary-parity}) at
