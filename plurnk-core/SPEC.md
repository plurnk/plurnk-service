...
§operator-config-only-home **The cascading environment is the only home for a choice.** The platform is defined by three surfaces, and each owns one kind of fact: `plurnk.md` owns the language (a value whose change would make a different language), turn 0 owns the orientation (what the model is shown and taught), and the packages' `.env.defaults` own every choice (a value whose change would still be plurnk, behaving differently). Code holds mechanism only: a constant in code is legitimate only when it derives from one of those surfaces or from an external standard. Because the floor is always assembled ({§operator-config-env-defaults}), a value written beside a read can only disagree with the panel — `PLURNK_SERVICE_MAX_TURNS` once had two such values in code that disagreed with each other. `scripts/env-surface-policy.mjs` enforces this in `root:lint` over shipped source: every knob read is declared, live or as an optional knob, and a computed name is covered by one declared example of its family; a read never carries its own value; a constant named `DEFAULT_*` is by its own name a default living in code; one reader touches the process environment; a live declaration is consumed somewhere; a retired key is declared nowhere; one package owns a key. Its allowance file is a ratchet — new debt is refused, and so is a paid debt left on the books — so the debt only shrinks, and done is an empty allowance (#771). A daemon is one trust domain: a different security situation is a different daemon with a different panel, never a second vocabulary inside one.
