...
§wait-obligation-matrix **Runtime resolution.** Apply this table after earlier operations have executed. The durable TASK retains its authored inventory even when the actual outcome differs. A live obligation is a worker-owned unresolved child or non-detached stream, read as one row from the `worker_obligations` view ({§worker-obligations}). Unobserved results include this turn's retrievals, mutations and failures, and undelivered stream or child conclusions. Scope `<timeout[,poll]>` follows {§worker-wait-timing} only for a waiting intent.
