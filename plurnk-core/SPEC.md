...
- **The window is a partition, never a fraction.** The provider owns the complete envelope. `provider.contextWindow` is the effective total window: the lower of an operator `PLURNK_PROVIDERS_CONTEXT_WINDOW` cap and detected/cataloged physics, or the explicit declaration when no natural window is knowable. Percentage reasoning and completion reserves derive from that effective window; absolute pins remain absolute. Core derives `promptBudget = provider.contextWindow − provider.reasoningReserve − provider.completionReserve − PLURNK_SERVICE_SAFETY`, and passes the combined reserves as the per-call generation envelope. A null window or reserve is genuinely unknown and yields no cap; contradictory absolute reserves fail hard. All provider knobs are alias-scopable. Local llama-server aliases must keep their reasoning reserve, reasoning budget, and serving-box `--reasoning-budget` aligned because the server does not honor a per-request reasoning cap. {§tokenomics-window-partition} {§tokenomics-window-unpollable-deliberate}
