...
- **Schema-version stamp.** {§db-schema-version-stamp} Every plurnk DB carries SqlRite's `PRAGMA user_version` (current: `4`). Versioned `MIGRATE` blocks are the sole schema-evolution history; the latest migration version is therefore also the cross-repo drift stamp. Any change to the schema's *shape* — tables, columns, identity keys — adds the next migration in the same commit. External consumers (bench's digest) read the stamp with zero table dependency and fail hard on mismatch — "schema v4 required, found v3" — instead of rotting silently against a moved schema (the pre-rename specimen class: "no such table"). `INIT` is reserved for genuinely repeatable database posture or seeds, never schema creation or a manual `user_version` assignment.
...
**Web search and retrieval are one first-class composition.** {§web-search-retrieval} A search runtime enumerates candidate URLs and hands each to the engine as a `content: null` `entry()` request (§exec-entry-sink): the guarded `WebFetcher` sink fetches every candidate — in parallel, off the write-serialization chain — and materializes successful bodies as ordinary HTTP entries. Discovery membership and rank remain SearXNG facts: a rejected fetch does not erase valid title/URL/snippet evidence or promote accessible junk above an authoritative result (#596). The executor therefore preserves the ranked digest (`title/url/snippet/publishedDate`) and reports `materialized: true|false`; without an entry sink it omits the verdict. Weights never ride the digest — FIND over successfully tagged entries is the real-weight listing, its body-less catalog rows carrying `{mimetype, tokens, lines}` natively. Because the search family is in `PLURNK_SERVICE_EXEC_HOLD` the cycle holds until the stream concludes (§exec-hold-until-concluded), and the executor awaits every `entry()` before it concludes — so the digest the model's next packet carries contains final materialization verdicts with no separate pass. Search prefetch and direct HTTP READ materialize the same decisive shape: configured readable projection in `body`, faithful DOM in `html`, response metadata + freshness validators in `header`, absolute identity under protocol+authority+path. FIND and matcher READ query that STORED model-facing body; they never re-fetch each match. A materialized page is therefore an ordinary hand-READable/queryable page, never a magic search entry. Deterministic integration coverage must compose search→materialize→FIND/matcher READ without network; the live lane must require real direct retrieval, real HTML projection, real search, and at least one materialized result body; demos must exercise multiple search/retrieval questions through a live model. **The search gates** (#406, owner ruling) are rail-family accounting — in-memory per-loop state cleaned at the same seam as strikes, restart-drop accepted (a post-restart duplicate re-fetches; the TTL makes it cheap): an IDENTICAL duplicate (same runtime + command in one loop) **strikes and serves** — status 409 (the strike rail counts the turn failure) carrying the prior ranked digest re-read live from the original exec entry, no re-fetch, no provenance prose; the per-turn CAP (`PLURNK_SERVICE_SEARCH_MAX_PER_TURN`) is flood control — 429 with a legible steer, nothing served. {§search-gate}
...
**Forensic cardinality.** The digest's machine-readable JSON preserves every log row. Its human Markdown waterfall groups identical per-turn op outcomes and typed `entry_materialized` narrations, reporting the exact count and sequence span (`×N (seq A–B)`). Thus amplification is conspicuous without making the diagnostic artifact itself pathological; packet files remain byte-identical records of what the model saw.
