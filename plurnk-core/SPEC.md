...
The worker's **first** model-emission row is exceptional: a born-OPEN turn-0 **exemplar** — a minimal worked example (`# PLAN0` → four environment `## FIND0` sections classified `init` → `## SEND0 [102]`) the model always opens on, so the grammar can stay thin (the example teaches the syntax, not a heavy grammar). Its SEND signal `102` names the next action, matching the continuation contract. {§model-entry}
...
- §find-result-projection **The authored target shape determines the result unit; result cardinality never changes it** ({§find-result-unit}). Returns `FindResult { status, content, mimetype, results, range, matchingPathCount, matchLocationCount, itemsTokenTotal, returnedItemsTokenTotal }`:
  ...
  `{ path, stream?, channels: { <uri>: { mimetype, tokens, lines } } }`.
