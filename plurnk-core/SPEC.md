...
- §overflow-turn-script **The recovery turn explains and classifies itself through an ordinary admitted program.** Its OPEN exact `turnOps` and dispatched rows contain a blunt PLAN naming Token Budget Overflow and the measured usage/ceiling/deficit, every exact FOLD, and terminal `SEND0 [102]`. Those rows have `origin="_plurnk"` and the classifications `_plurnk` and `overflow`; no model call or parallel receipt exists. The FOLD rows and effects remain durable under {§fold-open-meta-operations}.
...
carries the recovery turn's OPEN `turnOps`, actual PLAN, FOLD, and SEND rows, the one ephemeral
