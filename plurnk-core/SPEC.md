...
- **Per-turn weight.** A markdown table groups rendered log weight by turn - the `loop/turn` coordinate prefix - listed chronologically (oldest first). It reports packet composition without prescribing an action. {§tokenomics-turn-totals}
- **Largest open bodies.** A second table ranks up to `PLURNK_SERVICE_BUDGET_LARGEST_ITEMS` currently open log bodies by the same body-token number their log rows show (#466). Each row carries its `log:///<coord>/<op>` handle. Bodyless and already-folded rows are absent because neither has an open body in the current packet. The default is five; zero omits the ranking. {§tokenomics-largest-open-bodies}
...
- **Budget telemetry is state, not instruction.** The same categories of measurement render regardless of occupancy. Plurnk does not hide telemetry below an arbitrary threshold, label rows as FOLD targets, prescribe which history the model should curate, or carry a standing context-curation command in its personality. OPEN/FOLD/KILL remain documented capabilities; recovery directs the model only after overflow. The model receives accurate state and owns its context decisions. {§tokenomics-neutral-telemetry}
- **The Budget renders as visual Mermaid.** {§budget-mermaid} By default (`PLURNK_SERVICE_BUDGET_MERMAID`; set `off` for the tabular presentation), the Budget section keeps its ceiling/usage/free headline and replaces the per-turn table with two budget-scaled diagrams: a `treemap-beta` in which turn boxes, `system + context`, and `free` compose the ceiling; and a used-vs-free `pie`. The largest-open-bodies ranking remains a table because a ranking is not a composition (#450). `free`, `used`, and `system + context` resolve post-assembly like the headline figures. Syntax is validated in `plurnk-plurnkdown/demo/budget-mermaid.md`. #440
