...
- §tokenomics-provider-usage **Provider usage is transport and cost evidence, not curation state.** Every provider attempt preserves its validated `{§provider-cost}` evidence beside provider-reported prompt, completion, reasoning, and cached quantities. Turns and loops retain the ordered evidence. Their numeric USD projection is `null` if any included cost is unknown; free contributes zero, but unknown never does. Exact decimal evidence remains authoritative over the aggregate floating-point projection. Reasoning and completion are outputs the model cannot FOLD, so they never alter the model-facing Budget ledger.
