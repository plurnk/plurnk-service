...
`example` renders every turn — keep it terse. `documentation` is the deep prose; the consumer materializes it at `worker://~/_plurnk/skills/plurnk/<name>.md` for catalog discovery and on-demand READ. Don't dump prose into `example` (it floods the hot path) — put it in `documentation`.
