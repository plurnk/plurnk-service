...
Opt-in artifact package: the framework resolves it lazily by name; absent, the
seam degrades to a chars/2 estimate with a `tokenizer_unavailable` Notice —
never a silent estimate. The estimate is neither exact nor a proven upper
bound; correctness-sensitive consumers must reject it or apply their own
defensible policy.
